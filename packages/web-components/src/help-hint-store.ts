...
const HELP_HINT_NAMESPACE = "agenter:web-components:help:dismissed:v1";

const dismissedCache = new Map<string, boolean>();
const keyCache = new Map<string, string>();

export interface HelpHintIdentity {
  textContext: string;
  helpId?: string;
}
...
const keySeed = (input: HelpHintIdentity): string => {
  const context = normalize(input.textContext);
  return input.helpId ? `${input.helpId}\n${context}` : context;
};

export const resolveHelpHintDismissedKey = async (input: HelpHintIdentity): Promise<string> => {
...
export const readHelpHintDismissed = async (input: HelpHintIdentity): Promise<boolean> => {
...
export const dismissHelpHint = async (input: HelpHintIdentity): Promise<void> => {
  const key = await resolveHelpHintDismissedKey(input);
