...
import { AppRuntimeClient, type AppEnsureTerminalBindingInput, type AppRuntimeStore } from "../src";
...
class FakeAppRuntimeStore implements AppRuntimeStore {
...
describe("Feature: app runtime client", () => {
  test("Scenario: Given repeated runtime ensures for the same avatar When the shell name changes Then runtime identity stays avatar-scoped instead of app-session-scoped", async () => {
  ...
  test("Scenario: Given stale sessions share one Avatar principal When the app clears it Then principal authority removes every matching runtime context", async () => {
    const store = new FakeAppRuntimeStore();
    const client = new AppRuntimeClient(store);
    ...
    expect(cleared.clearedSessionIds).toEqual(["session:/repo:review-4", "session:/other-repo:review-4"]);
    expect(store.deletedSessionIds).toEqual(["session:/repo:review-4", "session:/other-repo:review-4"]);
    expect([...store.sessions.values()].map((session) => session.id)).toEqual([]);
  ...
  test("Scenario: Given generic app bindings When ensuring missing terminal and room resources Then metadata keys drive creation grants and focus without Shell branches", async () => {
    const store = new FakeAppRuntimeStore();
    const client = new AppRuntimeClient(store);
    ...
    expect(terminal.entry.metadata?.appId).toBe("shell");
    expect(room.created).toBe(true);
    expect(room.granted).toBe(true);
    expect(room.entry.metadata?.resourceKey).toBe("shell-1");
  });

  test("Scenario: Given a killed terminal kept in history When ensuring a app binding with the same resource key Then the runtime client creates a fresh TerminalSystem id instead of colliding with history", async () => {
    const store = new FakeAppRuntimeStore();
  ...
  test("Scenario: Given existing bound resources When ensuring them again Then app metadata reuses the resources and skips duplicate grants", async () => {
    const store = new FakeAppRuntimeStore();
  ...
  test("Scenario: Given an existing stopped terminal binding When the app requires start on reconnect Then the runtime client bootstraps the durable terminal before reuse", async () => {
    const store = new FakeAppRuntimeStore();
  ...
  test("Scenario: Given an existing legacy terminal without app metadata When ensuring the binding again Then the runtime client reconciles durable launch truth before reuse", async () => {
    const store = new FakeAppRuntimeStore();
  ...
  test("Scenario: Given self-evolution attention When invoking the runtime client Then attention does not expose app delegation authority", async () => {
    const store = new FakeAppRuntimeStore();
    const client = new AppRuntimeClient(store);
