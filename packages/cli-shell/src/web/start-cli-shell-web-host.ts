...
} from "./web-product-host";

const CLI_SHELL_WEB_HOST_TITLE = "agenter shell";
const CLI_SHELL_WEB_HOST_DESCRIPTION =
  "cli-shell --web hosts the existing backend terminal truth in a browser projection. Bun.Terminal-style PTY helpers are optional harness infrastructure only, not product truth.";
...
const postProductEvent = async (action: unknown): Promise<void> => {
  ...
  if (!response.ok) {
    throw new Error(\`cli-shell product event failed: \${response.status}\`);
  }
};
            ...
            if (request.method === "POST" && requestUrl?.pathname === CLI_SHELL_WEB_PRODUCT_EVENT_PATH) {
              try {
                ...
                if (!isCliShellWebProductHostAction(body)) {
                  writeJsonResponse(response, 400, { error: "invalid cli-shell product event" });
                  return;
                }
                ...
                writeJsonResponse(response, 400, {
                  error: error instanceof Error ? error.message : "invalid cli-shell product event payload",
                });
              }
              return;
            }
