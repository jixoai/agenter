...
const buildFrontendBuildPrompt = (workspacePath: string): string =>
  [
    ...
    "你现在只有 root_workspace_list 和 root_workspace_bash 这两个直接工具。",
    ...
    "如果你忘了 terminal CLI 的格式，先在 shell 里执行 ccski info agenter-terminal。",
...
const buildFrontendDiskFeedbackPrompt = (workspacePath: string): string =>
  [
    ...
    "先用 root_workspace_bash 进入 shell。",
...
const buildPrivateFrontendDiskReminder = (workspacePath: string): string =>
  [
    ...
    `唯一正确的工作目录是：${workspacePath}`,
    "不要假设系统默认给你 terminal。现在只做这一件事：先用 root_workspace_bash 进入 shell。",
...
const buildBackendDeliveryPrompt = (url: string, workspacePath: string): string =>
  [
    ...
    "你现在只有 root_workspace_list 和 root_workspace_bash 这两个直接工具。",
    "长期服务只放在 terminal 里启动；一次性的监听/日志检查放在 root_workspace_bash。",
    `唯一正确的工作目录是：${workspacePath}`,
    "先用 root_workspace_list 看清已挂载路径，再用 root_workspace_bash 进入 shell。",
    ...
    `启动后立刻用 root_workspace_bash 检查 lsof -nP -iTCP:${new URL(url).port} -sTCP:LISTEN 是否看到 LISTEN。`,
    "再用 root_workspace_bash 检查工作目录里的 server.log，确认出现 Server running ...。",
...
const buildBackendTerminalRecipe = (url: string, workspacePath: string): string[] => [
  "先用 root_workspace_bash 进入 shell。",
  "长期服务在 terminal 启动；一次性检查放在 root_workspace_bash。",
  ...
  `如果 terminal read 只回显你自己的命令，不要在 terminal 里反复盲试；回到 root_workspace_bash 做客观检查。`,
  `用 root_workspace_bash 检查 lsof -nP -iTCP:${new URL(url).port} -sTCP:LISTEN。`,
  "再用 root_workspace_bash 检查 server.log 是否出现 Server running ...。",
...
const buildDeliveryFeedbackPrompt = (input: {
  ...
  [
    ...
    "先用 root_workspace_bash 进入 shell；如果当前没有 terminal，就先执行 terminal create --help，然后用 JSON 形式创建显式 cwd 的 terminal；如果已经有 terminal，就先恢复它。",
    "先修好 server.js 和服务，再用 root_workspace_bash 检查 LISTEN 与 server.log，最后只发送一条新的 PROJECT-URL。",
...
const buildMissingProjectUrlFeedbackPrompt = (expectedUrl: string, workspacePath: string): string =>
  [
    ...
    "先用 root_workspace_bash 进入 shell；如果当前没有 terminal，就先执行 terminal create --help，然后用 JSON 形式创建一个显式 cwd 的 terminal；如果已有 terminal，就先恢复它。",
    `再次确认当前目录必须是：${workspacePath}`,
    "如果 server.js 还没真正写到磁盘，就先把它写到这个目录；不要前台卡住，必须后台运行，并把输出重定向到 server.log。",
    `然后立刻用 root_workspace_bash 检查 lsof -nP -iTCP:${new URL(expectedUrl).port} -sTCP:LISTEN`,
    "再用 root_workspace_bash 检查 server.log 是否已经出现 Server running ...。",
...
const buildPrivateProjectUrlReminder = (input: {
  ...
  [
    ...
    input.serviceReady
      ...
      : `先恢复或创建 terminal，确认 cwd 是 ${input.workspacePath}，确保 server.js 已写好、服务已后台启动；监听和日志检查走 root_workspace_bash。`,
    input.serviceReady
