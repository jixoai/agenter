...
export const runRealRoomTerminalDeliveryScenario = async (
  ...
  try {
    const prompt = [
      ...
      "如果你忘了 terminal CLI 的格式，先执行 terminal create --help；只有在连 terminal 命令族本身都不清楚时，才允许再看 skill info agenter-terminal。长期服务放在 terminal 里跑，一次性检查放在 root_workspace_bash 里做；验证 URL 时优先用 curl。",
      ...
      `第 4 步：在发链接前，继续通过 root_workspace_bash 自行验证 ${url} 可访问。`,
