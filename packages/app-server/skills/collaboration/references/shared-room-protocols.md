...
- if the room expects both `/` and `/api/status` on one shared URL, the service owner should make both paths work on that same listener before the final delivery reply
- another participant saying "received" or confirming their own work does not automatically mean you need to reply again
