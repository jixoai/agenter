...
- NoteSystem 是 raw-note recording atom：文件 artifact 保持人类可读，但稳定身份、tags、references、MIME、rename 与 SQL query authority 属于 active `AVATAR_HOME` 下的 NoteSystem SQLite index。页面身份为 notebook -> section -> page，并返回 `bookId`、`sectionId`、`pageId`、`tagIds`、timestamps、MIME 和 normalized reference edges。
- NoteSystem CLI 是 `AVATAR_HOME` gated 的 JSON-first runtime CLI：`note draft/write/list/show/search/tags/query/rename` 使用 descriptor-backed JSON schema/help；legacy flag shape 只作为兼容入口，不是 AI-facing contract。
...
- `note` built-in skill 由 app-server package-owned skill catalog 提供，用于指导 AI 通过 JSON commands、tags、references、rename、MIME 和 read-only SQL 记录和验证 raw notes，而不是把 notes 伪装成 distilled memory。
- browser-facing NoteSystem browsing 必须走 typed surface：`note.catalog`、`note.page`、`note.search`、`note.tags`、`note.query` 以及 mutation projections for `note.write` / `note.rename` 由 app-server 基于当前 avatar `AVATAR_HOME` 派生，Studio/client-sdk 不得直接读取 `notes/*` 文件或 import `packages/app-server/src/note-system/*`。
