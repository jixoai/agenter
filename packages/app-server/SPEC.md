...
- app-server 的 browser/tRPC MCP projection 只能作为 selected AvatarRuntime `McpSystemSurface` 的 typed forwarding plane：它可以暴露 add/remove/enable/disable/list/query/start/stop/restart/invoke，但不得在 router 层重写 global/project/lifecycle/snapshot 语义、不得创建第二份 MCP store、也不得把 MCP invocation 注入成 provider-side hidden direct tool。
