...
- mcpSystem 持久化根属于 Avatar-owned private authority：global configs、project enablements、instance recovery state、snapshots 与 action facts 落在对应 Avatar principal 的私有 SQLite；`AppKernel` 按 Avatar principal 解析 `McpSystemSurface`，不要求任何 AvatarRuntime 正在运行。`mcp query` 只读投影 `mcp_installed` / `mcp_enabled`，不暴露 raw durable tables 或第三张 capability table。
...
- app-server 的 browser/tRPC MCP projection 只能作为 selected Avatar-owned `McpSystemSurface` 的 typed forwarding plane：它可以暴露 add/remove/enable/disable/list/query/start/stop/restart/invoke/probe，但不得在 router 层重写 global/project/lifecycle/snapshot 语义、不得创建第二份 MCP store、也不得把 MCP invocation 注入成 provider-side hidden direct tool。`mcp probe` 是 CLI-backed lightweight inspection surface：它打开 isolated MCP client session，支持 connect snapshot、ping、tool call、resource read、prompt get、completion 和 close，并返回 `stdin/stdout/stderr/exitCode` envelope；probe 不得写入 global config、project enablement、durable snapshot、instance record 或 action fact。
...
- daemon 启动时必须把平台内置 prompt 文档覆盖写入 `~/.agenter/builtin/<lang>/`；builtin prompt root 是平台管理资产，不属于 `AVATAR_HOME`，也不得作为 `super:` 的隐式父层。Avatar `AGENTER.mdx` 若要继承平台默认提示词，必须显式写 `<Slot src="global:builtin/$LANG/AGENTER.mdx" />`。
- Prompt 混合只能通过 MDX `<Slot src="..."/>` 完成，并且嵌套 Slot 必须继承父层 slot variables。`src` 必须按 URL 解析：`private:` 指向当前 prompt layer 的 private root，`public:` 指向当前 prompt layer 的 public root，`super:` 只指向当前 prompt layer 的父层同类 root，`global:` 指向 `~/.agenter` 全局资源根并支持 `$LANG` / `${LANG}` 展开；标准 `file://`、`http://`、`https://` 等协议由 resource loader 处理；`npm:<package-name>/<relative-file>` 必须先走 package `exports`，找不到再走 package-relative path；`app:<app-id>/<relative-file>` 必须通过 app package metadata 映射到对应 npm package resource。空 `super:` 表示当前文件在父层 root 下的同相对路径；没有父层时为空，不得 fallback 到 builtin。
