...
const RUNTIME_SLOT_PADDING: Record<IconSlotKind, number> = {
  bottomRight: 0.08,
  center: 0.14,
  topLeft: 0.08,
};
...
const createClipPath = (slot: IconSlotKind, bounds: SlotBounds): string =>
  `<clipPath id="clip-${slot}" clipPathUnits="userSpaceOnUse"><rect x="${bounds.x}" y="${bounds.y}" width="${bounds.width}" height="${bounds.height}" rx="${bounds.radius}" ry="${bounds.radius}"/></clipPath>`;
...
const unwrapSvgMarkup = (
  markup: string,
): { inner: string; viewBox: [number, number, number, number] | null } => {
  const svgMatch = markup.match(
    /<svg[^>]*viewBox="([^"]+)"[^>]*>([\s\S]*?)<\/svg>/iu,
  );
  if (!svgMatch) {
...
const fitMarkupToBounds = (
  ...
  const scale =
    ...
    clampSlotScale(scaleMultiplier);
  const dx = bounds.x + (bounds.width - width * scale) / 2 - minX * scale;
  const dy = bounds.y + (bounds.height - height * scale) / 2 - minY * scale;

  // Keep clip-path on the outer group. Some rasterizers drop the content when
  // clip-path and transform live on the same node.
  return `<g clip-path="url(#clip-${slot})"><g color="${color}" fill="${color}" stroke="${color}" transform="translate(${dx.toFixed(3)} ${dy.toFixed(3)}) scale(${scale.toFixed(5)})">${markup}</g></g>`;
};
...
const resolveSlotMarkup = (
  ...
  runtimeMarkup: string | undefined,
  ...
  if (runtimeMarkup?.trim()) {
    const runtimeSvg = unwrapSvgMarkup(runtimeMarkup);
    return fitMarkupToBounds(
      slot,
      runtimeSvg.inner,
      runtimeSvg.viewBox ?? [0, 0, bounds.width, bounds.height],
      bounds,
      color,
      RUNTIME_SLOT_PADDING[slot],
      slotScale,
    );
  }
...
const normalizeMap = <TValue extends { id: string }>(
  items: TValue[],
): SvgAssetMap<TValue> =>
...
export const parseGeometryFromSvg = (sourceSvg: string): IconGeometry => {
  ...
  const paths = Object.fromEntries(
    Array.from(
      sourceSvg.matchAll(/<path[^>]*id="([^"]+)"[^>]*d="([^"]+)"[^>]*>/gu),
...
export const renderIconSvg = (input: IconRenderInput): string => {
  ...
  if (!background) {
    throw new Error(
      `Unknown background token: ${input.config.backgroundToken}`,
    );
  }
  ...
  const ribbonPaths = [
    ...
    renderPathGroup(
      ...
      "url(#caps-b)",
      new Set([
        ...SLOT_PATH_EXCLUSIONS.topLeft,
        ...SLOT_PATH_EXCLUSIONS.bottomRight,
      ]),
    ),
  ].join("");
...
export const renderSlotPreviewSvg = (options: {
  color: string;
  customMarkup?: string;
  ...
  const bounds = { ...SLOT_BOUNDS[options.slot], x: 0, y: 0 };
  const slotMarkup = resolveSlotMarkup(
    options.slot,
    options.slotId,
    options.customMarkup,
    options.runtimeMarkup,
    normalizeMap(options.slotCatalog),
    bounds,
    options.color,
    options.scale,
  );

  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${bounds.width} ${bounds.height}" width="${bounds.width}" height="${bounds.height}"><defs>${createClipPath(options.slot, bounds)}</defs><rect width="${bounds.width}" height="${bounds.height}" rx="${bounds.radius}" ry="${bounds.radius}" fill="${options.surfaceFill ?? "#ffffff"}" fill-opacity="0.88" stroke="${options.surfaceStroke ?? "#c4ccd6"}" stroke-opacity="0.68"/><g>${slotMarkup}</g></svg>`;
};
