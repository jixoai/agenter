...
describe("Feature: ScrollView source contract", () => {
  test("Scenario: Given Studio feature and route code When scanning for raw scroll ownership Then only shared UI primitives may keep overflow-based scrolling", () => {
  ...
  test("Scenario: Given Studio feature and route code When scanning for flex-grid layout patch classes Then min-h-0 is not used outside shared primitives", () => {
  ...
  test("Scenario: Given Studio feature and route code When scanning for multipart shadcn alias usage Then only canonical Root-slot composition remains", () => {
    const violations = featureAndRouteRoots
      .flatMap((root) => walkFiles(root))
