...
describe("Feature: Room route hydration stability contract", () => {
  ...
  test("Scenario: Given app-view owns the embedded transcript When reading the route source Then Studio no longer forges chat send or read-ack effects outside the iframe", () => {
    expect(messageRoomRouteSource).toContain("const selectedViewerAccessToken = $derived.by(() => {");
    expect(messageRoomRouteSource).toContain("{selectedViewerAccessToken}");
    expect(messageRoomRouteSource).not.toContain("latestVisibleMessageByRoomId");
    expect(messageRoomRouteSource).not.toContain("latestVisibleReplayKeyByRoomId");
    expect(messageRoomRouteSource).not.toContain("handleLatestVisibleMessageIdChange");
    expect(messageRoomRouteSource).not.toContain("markGlobalRoomRead");
    expect(messageRoomRouteSource).not.toContain("sendGlobalRoomMessage");
    expect(messageRoomRouteSource).not.toContain("uploadRoomAssets");
  });
  ...
  test("Scenario: Given the browser is not authenticated When reading the room route source Then stale room tokens cannot keep the room composer or room mutations active", () => {
    ...
    expect(messageRoomRouteSource).not.toContain("initialSnapshotResolved={");
    expect(messageRoomRouteSource).not.toContain("initialMessages={");
