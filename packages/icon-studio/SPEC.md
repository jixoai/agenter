...
- 工具内部可以复用 workspace 级共享 primitive，例如 `@agenter/svelte-components`，但不能依赖 `@agenter/studio` 的本地 feature、路由或主题文件。
- `assets/*` 这类离线构建脚本只允许依赖 `icon-studio` 的包级导出契约，不允许跨目录 import app route 层。
- `@agenter/studio` 与 `@agenter/icon-studio` 只在品牌与资产层保持一致，不共享运行时路由树。
