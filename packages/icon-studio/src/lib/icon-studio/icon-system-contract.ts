...
export const LUCIDE_SLOT_PREFIX = "lucide:";
export const DEFAULT_SLOT_SCALE = 1;
export const MIN_SLOT_SCALE = 0.1;
export const MAX_SLOT_SCALE = 2;
export const SLOT_SCALE_STEP = 0.01;
...
export type LucideIconAssetMap = Record<string, LucideIconAsset>;
...
export interface IconPreset {
  ...
  label: string;
  paletteToken: string;
  slotScale?: Partial<Record<IconSlotKind, number>>;
...
export interface IconComposerConfig {
  ...
  paletteToken: string;
  slotScale?: Partial<Record<IconSlotKind, number>>;
  slotMarkup?: Partial<Record<IconSlotKind, string>>;
  slots: Record<IconSlotKind, string>;
  theme: IconTheme;
}
...
export const clampSlotScale = (value: number | null | undefined): number => {
  const normalizedValue =
    typeof value === "number" ? value : DEFAULT_SLOT_SCALE;
  if (!Number.isFinite(normalizedValue)) {
    return DEFAULT_SLOT_SCALE;
  }
  return Math.min(MAX_SLOT_SCALE, Math.max(MIN_SLOT_SCALE, normalizedValue));
};
