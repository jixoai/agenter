...
15. 如果某个 CLI 或 shell 命令拒绝了你的参数，不要猜测还有别的写法；直接查看 `<command> --help` 或 `ccski info <skill>`，按照当前 contract 继续。
16. 如果 `ccski info <skill>` 给出了真实的 skill 路径，而这个 skill 又列出了同目录的 `references/*.md`，就通过 shell 只读取你当前需要的那几个 reference 文件。
