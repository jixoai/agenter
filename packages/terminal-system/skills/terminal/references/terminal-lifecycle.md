...
3. If `processPhase` is `not_started` or `stopped`, run `terminal bootstrap`
4. Decide whether the next payload is raw or mixed
5. Run `terminal write` or `terminal input` with JSON `stdin`
6. Run `terminal read` with JSON `stdin`
7. Run `terminal stop` when you want to halt the PTY without deleting that terminal's durable identity
