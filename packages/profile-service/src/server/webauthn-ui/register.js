...
var root_2 = from_html(`<p class="error svelte-vreegn"> </p>`);
var root_3 = from_html(`<p class="svelte-vreegn"><strong>Profile:</strong> </p> <textarea readonly="" rows="6" class="svelte-vreegn"></textarea>`, 1);
var root = from_html(`<main class="shell svelte-vreegn"><section class="card svelte-vreegn"><header class="header svelte-vreegn"><p class="eyebrow svelte-vreegn">Profile Service</p> <h1 class="svelte-vreegn">Register passkey</h1> <p class="copy svelte-vreegn">Finish email bootstrap by creating a WebAuthn credential for this profile.</p></header> <label class="field svelte-vreegn"><span>Registration ticket</span> <input placeholder="ticket id" spellcheck="false" class="svelte-vreegn"/></label> <button class="primary svelte-vreegn"> </button> <div class="status svelte-vreegn"><p class="svelte-vreegn"><strong>Status:</strong> </p> <!> <!></div></section></main>`);

function Register_page($$anchor, $$props) {
	push($$props, true);

	let ticket = state(proxy(readSearchParam("ticket")));
	...
	const runRegistration = async () => {
		...
		try {
			...
			const verified = await jsonFetch("/auth/webauthn/register/verify", {
				method: "POST",
				headers: { "content-type": "application/json" },
				body: JSON.stringify({ ticketId: get(ticket), response })
			});
