...
- product launcher 与 core TUI 在复用 daemon 前必须校验 launcher identity compatibility；同一 package/version/sourceKind 的 workspace checkout 或 worktree 可复用同一 runtime home root daemon，但 package 版、workspace 版、不同版本或旧 daemon 缺少 identity 时必须给出明确错误并要求用户用当前命令执行 `agenter daemon restart`，不得静默连到错误 server。
