...
describe('WatcherPool', () => {
  ...
  describe('initWatcherPool()', () => {
    ...
    it('tracks watcher runtime state across multiple roots', async () => {
      ...
      await Promise.all([
        ensureWatcherRootForPath(join(homeDir, 'prompt.mdx')),
        ensureWatcherRootForPath(join(workspaceDir, 'settings.json')),
        ensureWatcherRootForPath(join(packageDir, 'ShellAssistant.mdx')),
      ])

      const statuses = listWatcherRuntimeStatuses()

      expect(statuses.map((status) => status.rootDir)).toEqual(
        expect.arrayContaining([realpathSync(homeDir), realpathSync(workspaceDir), realpathSync(packageDir)])
      )
      expect(statuses.every((status) => status.initialized)).toBe(true)
    })
  })
