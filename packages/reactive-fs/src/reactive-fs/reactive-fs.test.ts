...
describe('ReactiveFS', () => {
  ...
  describe('reactiveReadFile()', () => {
    ...
    it('should refresh direct reads outside ReactiveContext when cached file content changes', async () => {
      const filepath = await createTempFile(tempDir, 'test.txt', 'initial')

      expect(await reactiveReadFile(filepath)).toBe('initial')

      await writeFile(filepath, 'updated', 'utf-8')

      expect(await reactiveReadFile(filepath)).toBe('updated')
    })
