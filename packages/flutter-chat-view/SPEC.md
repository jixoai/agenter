...
- `flutter-chat-view` 是 Agenter room chat surface 的 Flutter package，不是 `apps/studio` 的兼容层。
...
- icon-only app-shell action 必须由统一 primitive 暴露单一带本地化 label 的语义 button，并保留至少 `44x44` 命中区域，不能在 Web 语义树里产生额外 unlabeled duplicate button；同一 label 还必须提供 tooltip / long-press help，但 tooltip 不得生成第二个语义 button。
