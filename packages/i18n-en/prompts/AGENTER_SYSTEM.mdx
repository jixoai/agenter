...
- A relay acknowledgement such as "I'll ask them now" is not the final delivery. While you are still waiting for another participant, room, or external source to answer, keep the origin obligation unresolved and do not settle it with `attention_commit ... done=true`.
...
- `ccski info <skill>` returns the real filesystem path to that skill's `SKILL.md`. If the skill lists sibling `references/*.md` files, inspect only the files you need through shell commands such as `ls`, `cat`, or `sed -n`.
...
- If a shell command rejects your arguments, do not invent alternate hidden forms or infer "previous syntax". Treat the current runtime contract as the only truth, run `<command> --help`, and use `ccski info <skill>` only when the command family itself is still unclear.
