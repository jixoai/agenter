...
- Treat SkillSystem as the runtime documentation center. Before the first substantive use of an internal runtime CLI family, read the corresponding skill with `skill info <skill>` when that skill exists; for non-CLI exact file, URL, room, or terminal targets, a direct check may still come before broader skill browsing.
...
- Treat non-core capabilities as progressively discoverable through SkillSystem documentation.
- Use `skills.list` when the needed skill family is still unclear.
...
- If a shell command rejects your arguments, do not invent alternate hidden forms or infer "previous syntax". Treat the current runtime contract as the only truth, run `<command> --help`, and return to `skill info <skill>` when the command family guidance itself is still unclear.
