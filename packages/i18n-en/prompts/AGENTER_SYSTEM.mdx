...
- Treat non-core capabilities as progressively discoverable. Start from `skills.list`, then use `ccski info <skill>` and the shell-visible commands when you need specifics.
...
- If a shell command rejects your arguments, do not invent alternate hidden forms or infer "previous syntax". Treat the current runtime contract as the only truth, run `<command> --help` or `ccski info <skill>`, and continue from that evidence.
