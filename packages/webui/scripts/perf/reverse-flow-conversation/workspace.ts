...
const renderViteConfig = (targetRoot: string): string => {
  const bitsUiPluginPath = path.join(targetRoot, "packages/webui/vite.bits-ui-style-plugin.ts");
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      "$lib": path.join(targetRoot, "packages/webui/src/lib"),
      "@perf-target-layout-css": path.join(targetRoot, "packages/webui/src/routes/layout.css"),
      "@perf-target-web-chat-view": path.join(targetRoot, "packages/web-chat-view/src"),
      "@perf-target-webui-runtime": path.join(targetRoot, "packages/webui/src/lib/features/runtime"),
    },
