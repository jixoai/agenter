...
import {
  ...
  type SchedulerInputSignals,
  type SchedulerKernelState,
  type SchedulerPhase,
  type SchedulerStateLogItem,
} from "./observability-panel-data";
import { StatusIcon } from "./ObservabilityPanelShared";

interface ObservabilitySchedulerSectionProps {
  activePhase: SchedulerPhase;
...
export const ObservabilitySchedulerSection = ({
  activePhase,
  kernel,
  inputSignals,
  logs,
  ...
  apiCalls,
  cycleDigests,
}: ObservabilitySchedulerSectionProps) => (
  <ScrollViewport className="flex flex-1 flex-col gap-2 rounded-lg bg-slate-50 p-2">
    <section className="@container grid grid-cols-2 gap-2 text-[11px] md:grid-cols-4">
      ...
      <div className="rounded-lg bg-slate-100 px-2 py-1.5 text-slate-700">
        ...
        <p className="mt-1 text-[10px] text-slate-500">
          {kernel?.lastWakeCause ?? kernel?.lastWakeSource ?? "-"} @ {toTime(kernel?.lastWakeAt)}
        </p>
      </div>
      <div className="rounded-lg bg-slate-100 px-2 py-1.5 text-slate-700">
        <InlineAffordance className="font-medium">
          ...
          <InlineAffordanceLabel>attention debt</InlineAffordanceLabel>
        </InlineAffordance>
        <p className="mt-1 text-[10px] text-slate-500">
          ctx {kernel?.activeContextCount ?? 0} · items {kernel?.activeItemCount ?? 0} · scores{" "}
          {kernel?.unresolvedScoreCount ?? 0}
        </p>
      </div>
      <div className="rounded-lg bg-slate-100 px-2 py-1.5 text-slate-700">
        <InlineAffordance className="font-medium">
    ...
    <div className="rounded-lg bg-white px-2 py-2 text-[11px] text-slate-600 ring-1 ring-slate-200">
      <div className="flex items-center justify-between gap-2">
        ...
        <span>{kernel?.runtimeStatus ?? kernel?.waitingReason ?? kernel?.gate ?? "waiting_input"}</span>
      </div>
      <p className="mt-1 text-[10px] text-slate-500">
        updated {toTime(kernel?.updatedAt)} · next auto wake {toTime(kernel?.nextAutoWakeAt)} · backoff{" "}
        {kernel?.backoffMs ?? 0} ms
      </p>
      <p className="mt-1 text-[10px] text-slate-500">
        reason {kernel?.waitingReason ?? "none"} · retry {kernel?.retryCount ?? 0} · last progress{" "}
        {toTime(kernel?.lastProgressAt)}
      </p>
      <p className="mt-1 text-[10px] text-slate-500">
        inputs u{inputSignals.user.version} t{inputSignals.terminal.version} k{inputSignals.task.version} a
        {inputSignals.attention.version} · response {toTime(kernel?.lastResponseAt)} · error{" "}
        {kernel?.lastError ?? "none"}
      </p>
      {kernel?.blockedReason ? (
        <p className="mt-1 text-[10px] text-amber-700">blocked: {kernel.blockedReason}</p>
      ) : null}
    </div>
    ...
    <div className="rounded-lg bg-white px-2 py-2 text-[10px] text-slate-500 ring-1 ring-slate-200">
      scheduler state patches: {logs.length}
    </div>
  </ScrollViewport>
);
