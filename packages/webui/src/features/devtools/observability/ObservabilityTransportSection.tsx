...
import { PanelHeader, StatusIcon } from "./ObservabilityPanelShared";

interface ObservabilityTransportSectionProps {
...
export const ObservabilityTransportSection = ({
  modelCalls,
  hasMoreModel,
  loadingModel,
  onLoadMoreModel,
}: ObservabilityTransportSectionProps) => {
  ...
  return (
    <div className="grid h-full grid-rows-[auto_minmax(0,1fr)] rounded-lg bg-slate-50">
      ...
      <AsyncSurface
        ...
        empty={<p className="px-3 py-3 text-[11px] text-slate-500">No transport records yet.</p>}
        className="flex-1"
      >
                    ...
                    >
                      <AccordionTrigger className="py-2 hover:no-underline">
                        <InlineAffordance className="flex flex-1" fill>
                          <InlineAffordanceLeadingVisual>
                            <StatusIcon status={toStatusTone(call.status)} />
                          </InlineAffordanceLeadingVisual>
