...
export const findSlashCommandToken = (value: string, cursor: number): SlashCommandToken | null => {
  const safeCursor = Math.max(0, Math.min(cursor, value.length));
  let start = safeCursor;
  while (start > 0) {
    const previous = value[start - 1];
...
export const replaceSlashCommandToken = (
  value: string,
  token: Pick<SlashCommandToken, "from" | "to">,
  replacement: string,
): { value: string; cursor: number } => {
  const normalizedReplacement = replacement.startsWith("/") ? replacement : `/${replacement}`;
  const nextValue = `${value.slice(0, token.from)}${normalizedReplacement}${value.slice(token.to)}`;
