...
const readUsage = (
	...
	const promptTokens = toPositiveNumber(usage.promptTokens) ?? 0;
	const completionTokens = toPositiveNumber(usage.completionTokens) ?? 0;
	const totalTokens = toPositiveNumber(usage.totalTokens) ?? promptTokens + completionTokens;
	if (totalTokens <= 0) {
		return null;
	}
	return {
...
const estimateHeartbeatCost = (
	...
	const band = pickPricingBand(providerMetadata, usage.promptTokens);
	if (!band) {
		return null;
	}
	const inputCost = (usage.promptTokens / 1_000_000) * band.inputPerMillion;
	const outputCost = (usage.completionTokens / 1_000_000) * band.outputPerMillion;
	return {
...
export const buildHeartbeatContextState = (
	...
	const providerLabel = providerMetadata ? [providerMetadata.providerId, providerMetadata.model].filter(Boolean).join(' · ') : null;
	...
	const progress = maxContextTokens ? Math.min(1, usage.totalTokens / maxContextTokens) : null;
	return {
		kind: 'available',
		modelCallId: latestCall.id,
		status: latestCall.status,
		...
		maxContextTokens,
		progress,
		remainingTokens: maxContextTokens ? Math.max(0, maxContextTokens - usage.totalTokens) : null,
...
export const buildHeartbeatStatusState = (input: {
	...
	if (scheduler) {
		switch (scheduler.runtimeStatus) {
			...
			case 'backoff':
				label = 'Backoff';
				tone = 'warning';
				detail = [scheduler.waitingReason ? humanizeToken(scheduler.waitingReason) : null, formatDurationLabel(scheduler.backoffMs)]
	...
	const detailParts = [detail && detail.length > 0 ? detail : null, resourceHint].filter(
