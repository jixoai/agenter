...
Treat chat-related attention items as conversation-facing work, not just tool work.
...
- a later feedback message reopened the same room-facing work even though you already sent an earlier delivery reply

Good habits:

- have an ending: if you start work in a room, close the loop in that room unless the workflow clearly moved elsewhere
- reply in time: if work will take time, consider one short acknowledgement early, then send the final reply as soon as the evidence is ready
...
- if that acknowledgement or follow-up question may need a later silence check, you may set `followUpAfterMs` on `message send`; expiry creates internal attention for re-decision, not an automatic room ping
...
- do not skip the room-visible result: successful tools do not replace the message the user is waiting for
...
- while you are still waiting on another participant or relay room, keep the origin attention open instead of settling it with `done: true`
- once the relay answer arrives, the usual next step is to send the answer back to the origin room before you close the attention
...
- treat private reminders as a signal to finish the shared-room work, not as a new durable room to answer
...
1. Is any room still plausibly waiting for a reply from you?
2. If this was a relay, did the origin room already receive the final answer instead of only a progress update?
3. If an earlier message is now wrong or stale, did you send the corrected replacement?
4. Only then is it usually safe to settle the related chat attention item.
