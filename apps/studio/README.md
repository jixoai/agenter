...
```sh
pnpm --filter agenter-app-studio dev
pnpm --filter agenter-app-studio typecheck
pnpm --filter agenter-app-studio test
pnpm --filter agenter-app-studio build
pnpm --filter agenter-app-studio preview
```
...
- In a workspace checkout, default `agenter studio` serves the canonical static build from `apps/studio/build`; run `bun run build:studio` before static-mode browser verification
