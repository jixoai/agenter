...
describe("Feature: Workspace workbench state primitives", () => {
  ...
  test("Scenario: Given browser CLI discovery mixes workspace runtime and builtin groups When ordering for display Then app commands stay above builtin noise without losing entries", () => {
    const groups: WorkspaceCliCatalogGroup[] = [
      {
  ...
  test("Scenario: Given the raw CLI catalog starts with builtins When route initialization asks for the default detail Then ordered app commands win before builtin fallbacks", () => {
    const groups: WorkspaceCliCatalogGroup[] = [
      {
        id: "just-bash-builtins",
        title: "just-bash builtins",
