...
const renderViteConfig = (targetRoot: string): string => {
  const bitsUiPluginPath = path.join(targetRoot, "apps/studio/vite.bits-ui-style-plugin.ts");
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      "$lib": path.join(targetRoot, "apps/studio/src/lib"),
      "@perf-target-layout-css": path.join(targetRoot, "apps/studio/src/routes/layout.css"),
      "@perf-target-web-chat-view": path.join(targetRoot, "packages/web-chat-view/src"),
      "@perf-target-studio-runtime": path.join(targetRoot, "apps/studio/src/lib/features/runtime"),
    },
