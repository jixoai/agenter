...
const authenticateWithManagedKey = async (page: Page): Promise<void> => {
  ...
  const storedToken = await page.evaluate(() => window.localStorage.getItem("agenter:studio:auth-session"));
  if (storedToken !== null) {
    return;
  }
  ...
  await activateUntil(signChallengeButton, async () => {
    return (await page.evaluate(() => window.localStorage.getItem("agenter:studio:auth-session"))) !== null;
  });
};
