...
- Shell-assistant seed flow only seeds missing `AGENTER.mdx` prompt guidance. The seeded `AGENTER.mdx` must stay a thin MDX composition wrapper that first inherits builtin runtime guidance through `global:builtin/$LANG/AGENTER.mdx` and then includes package-owned `app:shell/ShellAssistant.mdx`, so user-edited Avatar prompts remain the truth while app upgrades can update Shell-owned guidance. The package prompt teaches NoteSystem recording through the projected JSON-first `note` CLI, the `note` skill, notebook `shell-assistant-book`, mandatory `mime`, and the `content`/`contentFile` input law; it must not create or list `user-model.md`, `pairing-playbook.md`, `terminal-habits.md`, `self-evolution-log.md`, or `hosting-objective.md` as the default recording pack.
