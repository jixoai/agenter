...
import type { ShellRoomSurfaceStore } from "../src/surfaces/room-surface";
import type { ShellApprovalRequest, ShellApprovalStore } from "../src/surfaces/top-layer-surface";
...
let activeApp: ShellApp | null = null;
...
const pendingApproval: ShellApprovalRequest = {
...
class RecordingApprovalStore implements ShellApprovalStore {
  approval: ShellApprovalRequest | null = null;
...
class ManualStatusProvider implements ShellStatusProvider {
  #state = {
    runtime: { label: "Idle" },
    actions: ["Help", "Chat"],
  };
...
class RecordingRoomStore implements ShellRoomSurfaceStore {
...
const createAttachedRoom = (store: RecordingAttachedRoomStore): ShellRoomBootstrapResult => {
  ...
  return attached as unknown as ShellRoomBootstrapResult;
};
...
const startApp = async (
  input: {
    ...
    statusProvider?: ShellStatusProvider;
...
describe("Feature: shell app runtime", () => {
  test("Scenario: Given shell starts When rendered Then it mounts one protocol-backed pane and native statusbar", async () => {
  ...
  test("Scenario: Given a terminal source rejects input When typing text Then shell does not follow cursor", async () => {
    const { recordings, setup } = await startApp();
  ...
  test("Scenario: Given a focused shell pane When Ctrl+B then N is pressed Then shell splits and focuses the new pane", async () => {
    const { setup, recordings } = await startApp();
  ...
  test("Scenario: Given a app-bound terminal policy without split capability When Ctrl+B then N is pressed Then shell does not create a local terminal substitute", async () => {
    setup = await createTestRenderer({ width: 64, height: 18, useMouse: true, kittyKeyboard: true });
    const recordings: RecordingSource[] = [];
    const notices: string[] = [];
    const createAppBoundSource = (id: string): PaneSource => {
    ...
    const app = new ShellApp({
      renderer: setup.renderer as TestRenderer,
      terminalSourcePolicy: {
        createInitialSource: (input) => createAppBoundSource(input.id),
        describeSplitUnavailable: () => "App-bound terminal split is not implemented",
      },
    ...
    expect(notices).toEqual(["App-bound terminal split is not implemented"]);
    const frame = setup.captureCharFrame();
    expect(frame).toContain("App-bound terminal split is not implemented");
  ...
  test("Scenario: Given a focused terminal pane When bare Shift+Left is pressed Then shell forwards it to the terminal instead of moving focus", async () => {
    const { setup, recordings } = await startApp();
    setup.mockInput.pressKey("b", { ctrl: true });
    setup.mockInput.pressKey("n");
    await setup.renderOnce();
  ...
  test("Scenario: Given a terminal source title changes When the pane refreshes Then shell projects the title in pane chrome", async () => {
    const { setup, recordings } = await startApp();
  ...
  test("Scenario: Given live status provider changes When it emits Then shell refreshes statusbar and terminal title chrome", async () => {
  ...
  test("Scenario: Given a terminal pane title bar When clicking its x affordance Then shell opens close confirmation in top layer", async () => {
  ...
  test("Scenario: Given a app-bound terminal pane When Run in background is clicked Then shell only closes the UI attachment", async () => {
    setup = await createTestRenderer({ width: 64, height: 18, useMouse: true, kittyKeyboard: true });
    const recordings: RecordingSource[] = [];
    const createAppBoundSource = (id: string): PaneSource => {
      const recording = createRecordingProtocolSource(id);
  ...
  test("Scenario: Given a app-bound terminal pane When Terminate terminal is clicked Then shell terminates the attached terminal before exiting", async () => {
    setup = await createTestRenderer({ width: 64, height: 18, useMouse: true, kittyKeyboard: true });
    const recordings: RecordingSource[] = [];
    const createAppBoundSource = (id: string): PaneSource => {
      const recording = createRecordingProtocolSource(id);
  ...
  test("Scenario: Given a terminal cursor is viewport-local When shell renders Then it commits the cursor on the first content row", async () => {
    const { setup, recordings } = await startApp();
  ...
  test("Scenario: Given Ctrl+B then H When rendered Then Help opens as a direct OpenTUI surface", async () => {
    ...
    expect(setup.captureCharFrame()).toContain("shell help");
    expect(recordings[0].inputChunks).toEqual([]);
  });
  ...
  test("Scenario: Given Ctrl+B then C When rendered Then Chat opens as a direct OpenTUI surface", async () => {
    ...
    expect(setup.captureCharFrame()).toContain("shell chat");
    expect(recordings[0].inputChunks).toEqual([]);
  });

  test("Scenario: Given Chat title actions When clicking left and float Then shell moves Chat through host layout modes", async () => {
    const { setup } = await startApp();
  ...
  test("Scenario: Given a focused shell pane When Option arrows are pressed Then shell moves by terminal word boundary", async () => {
    const { setup, recordings } = await startApp();
  ...
  test("Scenario: Given a focused shell pane When Shift arrows are pressed Then shell extends backend selection by cell", async () => {
    const { setup, recordings } = await startApp();
  ...
  test("Scenario: Given a focused shell pane When Shift Option arrows are pressed Then shell extends backend selection by word", async () => {
    const { setup, recordings } = await startApp();
  ...
  test("Scenario: Given the mixed statusbar When Help and Chat are clicked Then shell toggles the corresponding panes", async () => {
    const { setup } = await startApp();
  ...
  test("Scenario: Given app runtime keeps layout-derived active actions local When layout attention sync is disabled Then Help and Chat still underline correctly", async () => {
  ...
  test("Scenario: Given app surfaces are open When statusbar renders Then active actions are underlined", async () => {
    const { setup } = await startApp();
  ...
  test("Scenario: Given app surfaces are open When statusbar renders Then bracket borders stay plain while only inner content is active", async () => {
    const { setup } = await startApp();
  ...
  for (const shortcut of [
    ...
    test(`Scenario: Given a focused terminal pane When ${shortcut.label} is pressed Then shell copies backend terminal selection without forwarding input`, async () => {
      const { setup, recordings } = await startApp();
  ...
  test("Scenario: Given a local terminal selection string When Super+C is pressed Then shell copies it through OSC52", async () => {
    const { setup, recordings } = await startApp();
    const copied: string[] = [];
    recordings[0].copyResult = "selected terminal text";
    setup.renderer.copyToClipboardOSC52 = ((text: string, target?: ShellClipboardTarget) => {
      copied.push(`${target ?? SHELL_CLIPBOARD_TARGETS.clipboard}:${text}`);
      return true;
    ...
    expect(recordings[0].copyRequests).toEqual(["terminal:clipboard"]);
    expect(copied).toEqual([`${SHELL_CLIPBOARD_TARGETS.clipboard}:selected terminal text`]);
  });

  test("Scenario: Given a live terminal copies asynchronously When selection text arrives Then shell writes OSC52 clipboard data", async () => {
    const { setup, recordings } = await startApp();
    const copied: string[] = [];
    setup.renderer.copyToClipboardOSC52 = ((text: string, target?: ShellClipboardTarget) => {
      copied.push(`${target ?? SHELL_CLIPBOARD_TARGETS.clipboard}:${text}`);
    ...
    expect(recordings[0].copyRequests).toEqual(["terminal:clipboard"]);
    expect(copied).toContain(`${SHELL_CLIPBOARD_TARGETS.clipboard}:async selected text`);
  });
  ...
  test("Scenario: Given a shell pane drag selection When the backend selection ends Then shell mirrors it to the primary selection", async () => {
    const { setup, recordings } = await startApp();
    const copied: string[] = [];
    ...
    expect(copied).toContain(`${SHELL_CLIPBOARD_TARGETS.primary}:selected shell text`);
  });

  test("Scenario: Given a mouse-aware TUI handles drag release When Shell receives pty-mouse pointer effects Then it does not mirror primary selection", async () => {
    const { setup, recordings } = await startApp();
    const copied: string[] = [];
    ...
    expect(copied).not.toContain(`${SHELL_CLIPBOARD_TARGETS.primary}:selected shell text`);
  });

  test("Scenario: Given a live shell pane drag selection When async selected text arrives Then shell writes it to primary", async () => {
    const { setup, recordings } = await startApp();
    const copied: string[] = [];
    setup.renderer.copyToClipboardOSC52 = ((text: string, target?: ShellClipboardTarget) => {
      copied.push(`${target ?? SHELL_CLIPBOARD_TARGETS.clipboard}:${text}`);
    ...
    expect(recordings[0].copyRequests).toContain("terminal:primary");
    expect(copied).toContain(`${SHELL_CLIPBOARD_TARGETS.primary}:async primary text`);
  });

  test("Scenario: Given primary selection completion When shell requests primary copy Then it uses one host capability path only", async () => {
    const { setup, recordings } = await startApp();
    const copied: string[] = [];
    setup.renderer.copyToClipboardOSC52 = ((text: string, target?: ShellClipboardTarget) => {
      copied.push(`${target ?? SHELL_CLIPBOARD_TARGETS.clipboard}:${text}`);
    ...
    expect(copied).toEqual([`${SHELL_CLIPBOARD_TARGETS.primary}:selected shell text`]);
  });
  ...
  test("Scenario: Given a renderer pane selection When Super+C is pressed Then shell copies the selected text through OSC52", async () => {
    const { setup } = await startApp();
    const copied: string[] = [];
    ...
    expect(copied).toContain(`${SHELL_CLIPBOARD_TARGETS.primary}:${selectedText}`);
    expect(copied).toContain(`${SHELL_CLIPBOARD_TARGETS.clipboard}:${selectedText}`);
    setup.renderer.copyToClipboardOSC52 = originalCopy;
  });

  test("Scenario: Given a renderer pane text selection When the drag finishes Then shell mirrors it to the primary selection", async () => {
    const { setup } = await startApp();
    const copied: string[] = [];
    const originalCopy = setup.renderer.copyToClipboardOSC52.bind(setup.renderer);
    setup.renderer.copyToClipboardOSC52 = ((text: string, target?: ShellClipboardTarget) => {
    ...
    expect(copied.some((entry) => entry.startsWith(`${SHELL_CLIPBOARD_TARGETS.primary}:`))).toBe(true);
    setup.renderer.copyToClipboardOSC52 = originalCopy;
  });

  test("Scenario: Given renderer pane text is selected When middle-clicking Then shell does not clear the visible selection", async () => {
    const { setup } = await startApp();
  ...
  test("Scenario: Given Room-backed Chat is toggled When the user sends a draft Then MessageSystem store receives the message", async () => {
    ...
    const frame = setup.captureCharFrame();
    expect(frame).toContain("Room: Shell Room");
  ...
  test("Scenario: Given shell is destroyed When a pane exits later Then cleanup remains idempotent", async () => {
