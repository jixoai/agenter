...
import type { ShellStore } from "../src";
...
export class FakeShellStore implements ShellStore {
  ...
  async publishGlobalTerminalComposedSurface(): Promise<unknown> {
    throw new Error("composed TerminalSystem surface must not be used by active shell");
  }
  ...
  async archiveGlobalRoom(input: {
    ...
    const archived = {
      ...
      archivedAt: Date.now(),
      archivedBy: input.archivedBy ?? "shell",
