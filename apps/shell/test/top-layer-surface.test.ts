...
import {
  SHELL_APPROVAL_LEASE_MS,
  ShellTopLayerSurface,
  type ShellApprovalRequest,
  type ShellApprovalStore,
...
class RecordingApprovalStore implements ShellApprovalStore {
  approval: ShellApprovalRequest | null = pendingApproval;
...
let activeSurface: ShellTopLayerSurface | null = null;
...
const startSurface = async (store = new RecordingApprovalStore()) => {
  ...
  activeSurface = new ShellTopLayerSurface({
...
describe("Feature: shell OpenTUI top layer", () => {
  ...
  test("Scenario: Given a terminal approval is pending When pressing A Then the top layer approves it", async () => {
    ...
    expect(store.approved).toEqual([
      { terminalId: "terminal-1", requestId: "approval-1", durationMs: SHELL_APPROVAL_LEASE_MS },
    ]);
