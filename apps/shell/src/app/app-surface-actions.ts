...
export type ShellAppSurfaceKind = "help" | "chat";

export interface ToggleShellAppSurfaceInput {
  readonly kind: ShellAppSurfaceKind;
...
export const createShellAppSurface = (input: ToggleShellAppSurfaceInput): OpenTuiRenderableSurface => {
...
export const toggleShellAppSurface = (input: ToggleShellAppSurfaceInput): boolean => {
  if (!input.layout || !input.mux) {
    throw new Error("toggleShellAppSurface requires layout and mux");
  }
  ...
  const surface = createShellAppSurface(input);
