...
import {
  OPENCOMPOSE_APP_DYNAMIC_QUIET_MS,
...
export interface ShellLiveTerminalProtocolSourceInput {
...
export class ShellLiveTerminalProtocolSource implements TerminalProtocolPaneSource {
  ...
  constructor(input: ShellLiveTerminalProtocolSourceInput) {
    this.id = input.id;
    ...
    this.#mirror = createOpenComposeLiveTerminalMirror({
      ...
      pacing: input.pacing ?? {
        mode: "dynamic",
        dynamicQuietMs: OPENCOMPOSE_APP_DYNAMIC_QUIET_MS,
      },
