...
import { SHELL_CLIPBOARD_TARGETS } from "./host-copy";
...
import { ShellPaneResizeController } from "./pane-resize-controller";
import {
...
export interface ShellMuxRenderableInput {
...
export class ShellMuxRenderable {
  ...
  #resolveSource(node: ChildLayoutNode): PaneSource {
    ...
    if (!source) {
      throw new Error(`missing shell pane source: ${sourceId}`);
    }
    return source;
  }
        ...
        protocol.subscribeSelectionText?.((event) => {
          if (event.text.length > 0) {
            this.#renderer.copyToClipboardOSC52(
              event.text,
              event.target === "primary" ? SHELL_CLIPBOARD_TARGETS.primary : SHELL_CLIPBOARD_TARGETS.clipboard,
            );
          }
        }),
