...
// shell keeps renderer semantic selection as an explicit local behavior.
...
interface ShellRendererClickTracker {
...
export interface ShellRendererSelectionTarget {
...
export interface ShellRendererSelectionBehavior {
  handleMouseDown(event: MouseEvent): boolean;
  handleMouseUp(event: MouseEvent): boolean;
}

export interface ShellRendererSelectionBehaviorInput {
  readonly renderer: CliRenderer;
  readonly resolveTargets: () => readonly ShellRendererSelectionTarget[];
  readonly semanticClickMaxDistanceCells?: number;
}

type ShellSemanticSelectionTarget = ShellRendererSelectionTarget & {
  readonly renderable: TextRenderable | EditBufferRenderable;
};

interface ShellRendererSelectionPoint {
  readonly row: number;
  readonly col: number;
...
export class ShellRendererSelectionController implements ShellRendererSelectionBehavior {
  #lastClick: ShellRendererClickTracker | null = null;
  readonly #semanticClickMaxDistanceCells: number;

  constructor(readonly input: ShellRendererSelectionBehaviorInput) {
  ...
  #resolveTarget(renderable: Renderable | null): ShellSemanticSelectionTarget | null {
    if (!renderable) {
      return null;
    }
    const targets = this.input.resolveTargets().filter(
      (target): target is ShellSemanticSelectionTarget =>
  ...
  #resolveLocalPoint(
    target: ShellSemanticSelectionTarget,
    event: Pick<MouseEvent, "x" | "y">,
  ): ShellRendererSelectionPoint | null {
  ...
  #selectWord(target: ShellSemanticSelectionTarget, point: ShellRendererSelectionPoint): boolean {
  ...
  #selectLine(target: ShellSemanticSelectionTarget, point: ShellRendererSelectionPoint): boolean {
    if (target.renderable instanceof TextRenderable) {
...
export const createShellRendererSelectionBehavior = (
  input: ShellRendererSelectionBehaviorInput,
): ShellRendererSelectionBehavior => new ShellRendererSelectionController(input);
