...
describe("Feature: cli-shell app settings", () => {
  ...
  test("Scenario: Given keybindings file is missing or invalid When cli-shell parses keybindings Then it falls back to app defaults", () => {
  ...
  test("Scenario: Given cli-shell app config files are missing When read from disk Then app config falls back to durable defaults", async () => {
  ...
  test("Scenario: Given cli-shell persists Chat layout and keybindings When read back from disk Then the saved app files become the local truth", async () => {
    const dir = await mkdtemp(join(tmpdir(), "cli-shell-settings-"));
    tempDirs.push(dir);
