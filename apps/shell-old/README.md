# agenter-app-shell-old

This directory preserves the old tmux-backed cli-shell implementation. It is no longer the target of `agenter shell`; the official Shell app now lives in `apps/shell`.
...
- `--session=<name>` selects the cli-shell app resource key. For example, `--session=4` maps to tmux session `shell-4`.
- `--tmux=<path>` selects the tmux executable. It defaults to `tmux`.

cli-shell is hosted by tmux, but tmux is only the local host shell. The actual shell target still comes from TerminalSystem through app binding, and MessageRoom still comes from MessageSystem.

cli-shell uses a dedicated tmux socket named `agenter-cli-shell`. Its status bar and key bindings stay inside that app-owned tmux server instead of changing your normal tmux server.
...
cli-shell installs a bottom tmux status bar. The left side identifies the current app session, and the right side keeps the clickable actions visible:

```text
cli-shell  shell-5  @bangeel  ◉ Avatar started; waiting for new Heartbeat...  managed:off                         Help  Chat
```

The default tmux window list is hidden in cli-shell sessions so it does not push the app actions off screen.
...
- The left side also carries a compact Avatar Heartbeat preview so the app chrome still reflects assistant activity.
