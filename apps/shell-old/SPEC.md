...
> 本文件记录 `agenter-app-shell` 作为 extension app 的长期职责。

## 1. 角色

`agenter-app-shell` 是社区/extension 级 terminal app：

- 通过 `agenter shell` 被 descriptor-driven launcher 启动
- 代码位置属于 `apps/cli-shell`，不属于 core packages
- 可使用 tmux 作为本地 terminal host/compositor
- 通过 generic app-extension/client-sdk API 绑定 TerminalSystem、AvatarRuntime、MessageRoom 与 attention
- 不把 TerminalSystem 当作 cli-shell 的产品渲染层、terminal pair 层或 pane topology 存储层
- 不把 tmux 当作 Shell 真相、提示词真相、授权真相或 MessageRoom 真相

## 2. 长期法则

- package 只能通过 `@agenter/client-sdk` 与 `@agenter/app-runtime` 消费平台能力；不得 import core runtime internals。
- optional `@avatar` 与 `--session` grammar 属于 app-local parsing：Avatar identity 与 shell name 必须分离，shell name 不能成为 runtime identity。
...
- `--session=<name>` 只选择 cli-shell app resource key，例如 `--session=4` 对应 `shell-4`。它不得变成 AvatarRuntime identity，也不得被解释成“创建一个干净 Avatar 上下文”。
...
- cli-shell 必须把 tmux 作为 app shell，而不只是 process launcher：默认 attach 进入 primary shell pane，并自动打开/复用右侧 Chat dock pane。Chat cover/popup 只是显式 titlebar 布局请求，不是默认启动布局。
- cli-shell 必须使用 app-owned tmux socket namespace（当前为 `agenter-cli-shell`），不得把 status/key binding 写入用户默认 tmux server。
...
- cli-shell 必须安装底部 tmux status bar：左侧展示 app id、session、Avatar、Avatar Heartbeat 预览、managed state；managed state 必须可点击并切换 room-bound hosting attention。右侧只展示 Help 入口与 Chat 入口；Dock pane fallback、Mouse toggle、Shell focus 与 Refresh 是 keyboard-accessible expert actions，不得重新挤占右侧主入口。
...
- cli-shell 必须提供 Help popup，`Ctrl+b` 后按 `?` 可打开；Mouse 开启后点击 Help 也可打开。Help 必须用非 tmux 用户能理解的方式说明点击主入口、“先按 Ctrl+b 松开，再按下一键”、Mouse、默认右侧 Chat dock、cover popup、Dock pane、copy-mode 与 popup 退出方式。
...
- Chat 是 cli-shell app shell 中的单例 surface：同一个 tmux session + Avatar 下，默认 attach、状态栏 `Chat`、dock fallback、titlebar layout request 必须收敛到同一个 Chat surface。默认 attach 必须打开/复用右侧 dock pane。状态栏 `Chat` 是 toggle：closed 时按保存的默认布局打开；popup 或 pane 已打开时再次触发必须关闭当前 Chat surface、恢复 shell focus/highlight，不得再打开第二个 Room。
...
- tmux cover/popup 是显式 modal/top-layer 布局：popup 打开时 tmux client 的鼠标事件归 popup 模式优先处理，底部 status bar 即使可见也不得被承诺为可靠鼠标控制面。默认右侧 dock pane 是鼠标可点击主路径；cover 模式下应通过 Chat titlebar 或键盘退出/切换。
...
- cli-shell 的默认 tmux attach 必须为了默认 Chat 体验打开/复用右侧 Room pane，但该 pane 仍然只是 presentation-local Chat surface，不是 MessageRoom truth。
- cli-shell 必须安装 app-local tmux key bindings：`Ctrl+b` 后按 `?` 打开 Help popup，按 `c` toggle Chat，按 `C` 打开/聚焦 Chat pane fallback，按 `m` 切换 Mouse，按 `s` 聚焦 shell pane，按 `r` 刷新状态栏。
- cli-shell 不得创建 `terminal-1` / `terminal-2` 作为 active app surface；这些名字只允许出现在 legacy residue audit 或 migration cleanup 语境。
- cli-shell 不得写入 `terminalRuntimeKind=composed`、`composedShellTerminalId` 或发布 `AppTerminalComposedSurfaceState`。
...
- cleanup 必须通过同一个 app-owned tmux socket namespace 清理 extension-owned tmux sessions，并清理 MessageRoom resources、runtime sessions，也可作为 migration helper 清理旧 terminal-1/terminal-2 residue。
