...
const runOptionalTmuxActionStep = async (
  ...
  try {
    ...
    // The app truth is the session option update; refresh is only a best-effort projection nudge.
  }
};
...
export const runCliShellTmuxAction = async (
  ...
  if (action === "managed") {
    await runTmuxActionStep(input, ["display-message", "cli-shell: managed toggle requires app runtime"], executor);
