...
export interface CliShellStore extends AppRuntimeStore {
...
const resolveSelectedAvatar = async (
  ...
  const runtimeClient = createCliShellAppRuntimeClient(store);
...
const resolveCliShellAvatarRuntime = async (input: {
  ...
  runtimeClient: ReturnType<typeof createCliShellAppRuntimeClient>;
  avatar: GlobalAvatarCatalogEntry;
  avatarCreated: boolean;
  session: SessionEntry;
  clearedRuntimeSessionIds: string[];
