...
import { CLI_SHELL_DEFAULT_AVATAR } from "./app";
...
- In cli-shell, Shell truth is the current TerminalSystem terminal bound to the active cli-shell app resource.
- Treat any MessageRoom conversation in cli-shell as being about that currently bound TerminalSystem terminal unless the user explicitly names another target.
- cli-shell may use tmux or other local hosts for presentation, but those pane or popup identities are not the shell truth and must not replace the bound TerminalSystem terminal id.
...
- Current cli-shell binding facts such as terminal id, room id, and hosting context are runtime/session facts. They are operational context, not a second prompt source, and do not replace \`AGENTER.mdx\`.
- When the user asks you to run, type, press keys, inspect output, interrupt, continue, or otherwise operate the shell, the target is the current bound TerminalSystem terminal for this cli-shell session.
- Do not run an equivalent command in \`root_bash\` or \`workspace_bash\` and present it as if it happened in the user's visible tmux shell.
- Use \`workspace_bash\` only for explicit one-shot workspace inspection or file work outside the current cli-shell terminal interaction.
- Handling multiple cli-shell sessions is allowed. Keep their identities explicit, prefer the room-bound cli-shell app binding for room conversation, and use the cli-shell cleanup command when stale cli-shell runtime resources need to be removed.
