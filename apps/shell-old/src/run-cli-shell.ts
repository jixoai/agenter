...
const writeRoomSummary = (attached: CliShellRoomBootstrapResult): void => {
  ...
  console.log(`source: ${process.env.AGENTER_APP_SOURCE?.trim() || "direct"}`);
...
export const runCliShellWithDependencies = async (
  ...
  const appArgv = argvInput.slice(2);
  if (isCliShellMetadataOnlyArgv(appArgv)) {
    parseCliShellArgs(appArgv);
    return;
  }
  const args = parseCliShellArgs(appArgv);
