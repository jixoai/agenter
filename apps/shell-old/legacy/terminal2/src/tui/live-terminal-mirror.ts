...
export type CliShellLiveTerminalRefreshMode = "fixed" | "dynamic";

export interface CliShellLiveTerminalPacingOptions {
  mode?: CliShellLiveTerminalRefreshMode;
  fixedFps?: number;
  activeFps?: number;
  idleFps?: number;
...
export interface CliShellLiveTerminalTransportSessionInput {
  transportUrl: string;
  terminalId?: string;
...
export type CliShellLiveTerminalTransportSessionFactory = (
  input: CliShellLiveTerminalTransportSessionInput,
...
export const CLI_SHELL_APP_DYNAMIC_QUIET_MS = 120;
...
export const createCliShellLiveTerminalMirror = (input: {
  ...
  const handleServerMessage = (message: TerminalTransportServerMessage): void => {
    if (message.type === "frameDirty") {
      ...
      // Dirty is a coarse server hint. Fixed pacing remains the default app law:
