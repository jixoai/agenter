...
import { buildCliShellComposedSurface, toAppTerminalComposedSurface } from "../tui/composed-surface";
import {
...
import {
  CLI_SHELL_APP_DYNAMIC_QUIET_MS,
  ...
  const bumpLiveRevision = (trigger = "manual"): void => {
    if (liveTerminalRevisionTimer) {
      perfTracer.record({
        ...
        detail: { host: "web-app-host", trigger, inFlight: true },
      });
      return;
    }
    perfTracer.record({
      kind: "render-revision-scheduled",
      detail: { delayMs: LIVE_TERMINAL_RENDER_INTERVAL_MS, host: "web-app-host", trigger },
    });
    liveTerminalRevisionTimer = setTimeout(() => {
      liveTerminalRevisionTimer = null;
      perfTracer.record({ kind: "render-revision-committed", detail: { host: "web-app-host", trigger } });
      host.renderNow();
  ...
  const syncMirror = (inputMirror: {
    ...
    const mirror = createCliShellLiveTerminalMirror({
      ...
      pacing: {
        ...
        dynamicQuietMs: CLI_SHELL_APP_DYNAMIC_QUIET_MS,
      },
      trace: perfTracer,
  ...
  const publishSurface = (context: {
    ...
    void input.store
      .publishGlobalTerminalComposedSurface({
        terminalId: context.model.terminalId,
        surface: toAppTerminalComposedSurface(surface),
      })
      .then(() => {
