...
import { toAppTerminalComposedSurface } from "../src/tui/composed-surface";
import { startCliShellWebHost } from "../src/web";
import { startCliShellWebProductHost } from "../src/web/web-app-host";
...
describe("Feature: cli-shell web host", () => {
  test("Scenario: Given an attached backend terminal When starting cli-shell web host Then it serves terminal-2 app host assets and browser facts for the final app truth", async () => {
    ...
    expect(pageHtml).toContain('"productStateUrl":"/app-state.json"');
    expect(pageHtml).toContain('"productEventUrl":"/app-event"');
    ...
    const productStateResponse = await fetch(new URL("/app-state.json", controller.url));
  ...
  test("Scenario: Given Web app events When dialogue and resize actions are posted Then terminal-2 composed state changes through the app host", async () => {
    const store = createFakeCliShellProductHostStore();
    await store.createGlobalTerminal({
    ...
    const openResponse = await fetch(new URL("/app-event", controller.url), {
      method: "POST",
    ...
    const draftResponse = await fetch(new URL("/app-event", controller.url), {
      method: "POST",
      headers: { "content-type": "application/json" },
    ...
    const resizeResponse = await fetch(new URL("/app-event", controller.url), {
      method: "POST",
      headers: { "content-type": "application/json" },
    ...
    const stateResponse = await fetch(new URL("/app-state.json", controller.url));
  ...
  test("Scenario: Given terminal-2 already carries backend-published composed surface When cli-shell web host starts Then terminal-snapshot endpoint serves that terminal-2 app truth directly", async () => {
    const store = createFakeCliShellProductHostStore();
    await store.createGlobalTerminal({
      terminalId: "shell-2-app:terminal-1",
      backend: "xterm",
      command: ["/bin/bash"],
      cwd: "/repo",
    });
    await store.createGlobalTerminal({
      terminalId: "shell-2-app",
      backend: "xterm",
      command: ["/bin/bash"],
      cwd: "/repo",
      metadata: {
        terminalRuntimeKind: "composed",
        composedShellTerminalId: "shell-2-app:terminal-1",
      },
    });
    await store.setGlobalTerminalConfig({
      terminalId: "shell-2-app",
    ...
    await store.publishGlobalTerminalComposedSurface({
      terminalId: "shell-2-app",
      surface: {
        shellTerminalId: "shell-2-app:terminal-1",
        terminalId: "shell-2-app",
        seq: 1,
        cols: 80,
        rows: 24,
    ...
    const attached = {
      ...
      managed: {
        managed: false,
        hostingActive: false,
        contextId: "ctx-hosting-shell-2-app",
        hostingMatches: [],
      },
    } as unknown as CliShellBootstrapResult;
  ...
  test("Scenario: Given terminal-2 publishes collapsed and dialogue-open app screens When cli-shell web host starts Then web consumes the same terminal-2 snapshots as native host adapters", async () => {
    for (const mode of ["collapsed", "dialogue-open"] as const) {
      ...
      await store.publishGlobalTerminalComposedSurface({
        terminalId: visibleTerminalId,
        surface: toAppTerminalComposedSurface(terminal2Surface),
      });
      const attached = createAttachedFixture({
  ...
  test("Scenario: Given Web app host owns terminal-2 When dialogue submit and shell scroll are dispatched Then events bridge to dialogue backend and shell backend owners", async () => {
