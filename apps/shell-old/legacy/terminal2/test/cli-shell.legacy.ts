...
describe("Feature: cli-shell orchestration", () => {
  test("Scenario: Given bare argv When parsing cli-shell args Then the app defaults to shell-assistant on shell-1", () => {
  ...
  test("Scenario: Given explicit avatar session and backend When parsing cli-shell args Then avatar override shell name normalization and backend truth stay app-local", () => {
    const parsed = parseAttachArgs([
      "@default",
      "--session=prod",
  ...
  test("Scenario: Given room-only startup flags When parsing cli-shell args Then the app selects MessageRoom diagnostics without terminal backend flags", () => {
  ...
  test("Scenario: Given launcher-owned daemon env When parsing cli-shell args Then the app consumes daemon context without inventing a local port authority", () => {
    const parsed = parseAttachArgs([], {
  ...
  test("Scenario: Given cleanup argv When parsing cli-shell args Then cleanup is a app-local management command", () => {
  ...
  test("Scenario: Given an unsupported backend flag When parsing cli-shell args Then the app rejects it explicitly", () => {
    expect(() => parseCliShellArgs(["--backend=ghostty-web"])).toThrow("unsupported terminal backend");
  });

  test("Scenario: Given help or version argv When classifying cli-shell execution Then app metadata requests return before attach side effects", () => {
  ...
  test("Scenario: Given shell-assistant is missing When bootstrapping cli-shell Then it ensures avatar runtime prompt memory terminal and room without mutating explicit avatars", async () => {
    ...
    expect(attached.visibleTerminal.entry.processKind).toBe("app");
  ...
  test("Scenario: Given cli-shell resources When cleanup is planned and confirmed Then only app-bound terminals rooms and shell assistant sessions are removed", async () => {
    const store = new FakeCliShellStore();
  ...
  test("Scenario: Given legacy cli-shell single terminal resources When cleanup is planned Then app metadata still selects them by shell name", async () => {
    const store = new FakeCliShellStore();
    await store.createGlobalTerminal({
  ...
  test("Scenario: Given cli-shell bootstraps terminal-2 When inspecting terminal creation Then terminal-2 is a composed app terminal without a shell child command", async () => {
    const store = new FakeCliShellStore();
    const attached = await bootstrapCliShell({
      store,
      workspacePath: "/repo",
    ...
    expect(visibleTerminal?.processKind).toBe("app");
  ...
  test("Scenario: Given explicit ghostty-native backend When bootstrapping cli-shell Then the terminal binding carries backend launch truth through the generic app contract", async () => {
    const store = new FakeCliShellStore();
    const attached = await bootstrapCliShell({
      store,
      workspacePath: "/repo",
  ...
  test("Scenario: Given cli-shell managed mode is enabled and later disabled When the app toggles hosting Then only attention state changes and terminal authority is untouched", async () => {
    const store = new FakeCliShellStore();
  ...
  test("Scenario: Given the default shell-assistant prompt seed When inspecting the content Then collaboration variance memory roles and auto-dream remain explicit examples instead of built-in modes", () => {
    ...
    expect(prompt).toContain("the visible app world is the current Terminal instance plus its MessageRoom");
    ...
    expect(prompt).toContain("Use terminal system commands as the normal bridge to that app world");
    ...
    expect(prompt).toContain("Internal cli-shell implementation terminals are app plumbing");
