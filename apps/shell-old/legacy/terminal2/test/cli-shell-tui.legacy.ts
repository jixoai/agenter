...
const createRoomAppTestSetup = async (input: {
  ...
  const store =
    ...
    : harness.store;
  const roomBinding: AppEnsureBindingResult<GlobalRoomEntry> = {
...
describe("Feature: cli-shell interactive TUI", () => {
  ...
  test("Scenario: Given CliShellCoreApp debug mode is enabled When the app renders Then the debug bar owns the top row and shell projection starts below it", async () => {
  ...
  test("Scenario: Given CliShellCoreApp debug mode is disabled When the app renders Then shell projection keeps the top row without the debug bar", async () => {
  ...
  test("Scenario: Given terminal-2 is the single app screen When native core renders collapsed and dialogue-open modes Then both modes are published as the same terminal-2 surface consumed by host adapters", async () => {
  ...
  test("Scenario: Given terminal-2 advertises a transport URL When native core attaches to the composed app Then it never opens a terminal-2 pull session", async () => {
    const shellLines = ["$ agenter shell", "shell-1:~/project $"];
  ...
  test("Scenario: Given dialogue backend owns scroll When the wheel targets dialogue Then shell viewport is not changed by the event", async () => {
    ...
    const visibleTerminalEntry: GlobalTerminalEntry = {
      ...createGlobalTerminalEntry("shell-1:terminal-2", ["app"]),
      processKind: "app",
      metadata: {
        terminalRuntimeKind: "composed",
        composedShellTerminalId: "shell-1:terminal-1",
