...
- **BREAKING**: Tighten `cli-shell` tmux action normalization so status-bar ranges such as `user|help` and `help` resolve to the same app action before execution.
...
- Fix status-bar `Help` so mouse clicks and `Ctrl+b ?` both open the same Help popup through normalized app actions.
...
- `cli-shell-app`: Clarify and enforce singleton Chat surface state, status-click action normalization, and tmux layout switching semantics.
...
- `apps/cli-shell/src/tmux-host.ts`, `apps/cli-shell/src/tui/run-cli-shell-room-tui.ts`, related cli-shell tests.
