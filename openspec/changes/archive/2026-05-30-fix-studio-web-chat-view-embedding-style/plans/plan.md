...
| 9 | User | `web-chat-view/example` is no longer just an example; it is upgraded into `app-view`. Avoid `app-shell` because the name conflicts with existing project language. In `app-view` mode it can provide the complete standalone app capability or only a partial embedded capability, like Chrome can run as a full browser or expose Chrome WebView as an in-app browser. | Rename the app-facing boundary to `app-view`, keep Studio embedding iframe-based and URL-driven, and implement a partial room embed mode inside the same app-view app shell instead of making Studio reconstruct chat-view internals. |
...
| `packages/web-chat-view/example/src/routes/+page.svelte`, `packages/web-chat-view/example/README.md` | The current app-mode name is `app-view`, even though the filesystem path still says `example`. | The change should record the app semantics separately from the legacy path name. |
| User Round 3 app-view clarification | The full app surface and the partial embedded surface are two modes of the same `app-view`, analogous to Chrome browser and Chrome WebView. | Studio should load app-view through a app boundary and select the partial room mode with URL facts, not build a second Studio-local chat shell. |
...
| ---------- | ------------------------ | -------------- |
| OpenSpec artifacts before apply | Commit containing `plans/plan.md`, specs, and `tasks.md` before app-code work starts | Existing earlier change commits reviewed; Round 4 updates pending commit |
...
Studio should embed `web-chat-view` without losing the canonical Web Chat app shell. Avatars, message bubbles, read indicators, icons, and composer controls should retain sane chat-app proportions inside Studio, but the durable target is now stricter: Studio must either embed the same Framework7-backed Web Chat island used by the full review shell, or `web-chat-view` must expose an explicitly supported embedded Framework7 island variant. Round 3 stops local CSS patching until the shell/topology mismatch is resolved.

The app-facing name for that shell is `app-view`. `packages/web-chat-view/example` remains a historical filesystem path for now, but semantically it is the Web Chat app-view. App-view has two legal operating shapes: a standalone full app mode and a partial embedded room mode. The relationship is the same as a browser app that can either run as the complete Chrome app or expose a Chrome WebView inside another application.

### Underlying Drive

`@agenter/web-chat-view` is now a shared app atom, but the atom boundary was misread. `WebChatViewHost` is the transcript/composer leaf; the full app atom also includes Framework7 `App/View/Page/messagesContent` topology and app-level styles. A shared app atom cannot depend on host-local Workbench layout to impersonate a Framework7 page.
...
- Current platform laws: Studio owns operator shell and superadmin controls; `web-chat-view` owns the Web Chat app surface; Framework7 owns chat/page/navigation topology.
- Round 1/2 diagnosis was incomplete: it fixed leaf geometry symptoms while leaving the app-shell mismatch intact.
...
Round 3 treats `WebChatViewHost` as the leaf boundary, not the whole app boundary. Studio can still pass:
...
| ---- | ---------------------------- | -------------------------- |
| Change embedding technology to iframe/custom element | This changes the app integration boundary and event/resize/auth contracts. | Resolved as iframe app-view; auth travels through URL facts, size stays CSS-owned, events stay backend-owned. |
...
| Path | Why rejected |
| ---- | ------------ |
| Patch only Studio route CSS | It hides the app-shell mismatch and breaks other hosts. |
