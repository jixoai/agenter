...
The backend SHALL provide an opt-in real-provider scenario where one Avatar first delivers a tiny app, then the runtime is stopped, later restarted, and the same task continues through durable room/workspace/attention facts.
...
- **WHEN** the runtime is stopped, started again, and the user sends one follow-up request in the same room
