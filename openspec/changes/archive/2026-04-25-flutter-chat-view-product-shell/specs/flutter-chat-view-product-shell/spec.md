...
### Requirement: Flutter app shell SHALL separate profiles, chat stage, and detail surfaces
The standalone Flutter Web app SHALL present room chat through a app shell rather than a demo configuration screen. The shell SHALL separate app-level profile selection, the conversation-first chat stage, and detail surfaces for room facts or selected messages.

#### Scenario: Desktop shell presents a multi-surface chat workspace
- **WHEN** the app shell is rendered in a wide viewport
...
- **WHEN** the app shell is rendered in a narrow viewport
...
### Requirement: Flutter app shell SHALL persist reusable connection profiles
...
- **THEN** the app shell restores that profile from local persistence
...
- **THEN** opening that link hydrates the app shell with the same target room configuration

### Requirement: Flutter app shell SHALL treat room connection configuration as a secondary workflow
