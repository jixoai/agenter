...
The repository currently has two different app delivery laws:

1. `agenter shell` is a descriptor-driven app command. Core CLI resolves the controlled descriptor, ensures or reuses daemon/auth authority, injects explicit environment variables, and lets `agenter-app-shell` own app grammar and UI lifecycle.
...
The second law makes the Web operator surface look like a core subsystem even though it is app UI. The requested migration intentionally breaks compatibility: the active operator app becomes `agenter-app-studio`, started by `agenter studio`; the old `web` command is removed; the existing `@agenter/ui-studio` icon composer is renamed to `@agenter/icon-studio` so names are unambiguous.

## Goals / Non-Goals

**Goals:**

- Make `studio` obey the same descriptor-driven app command law as `shell`.
- Move Studio app lifecycle, static serving, dev serving, and user-facing CLI grammar into `agenter-app-studio`.
...
- Rename the old `ui-studio` icon composer to `icon-studio` without mixing it into the operator app.
- Migrate durable docs/specs/tests from WebUI naming to Studio naming where they describe the active app.
...
- Do not merge icon composer features into the operator Studio app.
- Do not add app-specific branches to `app-server`, `client-sdk`, terminal, message, avatar, or attention systems.

## Decisions

### 1. Studio is a app package, not a core web mode

`agenter studio` will be registered in `packages/cli/src/app-command-registry.ts` as descriptor data:
...
Reason rejected: even a thin built-in alias keeps app lifecycle as a privileged core special case and preserves the old architecture under a new name.

### 2. `web` is removed, not deprecated

`web` is removed from the built-in command set. After migration, `agenter web` follows the same unsupported-command path as any unknown app command.

Rationale:

- the user explicitly requested a breaking update
- no alias means scripts fail loudly and migrate to `agenter studio`
- it prevents two public command names from describing the same app

### 3. Studio owns both static and dev serving

`agenter-app-studio` will expose app CLI grammar such as:
...
Static asset helpers move from `packages/cli/src/webui-static-root.ts` to Studio-owned code with Studio naming. CLI packaging no longer copies Studio assets into `packages/cli/assets/webui`; published Studio package artifacts are the app package's concern.
...
- `studio` should name the operator app
...
### 5. App storage keys migrate deliberately

Source code and tests that describe active app identity should move from `webui/*` or `agenter:webui:*` to `studio/*` or `agenter:studio:*`. Existing user-local persisted UI preferences may be reset by this breaking change. No migration shim is required.

Rationale:

- the change is explicitly breaking
- durable app identity should not keep obsolete namespace names
...
Durable specs and package specs should use Studio for the active SvelteKit operator app. Historical references such as `@agenter/webui-bak` may remain only when explicitly describing the archived React reference package.
...
- [Core CLI tests still assume `agenter web`] -> Rewrite those tests as Studio app-launch tests and add an explicit unsupported `web` scenario.
...
1. Write BDD tests that fail for:
   - `studio` descriptor resolution to `agenter-app-studio`
   - `web` removed from built-ins
   - Studio app launch receives daemon env and owns old dev/static flags
   - package identity renames for Studio and icon-studio
2. Move `packages/webui` to `apps/studio` and update package metadata, bin, exports, scripts, README, Svelte/Vite/Storybook/Playwright configs, and active app namespaces.
...
5. Add the `studio` descriptor and implement `agenter-app-studio` app runner.
6. Update durable specs and package specs.
7. Run OpenSpec validation, CLI/app targeted tests, Studio/icon-studio typecheck/build/tests, and merge verification before landing.
