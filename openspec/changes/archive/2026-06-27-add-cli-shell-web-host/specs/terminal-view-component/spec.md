...
The official browser-facing `cli-shell --web` host SHALL reuse `web-terminal-view` as its protocol-1 projection primitive for terminal-2 instead of introducing a second app-local Web terminal implementation.
...
- **AND** the browser host does not need to decode cli-shell protocol 2 just to render the official app surface
...
- **AND** the app does not depend on a canvas-only shell surface for its official Web host mode
