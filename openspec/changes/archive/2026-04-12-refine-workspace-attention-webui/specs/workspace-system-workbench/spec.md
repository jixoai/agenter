...
The `Explorer`, `Rules`, and `Private` workspace modes SHALL reuse one shared `page-content` header that exposes a `View as` avatar switcher plus the current workspace root path. The `View as` control SHALL show avatar identity with both icon/avatar mark and nickname.
...
- **AND** mode changes do not create three unrelated page-header patterns
...
- **AND** the header continues to show the avatar icon/avatar mark plus nickname for the active lens
...
### Workspace Views HelpHint Copy (The OpenSpec Truth)
*   **Explorer Tree Header**: "HelpHint: folders toggle, large directories are virtualized"
*   **Rules Tree Header**: "HelpHint: full catalog loads in one pass and masters inline"
*   **Private Tree Header**: "HelpHint: private assets reuse tree model without badges"
*   **Rules Summary Drawer**: "HelpHint: Rules does not need preview or inspector"
*   **Media Preview Drawer**: "HelpHint: unsupported files fall back to a no-preview notice with metadata only."

### Explorer Tree API & Pagination (Normative)
- The Workspace Explorer SHALL load data per-path via an explicit interface rather than fetching the entire tree globally.
- **Parameters**: Requests SHALL specify the target `path` and an optional `deep` integer (defaulting to 1, which represents direct children only).
- **Pagination Safety**: To prevent UI or memory lockups on massive directories (like `node_modules`), the backend SHALL enforce a maximum return limit of `100` items per directory level per request.
- **Inline Loading State**: When a folder exceeds the 100-item limit, the UI SHALL NOT render a global "load more" footer at the bottom of the tree. Instead, it SHALL render an inline, click-able "Load N remaining items..." virtual node as the last logical child inside that specific folder's visual boundary.

### Explorer Search Mode (Normative)
- **Search as Filter**: Search within the Explorer SHALL behave as a glob/gitignore-style pattern matcher applied recursively over the workspace paths.
- **Component Reuse**: Search results SHALL be rendered using the identical hierarchical Tree Component used for standard browsing, effectively acting as a filter mask. It SHALL hide non-matching branches or flatten empty intermediate folders, rather than abandoning the tree context for a disconnected flat-list UI.

### FileViewer Interface & Sandbox Contract (Normative)
- **Iframe Isolation**: The `FileViewer` component SHALL be the universal viewport for all file previews (Text, HTML, Video, Image, Audio). It SHALL render content within an isolated `<iframe>` pointing to the `/fileviewer` endpoint.
- **Routing & MIME**: The endpoint SHALL accept `?path=<encoded_path>` and an optional `&mime=<mime_type>` parameter to force specific rendering engines internally. The host WebUI is completely agnostic to the rendering implementation.
- **Universal Fullscreen Capability**: The host `FileViewer` component SHALL inject a universal, floating "Fullscreen" action button (typically bottom-right) overlaid on top of the iframe. This ensures a consistent fullscreen experience for all file types without requiring the isolated content to implement its own fullscreen controls.
- **Security**: The `<iframe>` SHALL employ strict `sandbox` attributes to prevent memory leaks, rogue script execution escaping to the host, and cross-origin attacks, ensuring that rendering untrusted `.html` or `.md` files remains safe.

### FileViewer Mode Switching & Safety (Normative)
- **Source-to-Render Toggle**: For text-based files that support rendering (e.g., `.md`, `.svg`, `.html`), the `FileViewer` SHALL provide an internal toolbar toggle (Preview/Source). By default, these files MUST open in "Source" mode (CodeMirror-driven) to prioritize developer visibility.
- **HTML Safety Interlock**: If a user attempts to switch an `.html` file to "Preview" mode, the `FileViewer` SHALL present a modal or overlay "Security Warning". The actual `iframe` rendering SHALL only initiate after an explicit user confirmation ("Open Anyway"). This prevents zero-day cross-site scripting or local file exfiltration through malicious HTML snippets in the workspace.
