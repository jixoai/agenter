...
### Requirement: Runtime terminal publications SHALL support app TUI projections over TerminalSystem truth

Runtime terminal publications SHALL expose enough TerminalSystem truth for local and remote app TUIs to render current terminal state, send input, inspect output, and react to lifecycle or authorization changes without reconstructing terminal truth locally. App TUIs SHALL consume these publications as projections and SHALL NOT become terminal truth owners.

#### Scenario: App TUI hydrates terminal from runtime publication
...
#### Scenario: App TUI input targets the bound terminal
...
When a app binds a terminal to an AvatarRuntime, runtime model context SHALL be able to identify the current bound terminal and room through app context or attention/projection facts. The model SHALL NOT need to infer current app terminal identity from stale global terminal catalog rows.

#### Scenario: Avatar sees current cli-shell binding
- **WHEN** a cli-shell room message asks the Avatar to operate the terminal
- **THEN** model-visible context or discoverable runtime facts identify the current cli-shell app resource key and bound TerminalSystem terminal id
- **AND** the Avatar does not choose another terminal solely because it appears focused or stale in `terminal list`

#### Scenario: Stale terminal residue is not the app binding
- **GIVEN** old cli-shell terminals remain in the global terminal catalog
- **WHEN** the Avatar handles a fresh cli-shell session
- **THEN** runtime context distinguishes historical catalog rows from the current app binding
