...
- [x] 1.1 Rewrite `openspec/specs/cli-shell-app/spec.md` to remove both legacy `terminal-1/terminal-2` app chrome law and tmux-as-Shell truth law.
- [x] 1.2 Update `apps/cli-shell/SPEC.md` so it says cli-shell is a TUI over TerminalSystem, MessageSystem, AvatarRuntime/Core, and AttentionSystem.
- [x] 1.2.1 Rewrite `openspec/specs/terminal-screen-projection-law/spec.md` to remove cli-shell-specific `terminal-2` app truth assumptions.
...
- [x] 1.4 Rewrite or supersede `refine-cli-shell-tmux-app-shell` so tmux/OpenTUI are local presentation hosts, not durable Shell truth.
...
- [x] 1.6.2 Update entry documentation such as `apps/cli-shell/README.md` so it no longer implies TerminalSystem is absent from active cli-shell shell targeting.
...
- [x] 1.6.4 Add superseded notes to larger historical delta-spec sets such as `separate-cli-shell-app-from-terminal-view-components/specs/*` so direct spec-file entry cannot bypass the new boundary warning.
...
- [x] 1.6.6 Rewrite durable `openspec/specs/app-command-launcher/spec.md` path facts so cli-shell resolves from `apps/cli-shell` rather than stale `packages/cli-shell`.

## 2. App SDK Contract

- [x] 2.1 Define generic app binding output that includes app key, bound TerminalSystem terminal id, bound MessageSystem room id, AvatarRuntime identity, and attention context ids.
...
- [ ] 2.4 Add or revise app context delivery for selected Avatars without overwriting prompt files.
- [x] 2.5 Define the minimal generic carrier for current app binding facts without inventing a cli-shell-only prompt path.
...
- [x] 3.1 Rework cli-shell bootstrap to bind current TerminalSystem terminal and MessageSystem room through generic app binding.
- [ ] 3.2 Ensure `--avatar`, `--create-avatar`, and `--clear-avatar` keep their current user-facing semantics without "test Avatar" coupling.
- [ ] 3.3 Ensure explicit selected Avatars receive current app binding context.
- [x] 3.3.1 Rewrite `apps/cli-shell/src/shell-assistant-seeds.ts` so MessageRoom dialogue defaults to the current bound TerminalSystem terminal instead of tmux session truth.
...
- [ ] 5.2 Add BDD tests proving Avatar-visible context contains current cli-shell app binding.
...
- [ ] 6.3 Run targeted package tests for client-sdk, app-runtime, terminal-system integrations, and cli-shell.
