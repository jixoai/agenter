...
The `Heartbeat` tab SHALL render one continuous runtime surface backed by durable Heartbeat inspection truth. It SHALL show legacy ingress rows, request-side auxiliary rows, AI-visible request/response rows, and compact boundaries in chronological order without rebuilding the primary story from mixed chat rows, request-aux cards, and model-call cards.
...
- **THEN** the stage renders those rows from the durable Heartbeat inspection stream in chronological order
...
#### Scenario: Heartbeat opens after only room ingress was durably recorded

- **WHEN** a room/user ingress message has already been persisted for the runtime but no later assistant response row exists yet
- **THEN** opening `/avatars/runtime/{sessionId}/heartbeat` still shows that ingress row
- **AND** the operator does not see `No Heartbeat rows yet` while durable ingress facts already exist
