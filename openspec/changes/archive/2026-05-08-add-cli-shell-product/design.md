...
Agenter already has strong backend truths for global terminals, global rooms, AvatarRuntime identity, and superadmin WebUI operation. The new `agenter shell` app should reuse those truths rather than create a second local shell authority.

This change is not primarily a one-off cli-shell implementation. Roughly half of the change is to define the programmable app-extension capability that lets ordinary-user products attach to Agenter without polluting core modules. `@agenter/cli-shell` is the first verification extension for that capability. Core packages may know a app descriptor and capability envelope, but they must not import cli-shell implementation, branch on cli-shell UI semantics, or store cli-shell-only state in core runtime tables.
...
In this grammar, omitting `@avatar` summons the dedicated terminal assistant Avatar `@shell-assistant` into the app room. An explicit mention such as `@default` remains a supported override. `--session` is not an Agenter runtime session. It is only the cli-shell app's terminal naming input: `1` maps to the terminal name/id `shell-1`.
...
- Stable agent law: the summoned assistant defaults to the dedicated Avatar `@shell-assistant`. Cli-shell should ensure or create that Avatar through generic Avatar/app-extension APIs when missing. Explicit command mentions such as `@default` override the default and remain supported. This is cli-shell extension grammar and orchestration, not core launcher behavior.
- Stable prompt/memory law: auto-created `@shell-assistant` has an `AGENTER.mdx` prompt source that describes a flexible pair-programming relationship and the assistant's long-lived self-evolution. Self-evolution is orthogonal to managed mode: it can happen during normal conversation, user correction, post-work review, or a app/assistant-composed attention-cli loop such as a user-defined nightly reflection. `auto-dream` is only an example name for such a loop, not a built-in core feature or fixed score key. That prompt should strongly reference a dedicated memory pack so context compaction does not erase the assistant's accumulated fit or the current hosting objective, but the underlying prompt and memory files remain openly editable user assets rather than locked app-owned state.
- Stable hosting law: turning managed mode on commits a app-scoped AttentionItem with the literal fixed score key `scores: {"hosting": 1000}`. This score is the scheduling obligation. The Avatar decides, from prompt, memory, user intent, and current evidence, whether to keep watching indefinitely or settle the obligation by committing `hosting: 0`. Turning managed mode off from cli-shell always commits `hosting: 0`.
...
- Stable governance law: prompt tuning must not overfit a narrow evaluation fixture. If implementation exposes contradictions between the ideal design and real behavior, record them under `.chat/add-cli-shell-app/` instead of hiding them behind a test-specific prompt patch.

Current app references:

Final v8 design reference, collapsed default:

![Agenter shell app reference v8 toolbar](assets/cli-shell-app-reference-v8-toolbar-grid.png)

Final v8 design reference, dialogue panel open on the right:

![Agenter shell app reference v8 dialogue right](assets/cli-shell-app-reference-v8-dialogue-right-grid.png)

Paired source and terminal-grid auxiliary references:

- `assets/cli-shell-app-reference-v8-toolbar-grid.svg`
- `assets/cli-shell-app-reference-v8-toolbar-grid.txt`
- `assets/cli-shell-app-reference-v8-dialogue-right-grid.svg`
- `assets/cli-shell-app-reference-v8-dialogue-right-grid.txt`

Reference notes:
- The v8 PNGs are the accepted final-review app-effect references for this change.
...
- Define a generic app-extension runtime capability that future ordinary-user products can reuse without adding new core-app coupling.
- Let `agenter shell` automatically launch the external `@agenter/cli-shell` app package.
...
- Initialize `@shell-assistant` with a app-specific `AGENTER.mdx` and dedicated memory files for pair-programming fit, learned user preferences, terminal habits, self-evolution, and current hosting objectives.
...
- Default to superadmin auto-login for ordinary local app use.
...
- Do not replace the superadmin WebUI; cli-shell is a focused ordinary-user app.
...
### 1. Core CLI owns app package resolution, not app behavior

The core `agenter` CLI will keep first-party daemon/auth-service bootstrapping and add a small app launcher. The launcher maps app command `shell` to package `@agenter/cli-shell`, resolves the package bin, and forwards arguments and connection settings.

This mapping must remain descriptor-driven. The core launcher may own a controlled app registry entry, but the registry entry is data: command name, package name, bin/capability hints, and launch policy. It must not become `if (command === "shell")` app logic. Adding another ordinary-user app should add another descriptor and app package, not another hard-coded branch in runtime/core modules.

Alternative considered:
- Put shell directly into `packages/cli/src/run-cli.ts`.
  - Rejected because it would make every future app another core CLI branch and would couple app UX to platform bootstrapping.
...
The current CLI runtime is Bun-based, so the default remote runner can be `bunx @agenter/cli-shell`. The launcher still owns this behind a runner abstraction so tests can assert command construction and future non-Bun runtimes can override it without changing the app command law.

Alternative considered:
- Always use remote package execution.
  - Rejected because it makes monorepo tests depend on published packages and breaks local app iteration.

### 2.1 App process context is explicit env, not ambient rediscovery

The launcher should pass daemon/auth/app context through stable environment variables to avoid each app rediscovering platform state differently:
...
- `AGENTER_APP_COMMAND`
- `AGENTER_APP_PACKAGE`
- `AGENTER_APP_SOURCE` as `workspace`, `installed`, or `remote`

Products may also accept mirrored CLI flags for direct testing, but the env contract is the launcher-owned truth.

### 2.2 App extension runtime is the platform law cli-shell validates
...
1. Launch context: daemon endpoint, auth bridge, app descriptor, source, argv, and inherited stdio.
2. Resource binding: generic `appId + resourceKey` APIs that ensure or look up terminal and room resources through their owning systems.
3. Attention projection and mutation: app-scoped attention contexts/items, minimal attention-cli compatible commit/query/settle operations, and Heartbeat stream projection, without hidden prompt side channels.
4. Assistant initialization: app-driven Avatar ensure, prompt-source ensure, and memory-pack ensure through generic Avatar/workspace resource APIs.
...
Core owns these planes as platform interfaces. Cli-shell owns only its app grammar and presentation: `@avatar` parsing, `@shell-assistant` defaulting, `AGENTER.mdx`/memory pack initialization, `--session` normalization to `shell-<name>`, terminal-grid UI, toolbar semantics, and dialogue placement.

Because cli-shell is an external app process, the implementation must consume these planes through daemon/client-sdk style contracts. Local workspace package resolution may use source packages for testing and development, but it must not become permission to import core runtime internals from `@agenter/cli-shell`. The extension boundary should still be visible when the package is published or removed.

Alternative considered:
- Let cli-shell call a set of internal helper functions that happen to create terminals, rooms, focus, and takeover state.
  - Rejected because that would make cli-shell the hidden second app engine and would force every future app to copy or depend on cli-shell-specific glue.

### 3. App session name maps only to terminal identity

`--session=1` resolves to `shell-1`; `--session=prod` resolves to `shell-prod`. This name is a app terminal identity. It must never feed `session.create.name` as a new AvatarRuntime identity axis.
...
The app should list before creating:
...
- If no room exists, create one with backend-allocated room id and app metadata.

The app must not rely on terminal create being idempotent; current terminal-system rejects duplicate terminal ids.

These operations should be expressed through generic app resource binding APIs. The API input should use app namespace and app resource key, for example `appId=cli-shell`, `resourceKey=shell-1`, and desired resource kinds. The core API may store app metadata, but it must not understand `--session`, `shell-1`, toolbar labels, or right-side dialogue placement. Those are extension-owned semantics.
...
- `self-evolution-log`: distilled reflection outcomes, skill gaps, successful adaptations, and future improvement directions, including assistant/app-composed reflection loops.
...
Alternative considered:
- Make the app use only Avatar credentials.
  - Rejected for v1 because the ordinary local CLI app needs a zero-friction bootstrap and the user explicitly wants default superadmin login.
...
These are not app modes, prompt branches, or scoring labels. They are examples used to force the Avatar to observe the actual user, update `user-model` and `pairing-playbook`, and adapt its autonomy, interruption, explanation, and review cadence from evidence.

### 5.2 Self-evolution uses the programmable attention surface

Because this change upgrades kernel programmability, self-evolution should not be limited to static prompt text. For `add-cli-shell-app`, the app-extension runtime should expose enough minimal attention-cli compatible operations for products, skills, or assistants to:
...
This keeps `auto-dream`-style behavior in the composable layer. A user can teach `shell-assistant` a named loop, and the assistant can implement the current version with attention-cli plus memory/skill usage when the wakeup source already exists. Dedicated `watch` and future `schedule` primitives belong to the follow-up proposal `extend-attention-cli-self-evolution-runtime`, not to the first cli-shell app implementation. Core should only provide generic attention operations and scheduling laws; it should not branch on `auto-dream`, nightly reflection, or any other named self-evolution ritual.
...
- orthogonality: did it preserve app/core isolation, attention/delegation separation, and terminal truth ownership?
...
The first screen is the usable app, not a landing page. The shell-terminal should remain recognizably a normal terminal: ordinary terminal content owns the full screen except for a one-row Agenter dock at the bottom. The default dock is one terminal row, distinguished preferably by background color. A separator line is optional and should be treated as a theme affordance, not a second mandatory content row.
...
Room state is a projection over backend message truth. Cli-shell should provide an Agenter dialogue panel, and for the current app reference that panel opens on the right side. It is not persistent default chrome and not part of the collapsed one-row toolbar.

App status belongs in the bottom dock; it must not become top chrome.

Cli-shell must not show a `SHELLS` or `SESSIONS` rail. `SHELLS` still implies in-app shell switching, which is wrong for this app law. If the user wants `shell-2`, they run another `agenter shell --session=2` shell-terminal.
...
- Use the v4 2-3 row bottom strip without a dialogue panel.
  - Rejected because the default intrusion should be one line, and the app still needs an explicit dialogue panel.
- Use the v5 bottom dialogue panel and status-heavy dock.
  - Rejected because the dock content was too busy and the dialogue panel should be drawn on the right for app review.
...
The keybinding is settings-driven from the start. A default such as `Ctrl+G` may be used by configuration, but the durable invariant is that app keys are explicit and ordinary terminal interaction is not intercepted by accident.
...
### 11. App exit detaches; backend resources remain durable

Cli-shell process exit is a detach from the app view, not deletion of the backend terminal or room. Re-running the same command reconnects to the same terminal and room identity. Internal terminal process exit is different from shell-terminal exit: when the backend terminal process stops, cli-shell should show the stopped state in the bottom layer while preserving the room and terminal record.

### 12. Cli-shell owns a new bottom-only IA instead of reusing dashboard panels

The existing `@agenter/tui` package is a dashboard-style app with status bar, session list, chat panel, tasks panel, and LoopBus panel. Cli-shell must not reuse that IA because it would reintroduce top/side chrome and session navigation.

Implementation should use `@opentui/react` primitives directly for the bottom-only shell app. Shared primitives may be extracted later only when they are neutral terminal-app atoms, such as bottom strip rendering, terminal frame hydration, or keyboard routing helpers. They must not carry `@agenter/tui` session-list semantics into cli-shell.

## Implementation Discipline

Before implementation starts, the change follows these construction rules:

1. Start with platform laws, not TUI: implement the app-command launcher, app-extension runtime, daemon/client-sdk contract, and terminal/room/avatar resource binding before building rich terminal chrome.
...
4. Seed shell-assistant prompt and memory only when missing: `AGENTER.mdx` and memory files remain openly editable user assets, and the app must not run a guard loop that restores templates over user edits.
...
2. App-extension runtime daemon/client-sdk contract.
...
- [Risk] App room lookup by metadata can collide if metadata is too loose. -> Mitigation: require `app=cli-shell` and normalized `shellName`, with room title only as display text.
...
- [Risk] The generated reference image contains illustrative command names not guaranteed for v1. -> Mitigation: treat it as layout/app-effect reference, while specs own the durable behavior.
- [Risk] Future TUI iteration may drift back toward a full dashboard or top app chrome. -> Mitigation: keep the single shell-terminal / single terminal / bottom-only intrusion invariant in the cli-shell-app spec and tests.
- [Risk] App shortcuts may break normal shell muscle memory. -> Mitigation: terminal input owns default mode; app composer requires an explicit focus gesture and `Ctrl+C` continues to route to the backend terminal.
...
- [Risk] Prompt updates can overfit the evaluation fixture. -> Mitigation: keep the rubric aligned to orthogonal design laws and log contradictions or painful trade-offs under `.chat/add-cli-shell-app/` instead of hiding them in test-specific prompt wording.

## Migration Plan

1. Add the app launcher to `@agenter/cli` with local-first package resolution and auto daemon reuse.
2. Add `packages/cli-shell` with a package bin and a minimal shell app runtime.
...
4. Add the `@shell-assistant` prompt/memory initializer: seed missing `AGENTER.mdx`, create missing dedicated memory-role files, link those roles from the prompt, and leave the underlying files openly editable without app-side locking or reconciliation.
5. Build the TUI layout using the v8 PNG references as the accepted app effect, with the paired v8 SVG files for inspection/regeneration and the paired v8 TXT grids as terminal cell-grid contracts.
6. Extend or validate minimal attention-cli compatible `commit/query/settle` operations needed for app/assistant-composed self-evolution loops without adding fixed named behaviors such as `auto-dream`; defer richer `watch/schedule` primitives to `extend-attention-cli-self-evolution-runtime`.
...
10. Create `.chat/add-cli-shell-app/` during implementation and write contradiction/pain-point notes whenever the ideal design and observed behavior diverge.
