...
- **THEN** the core CLI resolves app command `shell` to package `@agenter/cli-shell`
...
### Requirement: App launcher SHALL pass daemon connection context to products
...
- **AND** it receives `AGENTER_APP_PACKAGE=@agenter/cli-shell`
- **AND** it receives `AGENTER_APP_SOURCE` as `workspace`, `installed`, or `remote`

#### Scenario: Launcher cleans up daemon it owns
- **WHEN** the launcher starts a daemon only for the app command
- **THEN** it owns cleanup of that daemon after the app exits
- **AND** it does not stop a daemon that was already running before app launch
