...
| Latest stage summary | latest `AssistantMessage:textPart`, when present. | Optional; never invent a title when no textPart exists. |
...
- Shows `No assistant textPart summary` when absent.
...
- latest assistant textPart preview as optional summary, not an invented title.
