...
- resolve the current host to one explicit `@agenter/cli-*` platform package
...
Each public `@agenter/cli-*` platform package SHALL be a thin distribution atom for exactly one host target. A platform package SHALL include only:
...
- **GIVEN** a maintainer inspects one published `@agenter/cli-*` package
