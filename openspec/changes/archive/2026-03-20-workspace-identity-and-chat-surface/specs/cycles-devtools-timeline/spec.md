...
The WebUI SHALL expose the cycle-oriented Devtools surface as a live timeline that summarizes cycle state, timing, and model/tool activity while the session is running, and that timeline SHALL use compact technical typography and restrained color contrast suitable for long reading sessions.
...
#### Scenario: Timeline remains visually compact
- **WHEN** the cycle timeline renders many technical rows
- **THEN** the timeline uses compact typography and restrained status color usage
- **THEN** the technical panel remains scannable without oversized headings or visually noisy chips
