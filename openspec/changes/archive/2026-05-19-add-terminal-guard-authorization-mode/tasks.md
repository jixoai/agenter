...
- [x] 1.2 Update delta specs for terminal authorization, managed seat authority, runtime terminal tools, WebUI, terminal-view components, cli-shell, Shell Assistant, app-extension runtime, and client runtime store.
...
- [x] 1.4 Update durable `SPEC.md` files after implementation so live permission requests are TerminalInstance-bound state with filterable subscriptions, not a app database.
- [x] 1.5 Update durable `SPEC.md` files after implementation so cli-shell managed/takeover is documented as attention-backed app state, not TerminalSystem authority or app write delegation.
- [x] 1.6 Update durable `SPEC.md` files after implementation so `ProductDelegation` is not documented as the current app-extension runtime authorization contract.
- [x] 1.7 Update durable `SPEC.md` files after implementation so TerminalSystem composed surfaces are app-opaque terminal frames, not cli-shell toolbar/dialogue/managed state.
...
- [x] 2.5 Refactor composed terminal surface contracts so TerminalSystem accepts generic frame data and app-opaque metadata instead of fields such as `managedLabel`, `dialogueDraft`, `unreadLabel`, `heartbeatLabel`, or `托管` defaults.
...
- [x] 2.7 Add short boundary comments near the composed surface type/publisher explaining that TerminalSystem transports app-rendered frames and must not learn app chrome semantics.
...
- [x] 3.5 Remove active `ProductDelegation` schemas, app-server store/routes/kernel methods, client SDK store methods, and app-extension client methods from the current public runtime contract.
- [x] 3.6 Delete or ignore existing local app delegation JSON data created by earlier cli-shell experiments; do not migrate it as durable platform truth.
- [x] 3.7 Add app-extension runtime tests proving products can use resource binding, assistant seed, and attention operations without app delegation APIs.
- [x] 3.8 Add boundary comments near app-extension runtime exports stating that the package provides generic app entry points and does not own terminal write authority.
...
- [x] 5.5 Add terminal-view component tests for scoped requests, custom callback handling, default popover rendering, Approve, Deny, expired status, coalesced updates, and non-cli-shell app embedding.
...
- [x] 6.3 Remove any cli-shell managed/takeover path that creates app write delegation, terminal write lease, permanent writer grant, or TerminalSystem-owned managed metadata.
...
- [x] 6.10 Add short comments in cli-shell managed/takeover code stating that hosting attention is app state and terminal write authority belongs only to TerminalSystem.
...
- [x] 6.13 Add cli-shell cleanup command support so stale cli-shell terminals, MessageRooms, and shell-assistant sessions can be removed through app-local orchestration over generic session, message-system, and TerminalSystem APIs.
...
- [x] 7.7 Run focused app-runtime tests proving delegation cleanup.
