...
Cli-shell SHALL grant the selected Shell Assistant guard terminal authority by default for the shell-truth and visible product terminals. Guard authority allows observation and approval-gated writes. Cli-shell managed/takeover SHALL be product-owned hosting attention state and SHALL NOT grant or imply terminal write authority.
...
- **THEN** terminal-2 remains the visible product-terminal surface that can later publish the shell result
...
- **THEN** cli-shell does not create a product write delegation, terminal write lease, permanent writer grant, or TerminalSystem-owned managed flag
...
- **WHEN** no product delegation record exists
...
#### Scenario: Composed terminal publication is product-rendered frame only
...
- **THEN** TerminalSystem receives only generic composed terminal frame data and product-opaque metadata
...
- **THEN** `shell-terminal-view` renders a default OpenTUI TopLayer approval overlay unless product code handles the request through its callback
...
- **THEN** cli-shell does not create product delegation or local writer state
...
- **THEN** the code includes a short boundary comment that managed/takeover is cli-shell product attention state
...
- **THEN** future product authors can reuse the platform without inheriting cli-shell-specific authority law

### Requirement: Cli-shell SHALL clean stale product resources through generic APIs

Cli-shell SHALL provide a product-local cleanup command for stale cli-shell terminal, MessageRoom, and shell-assistant session resources. Cleanup SHALL use existing session, message-system, and TerminalSystem APIs and SHALL NOT require core to learn cli-shell-specific deletion semantics.

#### Scenario: Cleanup dry-run is non-destructive
- **WHEN** the user runs `agenter shell cleanup`
- **THEN** cli-shell lists product-bound terminal ids, MessageRoom ids, and shell-assistant session ids
...
- **THEN** cli-shell deletes product-bound MessageRooms and shell-assistant sessions before deleting TerminalSystem instances
- **THEN** resources outside cli-shell product metadata are not deleted

#### Scenario: Cleanup can be scoped to one shell name
- **WHEN** the user runs `agenter shell cleanup --session=2 --confirm`
- **THEN** cli-shell targets the product resource key `shell-2`
