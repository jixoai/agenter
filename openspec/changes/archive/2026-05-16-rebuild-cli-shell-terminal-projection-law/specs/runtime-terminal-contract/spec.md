...
- **WHEN** terminal-2 composes shell offscreen frame, terminal-chat frame, and product chrome
- **THEN** runtime terminal publication SHALL expose terminal-2 as the final product screen truth
- **AND** clients SHALL NOT need host-local product composition to observe the accepted cli-shell product surface
...
### Requirement: Runtime terminal publications SHALL preserve product-host equivalence for cli-shell

Runtime terminal publications for cli-shell SHALL preserve enough terminal-2 truth for native and Web hosts to render the same final product surface.

#### Scenario: Native and Web consume the same terminal-2 publication
- **WHEN** native cli-shell and `cli-shell --web` attach to one product session
- **THEN** both hosts SHALL consume terminal-2 publication for the visible product screen
- **AND** neither host SHALL require a different runtime terminal truth to show accepted product state
...
- **THEN** runtime-facing facts SHALL distinguish terminal-1 shell truth, terminal-chat independent OpenTUI dialogue backend, and terminal-2 composed product screen
...
- **AND** terminal-2 product composition SHALL NOT replace terminal-1 as the shell observation source
