...
- [x] 2.10 Preserve a single stable tool invocation identity from streamed `tool_call` updates through final persisted Heartbeat `toolTrace`.
- [x] 2.11 Keep one Heartbeat Tool block per `invocationId` even when the matching `tool_result` is not adjacent in the same durable message.
...
- [x] 3.6 Add regression coverage for stable streamed-to-final tool invocation identity and running parameter hydration.
- [x] 3.7 Add regression coverage for non-adjacent `tool_call` / `tool_result` pairing in Heartbeat rendering.
