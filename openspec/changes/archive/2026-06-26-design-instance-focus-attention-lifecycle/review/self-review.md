...
| Shell-next owns terminal-room binding reaction in app code.                                | `apps/shell-next/src/app/lifecycle-reaction.ts` is app-local and uses public store APIs. `runtime.ts` wires it into attach lifecycle.                                                                                         | Aligned |
| No app runtime reaction host or core shell-next branch.                                    | No core module imports shell-next. App runtime store only exposes `archiveGlobalRoom`; the reaction lives under `apps/shell-next`.                                                                                                | Aligned |
| Unrelated terminal death does not archive the bound shell-next room.                           | Negative BDD scenario added in `apps/shell-next/test/run-shell-next.test.ts`; targeted run passed.                                                                                                                                    | Aligned |
...
1. No visible UI evidence exists because this iteration is backend/app runtime behavior only; evidence is command output and tests.
2. Full package typecheck did not pass because of pre-existing unrelated type drift:
   ...
   - `agenter-app-shell-next typecheck`: existing `senderActorId` / `readActorIds` fixture drift and missing room fields in other tests.
...
- Uncommitted relevant paths:
  ...
  - `apps/shell-next/src/app/runtime.ts`
  - `apps/shell-next/src/app/lifecycle-reaction.ts`
...
- `bun test apps/shell-next/test/run-shell-next.test.ts --test-name-pattern "bound terminal is killed|unrelated terminal B|background-run exits app attach"` -> pass
