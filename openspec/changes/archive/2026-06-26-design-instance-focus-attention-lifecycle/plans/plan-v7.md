...
| Should shell-next direct in-memory instance access mean direct imports or a daemon-side app host? | A local app process cannot safely import app-server's singleton instances without collapsing process boundaries. | Recommend daemon-side host gets real instances and runs shell-next-owned policy; shell-next process consumes typed lifecycle API/projections. |
...
5. App extension runtime owns the generic binding/lifecycle host surface.
6. shell-next owns the concrete app binding and policy: for a shell-next binding, terminal killed archives the bound room.
...
| Lifecycle reaction host location | If the reaction must run after shell-next exits, the host belongs in daemon/app-extension runtime, not local TUI. | Use a durable daemon-side host, but treat the terminal-room binding and archive reaction as shell-next-owned policy. |
...
- Persist or index app-owned bindings so shell-next's terminal -> room relation is inspectable after restart without becoming a universal kernel relation.
- Add a generic daemon-side app lifecycle reaction host that can observe real terminal/room control-plane lifecycle facts and invoke shell-next-declared app policy.
...
Why this is the durable path: it keeps all core systems orthogonal, makes shell-next an ordinary app policy owner, and lets future products bind terminal/room/task/browser resources without copying shell-next-specific glue.
...
Why this is weaker: it only works while the shell-next process is alive, reconstructs causal binding from metadata each time, and risks becoming another app-specific watcher pattern. It can be acceptable as a narrow stepping stone only if the durable binding host law is still written first.
...
Rejected because shell-next is an extension/app process. Direct imports would make process-local implementation shape the platform boundary, and it would not naturally survive daemon restart or remote app hosting. The correct place for true memory instances is inside the daemon/app-server app lifecycle host; the ownership of the terminal-room policy still belongs to shell-next.
...
- Custom exit condition from intent: before implementation, the change must state whether it is creating a new instance URI/ref law or extending existing app metadata binding, and must state both where the shell-next lifecycle reaction runs and that shell-next owns the terminal-room binding policy.
