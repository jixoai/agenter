...
const buildVisualTokens = (chip, density, { latestOpen = false, isLatest = false } = {}) => {
  const tokens = [];
  if (shouldShowPrimaryIcon(chip.kind)) {
...
const estimateChipWidth = (chip, density, options) => {
  ...
  return chipPadding + gapWidth + tokenWidth + densityAllowance;
};

const buildLine = (previousChip, currentChip, inputStart, density = "full") => {
  ...
  return {
    label,
    width: Math.max(
      density === "full" ? 24 : density === "medium" ? 22 : 15,
      label.length * (density === "full" ? 6 : density === "medium" ? 5.2 : 4.2) + 8,
    ),
    title:
...
const compressChipsForDensity = (chips, density) =>
  chips.map((chip) => {
    ...
    if (next.kind === "input") {
      next.metrics = next.metrics.slice(0, 2);
      next.metrics = next.metrics.map((metric, index) => (index === 0 ? metric : { ...metric, label: "" }));
    } else if (next.kind === "combo") {
      next.metrics = next.metrics.slice(0, 2);
      next.metrics = next.metrics.map((metric, index) =>
        index === 0 || metric.label === "" ? metric : { ...metric, label: "" },
      );
    } else {
      next.metrics = next.metrics.slice(0, 1);
    }
...
const estimateTimelineWidth = (record, chips, density, latestOpen) => {
  ...
  for (let index = 0; index < chips.length; index += 1) {
    total += estimateChipWidth(chips[index], density, { latestOpen, isLatest: index === chips.length - 1 });
    if (index < chips.length - 1) {
      total += buildLine(chips[index], chips[index + 1], record.input.startedAt, density).width;
    }
  }
  return total;
};
...
const selectVisibleChips = (record, atomicChips, containerWidth, latestOpen) => {
  ...
  const densities = ["full", "medium", "narrow"];
...
const renderChip = (chip, { latestOpen, isLatest, density }) => {
  const tokens = buildVisualTokens(chip, density, { latestOpen, isLatest });
  const classNames = ["chip", `chip--${chip.kind}`];
  if ((latestOpen && isLatest) || chip.kind === "pending") classNames.push("chip--open");
  return `<span class="${classNames.join(" ")}" title="${escapeHtml(chip.title)}">
    ${tokens.map((token) => renderVisualToken(token)).join("")}
  </span>`;
};
...
const renderTimeline = (record, chips, latestOpen, density) => {
  ...
  for (let index = 0; index < chips.length; index += 1) {
    ...
    fragments.push(renderChip(chip, { latestOpen, isLatest: index === chips.length - 1, density }));
    if (index < chips.length - 1) {
      const line = buildLine(chip, chips[index + 1], record.input.startedAt, density);
      fragments.push(
        `<span class="line" style="--line-width:${line.width}px" title="${escapeHtml(line.title)}"><span class="line-label">${escapeHtml(line.label)}</span></span>`,
      );
    }
  }
...
define(
  ...
  class extends HTMLElement {
    ...
    render() {
      ...
      const timelineBudget = Math.max(220, width - 22);
      ...
      this.shadowRoot.innerHTML = `
        ...
        <style>
          ...
          .status--streaming {
            --status-shine-base: color-mix(in oklab, var(--tone-accent, #2563eb), black 8%);
          ...
          .chip {
            ...
            flex: 0 0 auto;
            line-height: 1;
            border-color: var(--chip-border, var(--tone-border, #d9e1ea));
          }
          ...
          .line {
            position: relative;
            flex: 0 0 var(--line-width, 18px);
            min-inline-size: var(--line-width, 18px);
            block-size: 28px;
          }
          ...
          .line::after {
            ...
            border-top: 2px solid var(--metro-rail, #cbd5e1);
          ...
          :host([density="narrow"]) .line-label {
            font-size: 8.5px;
          }
          ...
          .chip--refusal {
            --chip-bg: color-mix(in oklab, var(--kind-refusal, #dc2626), white 92%);
            --chip-border: color-mix(in oklab, var(--kind-refusal, #dc2626), white 68%);
            --chip-ink: color-mix(in oklab, var(--kind-refusal, #dc2626), black 15%);
          }
      ...
      this.shadowRoot.innerHTML = `
        <style>
          ...
          .before {
            ...
            background: color-mix(in oklab, var(--tone-muted, #667085), transparent 78%);
          }
      ...
      this.shadowRoot.innerHTML = `
        <style>
          ...
          .config[data-state="saving"] .provider-pill {
            background-image: linear-gradient(
              100deg,
              color-mix(in oklab, var(--kind-pending, #b45309), white 82%) 18%,
              color-mix(in oklab, var(--kind-pending, #b45309), white 92%) 46%,
              color-mix(in oklab, var(--kind-pending, #b45309), white 82%) 72%
            );
            background-size: 220% 100%;
            animation:
