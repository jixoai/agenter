...
Current backend validation proves isolated room, attention, and terminal behaviors, but it still does not prove the most basic product promise end to end with a real provider: a user asks an Avatar to build something, the Avatar uses terminal tools to create and launch it, delivers a URL through a room, receives feedback, and improves the result in follow-up turns.
...
- Add follow-up verification that simulates the user opening the delivered URL, sending one round of product feedback, and waiting for the Avatar to update the running app.
