...
| `openspec/changes/add-studio-mcp-system-ui/design.md`                          | Current design frames MCP as Avatar-owned facts projected through Studio, with global/project/lifecycle/snapshot/action surfaces.            | Keep the platform boundary: Studio projects `mcpSystem`; it does not own MCP truth.                       |
...
| `openspec/specs/mcp-system/spec.md`                                            | `mcpSystem` owns global configs, exact-project enablement, lifecycle, snapshots, SQL query, and invocation facts under Avatar-owned storage. | UI must preserve Avatar/global/project separation and not invent shortcut semantics.                      |
...
| `openspec/specs/mcp-system/spec.md`           | MCP global configs, project enablement, lifecycle, snapshots, query, and calls are Avatar-owned facts. | Reuse; UI is a projection atom.                                  |
...
| --------------------------------------------------- | ---------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| `mcp相关的页面`                                     | Studio operator surface for managing and inspecting Avatar-owned MCP facts.  | `/mcp` workbench in `apps/studio`.                                |
...
- Current platform laws: `mcpSystem` owns Avatar-partitioned MCP truth; Studio is a projection surface; client runtime-store owns browser-facing contracts; Studio pages inherit shared workbench shell, toolbar, split-detail, scroll, and low-noise design laws.
...
- Avatar authority: selected Avatar nickname/principal and display label.
...
- MCP system atom: existing `mcpSystem` remains the only owner of durable MCP facts.
...
| Expose provider direct MCP tools          | Violates existing `mcpSystem` platform law; Studio should call the Avatar-owned MCP facade. |
