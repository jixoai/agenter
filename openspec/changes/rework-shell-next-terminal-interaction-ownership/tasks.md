...
- [x] 1.2 Confirm `apps/cli-shell` stays read-only and identify only the behavior references needed from `legacy/terminal2`.
...
- [x] 3.6 Delete the obsolete `apps/shell-next/src/terminal-engine/*` path after the source-owned adapters are in place.
...
- [x] 5.6 Run `bun run --filter 'agenter-app-shell-next' test`.
- [x] 5.7 Run `bun run --filter 'agenter-app-shell-next' typecheck`.
- [x] 5.8 Run `git diff --check`.
- [x] 5.9 Confirm `git diff -- apps/cli-shell` is empty.
- [x] 5.10 Verify `Run in Background` preserves app-bound terminal sources while `Terminate terminal` still disposes them.
