...
The core `agenter` CLI SHALL resolve product command `studio` to package `@agenter/studio` through descriptor data and launch it through the same product command launcher law used by other first-party products.
...
- **AND** Studio serving belongs to `@agenter/studio`
