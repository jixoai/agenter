...
**Reason**: Active operator UI delivery is no longer a core CLI responsibility. The app package `agenter-app-studio` owns static and dev serving after the migration.

**Migration**: Use `agenter studio` to launch the Studio app. Studio-owned specs define static root, nested-route fallback, and runtime env injection.
