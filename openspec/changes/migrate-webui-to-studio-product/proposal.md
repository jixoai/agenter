...
- Add `studio` to the product command descriptor registry so `agenter studio` resolves `@agenter/studio` with the same local-first package law as `agenter shell`. **BREAKING**
- Move Studio serving, static asset resolution, and dev-server startup into the `@agenter/studio` package, using launcher-provided daemon/auth context instead of importing core runtime internals. **BREAKING**
...
- `studio-product`: defines the `@agenter/studio` product package, its CLI surface, daemon context contract, static/dev serving behavior, and its boundary from core runtime modules.
...
- `webui-storybook-static-build`: migrate Storybook/static-build requirements to `@agenter/studio`.
