...
- [x] 0.1 Remove the old ambiguous host term from durable docs, tests, and role-facing implementation seams; use `web-terminal-view` for the Web component, `shell-terminal-view` for the native terminal component, and `cli-shell` for the product without forcing an immediate package rename.
...
- [x] 0.3 Record the durable split between backend terminal truth, component projection, and product composition so later changes do not collapse those layers again.
- [x] 0.4 Link this change explicitly to archived `add-cli-shell-product` as a corrective supplement that reopens architecture-dependent acceptance obligations rather than redefining the product effect from scratch.
...
- [x] 1.3 Return or preserve session actor truth in bootstrap/binding outputs wherever the product later needs delegation attribution, unread projection, managed-mode state, or reconnect behavior.
...
- [x] 1.5 Keep `shell-1` as the durable product session key while deriving two distinct terminal binding resource keys for terminal-1 and terminal-2, so lookup and reconnect cannot collapse the two backend terminal roles back into one resource identity.
...
- [x] 2.6.2 Persist attachment creation order and renewable liveness/lease facts in backend control-plane, and resolve the active geometry authority there instead of inside one product-local Web host helper.
...
- [x] 2.8.1 Introduce a distinct backend-authored composed runtime for terminal-2 final product truth; keep projection runtime semantics narrow as source-terminal passthrough rather than teaching it to invent accepted product chrome.
- [x] 2.8.2 Make control-plane runtime selection explicit for terminal-2 composition so the final product surface can exist without a fake PTY command, an empty-command bypass, or another disguised managed-shell path.
...
- [x] 3.6 Ensure `shell-terminal-view` decodes backend-authored terminal-2 final product truth instead of relying on native-host-local bottom/dialogue chrome that other terminal-2 attachments cannot observe.

## 4. Cli-shell Product

- [x] 4.1 Refactor `cli-shell` to compose one explicit `shell-terminal-view` plus one one-line bottom extension as the shell-first product surface, without requiring a premature standalone package split.
- [x] 4.2 Keep the bottom extension orthogonal to shell ownership while still wiring explicit product actions such as buttons, Button clicks, shortcuts, and transcript entry through native interaction paths with no overlay-only fake interaction state.
- [x] 4.2.1 Ensure final native `cli-shell` product affordances are backed by OpenTUI focusable/clickable primitives or another explicitly named host primitive rather than by transparent overlay hotspot layers or plain-text mouse handlers.
- [x] 4.3 Bind backend terminal geometry to the native shell window minus reserved product rows, and keep other same-terminal attachments projection-only unless geometry authority changes explicitly.
...
- [x] 4.6 Bind visible cursor ownership to explicit native focus ownership: focused `shell-terminal-view` owns the backend terminal cursor, while focused transcript or product input boxes own the visible product cursor.
...
- [x] 4.8 Ensure cli-shell bootstrap, native host, and `--web` host all expose the dual-terminal law explicitly: terminal-1 for shell authority and observation, terminal-2 for visible product projection.
- [x] 4.8.1 Move the accepted one-line bottom extension and any accepted transcript-open product chrome into backend-owned terminal-2 truth instead of keeping them only in native-host-local OpenTUI composition.
...
- [x] 5.1 Run `openspec validate separate-cli-shell-product-from-terminal-view-components --strict` after the final authoring pass and again before apply.
- [x] 5.2 Record the opening architecture evidence in `.chat/separate-cli-shell-product-from-terminal-view-components/opening-architecture.md`: implementation paths and contracts clearly separate backend truth, component projection, and product composition, explicitly identify session actor truth and session focus planes for runtime-owned bindings, and explicitly identify geometry authority versus projection-only layers.
- [x] 5.2.1 Record a code-evidence audit proving whether terminal-2 already owns the complete accepted product surface or whether native host still owns a second host-local product surface; if the latter remains true, keep apply blocked and map it to explicit implementation tasks.
- [x] 5.3 Record the closing product effect in `.chat/separate-cli-shell-product-from-terminal-view-components/closing-product-effect.md`: `cli-shell` presents one shell-first surface with exactly one bottom line in collapsed mode, using the archived `add-cli-shell-product` product references as the visual target.
...
- [x] 5.4.2 In Matrix A, record which modifier truth the owning native terminal program actually delivers for tested shortcut paths, and prove product action semantics do not rely on one mocked `meta` interpretation.
- [x] 5.4.3 If the owning native terminal program blocks one configured shortcut, record that host-blocked fact explicitly and still prove the same product action through native click or another host-lawful interaction path.
...
- [x] 5.5.1 In Matrix B, prove terminal-2 product-surface parity across native and Web attachments: one-line bottom chrome and transcript-open transitions appear from the same backend-owned terminal-2 truth rather than from native-only host overlays.
...
- [x] 5.6.2 In Matrix C, include a cursor-ownership repro proving shell cursor visibility transfers cleanly to transcript or product input focus and back again according to the OpenTUI focused-renderable tree rather than key-history heuristics, requested-focus intent alone, or another local cursor-owner toggle.
...
- [x] 5.8 For matrices A-D, record each result under `.chat/separate-cli-shell-product-from-terminal-view-components/` with `expected`, `actual`, `evidence`, `failure signal`, and `related archived reopen mapping` so later acceptance does not rely on narrative memory.
- [x] 5.9 Record inherited-law evidence in `.chat/separate-cli-shell-product-from-terminal-view-components/reference-inheritance.md`, proving this change still uses the existing `--backend=<name>` path and the existing one-line markdown bottom projection law rather than replacing them.
...
- [x] 5.11 Track the reopened `add-cli-shell-product` obligations corresponding to archived tasks `3.2`, `3.5`, and `3.6` for session-actor-correct grants, session-scoped focus, and repeated attach under principal mismatch.
- [x] 5.12 Track the reopened `add-cli-shell-product` obligation corresponding to archived task `4.7` for real interactive toolbar actions under the repaired projection architecture.
- [x] 5.13 Track the reopened `add-cli-shell-product` obligations corresponding to archived tasks `4.13` and `4.14` for shell-input routing and product-action isolation under the repaired projection architecture.
- [x] 5.14 Track the reopened `add-cli-shell-product` obligation corresponding to archived task `4.15` for geometry-authority-correct resize handling.
- [x] 5.15 Track the reopened `add-cli-shell-product` obligations corresponding to archived tasks `4.17` and `4.22` for continuous renderer-surface fidelity and focused behavior revalidation.
