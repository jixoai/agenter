...
`add-cli-shell-app` already established `agenter-app-shell` as an external app package. `promote-ghostty-native-cli-shell` already established `--backend=<name>` and the one-line markdown bottom-rendering law. The unresolved problem is no longer command grammar. It is that the current shell-first draft still mixes app terminology and component terminology.
...
5. app composition: `cli-shell`

The old ambiguous host term is actively harmful because it hides whether a statement is about a reusable component or about the app. That confusion keeps inviting a second frontend-owned terminal truth, which is the wrong law for a real shell app.

This change therefore plays a narrow but important role in the overall history:

- `add-cli-shell-app` already defined the app target and even shipped accepted reference images
...
- this change exists to repair the missing platform law before `add-cli-shell-app` can honestly be considered complete in practice
...
If those roles are not separated in the change itself, the implementation will keep drifting back toward app-specific component laws or component-specific app hacks.

This is also why the current move is a corrective supplement rather than a redesign from scratch. The app target from `add-cli-shell-app` is still valid. What failed was the system's ability to uphold that target under real acceptance pressure.

## Goals / Non-Goals

**Goals:**

- Replace the old ambiguous host term with precise app/component vocabulary.
- Keep backend terminal truth as the only authoritative source for render, durable change log, and terminal observation.
- Keep runtime-owned app resource binding actor truth aligned to the session runtime identity rather than avatar catalog metadata.
...
- Make app actions and terminal scroll affordances truth-bound: buttons and shortcuts must hit explicit app action contracts, while pointer or wheel scroll must hit backend viewport mutation truth instead of host-local replay.
- Separate app action semantics from native shortcut delivery semantics so `cli-shell` does not confuse "this action exists" with "this host actually delivered the configured modifier chord".
...
- Do not introduce an independent-view exception for same-terminal attachments inside this change; if a future app wants an independent viewport, it must use a separate terminal truth or a separate explicit contract.
- Do not force a package rename or a standalone native projection package in this change; role separation must become explicit in contracts, tests, and app composition first.
...
This change does not reject the architecture direction "backend performs headless terminal rendering, projection hosts render that backend truth back to native/Web surfaces". That direction still counts as a real shell app when all of the following hold:
...
For this change's stronger target law, the same rejection applies to accepted app chrome. If terminal-2 is declared as the final app terminal, then any accepted one-line bottom chrome or transcript-open state that belongs to that final app surface must also live in terminal-2 truth. A host-local toolbar or dialogue layer that Web attachments to terminal-2 cannot observe is still a second app-surface truth.
...
- terminal grants for `cli-shell` app-owned terminals derive actor truth from the session runtime
- room grants for app rooms derive actor truth from the same session runtime
...
When used inside `cli-shell`, `shell-terminal-view` is the default geometry authority for the attached terminal because it is bound to the native shell window after subtracting reserved app rows. It also needs one continuous renderer surface: it must not split one terminal into a rich live strip plus a plain-text historical mirror. For the accepted final law in this change, `shell-terminal-view` decodes terminal-2 as the complete final app-terminal surface instead of adding accepted bottom/dialogue chrome as a second host-owned truth.

For this change, `shell-terminal-view` is primarily a contract and component-role name, not a mandatory standalone package split. The first repaired implementation may keep the native projection module inside `packages/cli-shell` as long as the boundary is explicit enough that app composition and projection behavior are distinguishable in code and reusable in principle.

### 6. `cli-shell`

`cli-shell` is the app. It composes:

- one attached visible app terminal plus one attached shell-truth terminal under the same app session law
- one `shell-terminal-view`
- one one-line bottom extension
- one room / Avatar / LoopBus integration surface

The app law remains shell-first: the shell owns the main surface, while the bottom extension remains orthogonal chrome.

For this repaired architecture, attached terminal truth is literal app truth. A `cli-shell` instance binds its main shell surface to the visible app-terminal resource returned by bootstrap or reconnect, not to a later runtime focus projection such as `focusedTerminalId`. Session focus still matters for observation and generic runtime behavior, but it does not get to silently swap the shell body onto a different terminal truth after attach.

The stronger corrective target in this change is that terminal-2 itself owns the complete backend-authored final app surface. That means the accepted one-line bottom chrome and any accepted transcript-open state belong to terminal-2 truth so that native and Web hosts observe the same app state from the same terminal. Native host composition may still supply container, focus, and pointer primitives, but it does not get to keep a second accepted app-chrome truth outside terminal-2. In this target law, terminal-1 to terminal-2 projection belongs to a protocol-2 composition pipeline, while `shell-terminal-view` is the native decoder/renderer that displays terminal-2 truth in the owning shell host.
...
- `packages/terminal-system/src/projection-terminal-runtime.ts` remained a source-terminal passthrough runtime and did not own accepted app chrome
...
That earlier gap is what justified keeping terminal-2 final app truth as an explicit blocker in tasks and acceptance.

### 6.75 Recommended repair path for terminal-2 final app truth

The repair path adopted in code is:

1. keep terminal-1 as the only shell PTY/buffer/scrollback/cursor/viewport authority
2. introduce an explicit protocol-2 composition stage that consumes terminal-1 snapshot truth plus accepted app chrome state
3. publish the composed result into terminal-2 as backend-owned final app-terminal truth
...
5. keep Web host attached to terminal-2 so it naturally observes the same final app surface
...
- native host still gets to use OpenTUI primitives for focus, pointer ownership, and geometry, but not for a second accepted app-surface truth

The resulting code movement for this repair path is:

- terminal-1 snapshot and shared viewport truth remain where they were
- accepted bottom heartbeat/actions and accepted transcript-open app state move out of native-host-local paint-only state and into a backend composition model
- terminal-2 publication becomes the composed output of that model
- native and Web hosts both decode/render terminal-2 for accepted app-surface state
...
- add a distinct backend-authored composed runtime for terminal-2 final app truth instead of teaching projection passthrough to invent accepted app chrome
...
- keep terminal-1 as the only PTY-owning runtime; terminal-2 composed runtime owns snapshot/status/lifecycle publication for the final app surface without pretending to be a second shell process
...
So the implemented direction is not "push host frames into a managed terminal from the outside". It is "introduce a first-class backend runtime whose ordinary snapshot truth is already the composed terminal-2 app surface".
...
- mutation of `ProjectionTerminalRuntime` into a mixed passthrough-plus-app-composer runtime
...
- It keeps component contracts reusable and app contracts app-specific.

Rejected alternative:

- Keep one ambiguous host shortcut term.
  - Rejected because it collapses app and component boundaries again.
...
### 3. Runtime-owned app bindings use session actor truth

For runtime-owned terminal and room bindings, app-extension runtime derives grant actor ids, focus operations, and later attribution truth from the created or reused session runtime actor identity rather than from global avatar catalog metadata alone.
...
- app bootstrap outputs may preserve session actor truth for later delegation, unread projection, or attribution

### 3.25 One app session key derives two terminal binding identities

`cli-shell` keeps `shell-1` as the durable app session key, but it does not reuse that same terminal binding identity for both backend terminal roles. Instead it derives stable terminal binding resource keys `shell-1:terminal-1` and `shell-1:terminal-2`.

The room binding keeps `shell-1` because the room is the app session conversation surface, not one of the two terminal roles.

Rationale:

- the current app-runtime binding law already keys bindings by `appId + resourceKey + ownerSystem`
- deriving two app-local terminal resource keys preserves the existing platform contract without inventing a cli-shell-only backend abstraction
...
- Extend generic binding metadata immediately with a `terminalRole` dimension.
  - Rejected because the current evidence only proves a cli-shell app need, and derived keys already satisfy the architecture without polluting the platform law.
...
- if the app shell body follows focus projection instead of attach truth, the visible shell can drift onto the wrong backend terminal even though grants, room wiring, and terminal transport are otherwise correct

Rejected alternative:

- Treat session focus projection as sufficient shell-binding truth.
  - Rejected because focus is a runtime projection plane, not the app's attached terminal identity.
...
- reconnect behavior may use focus planes for observation or unread semantics without letting them redefine the app-owned shell surface
...
For the shell-first app:

- `shell-terminal-view` owns terminal-2 final app-surface geometry from the native shell window's full visible size.
- `cli-shell` derives terminal-1 shell-truth geometry from that terminal-2 app-surface geometry by subtracting reserved app rows and any docked transcript columns that belong to the accepted final app surface.
...
When `terminal-2` is the backend-owned final app-terminal surface, the native host may still use explicit host primitives such as OpenTUI `scrollbar`, focusable `Box`, `Input`, or equally explicit native controls to own click, focus, and scroll gestures. Those primitives remain lawful only as control projections. They do not get to become a second visible app truth.

For this change, "control projection" means:

- the primitive may decide which app action or viewport mutation request to send
...
- the primitive does not become the durable source of the resulting visible app state

The visible result remains valid only after backend-owned truth changes and is republished through `terminal-2`.

Rationale:

- native and Web hosts need lawful host-specific interaction primitives, but they still must converge on one observed app surface
- if a local click opens transcript chrome, toggles managed state, or changes placement without that result becoming observable from terminal-2, the native host still owns a second app truth
...
- Require terminal-2 composed surface to carry a full host-agnostic widget tree before any host may expose lawful interaction.
  - Rejected because the current architecture only needs one backend-owned visible surface plus explicit host control ownership. Forcing a second semantic UI tree here would over-model the app and delay the actual single-truth repair.
- Allow host-local interaction primitives to directly own the accepted visible result as long as they call the same controller logic.
  - Rejected because shared controller code does not prevent native and Web hosts from observing different visible app states.

Implementation note:

- accepted app actions such as managed toggle, transcript open, transcript close, placement changes, send, and viewport mutation may originate from host primitives, but the resulting visible state must be observable from the next terminal-2 publication
...
- for the current stack, terminal-2 composed publication may stay screen-first plus explicit app-state fields; this change does not require a separate backend widget tree as long as native-visible app state no longer lives only in host-local composition
...
The visible cursor owner inside `cli-shell` must be determined by explicit native focus ownership. When `shell-terminal-view` owns focus, the visible cursor belongs to the backend terminal projection. When a transcript entry or other app input box owns focus, that app input surface owns the visible cursor instead.

Rationale:

- Cursor visibility is part of terminal truth only while terminal input owns focus.
- The app already needs explicit interaction-mode boundaries, so cursor ownership should follow that same focus truth instead of local heuristics.
- Only one visible cursor owner may exist at a time; dual cursor ownership is a visible contradiction between shell truth and app-input truth.

Rejected alternative:

- Infer cursor ownership from whichever surface last handled a key event without an explicit focus model.
  - Rejected because the shell and app chrome can drift into contradictory cursor states under clicks, shortcuts, and transient overlays.
...
- click or pointer transfer between focusable shell and transcript/app-input boxes must update that same OpenTUI focus truth, so visible cursor ownership follows Box/input focus movement rather than an independent local toggle
...
### 9. The bottom extension remains orthogonal app chrome
...
- `cli-shell` is a shell app with an extension line, not a dashboard that happens to contain a terminal.

### 9.5 Chat panel is a traditional pinned chat room

The explicit Chat panel is the app room surface. It remains optional chrome over the shell-first app and follows traditional chat-room scroll semantics:
...
- clicking that button, pressing the equivalent app action, or sending the user's own message returns the panel to bottom-pinned mode
...
The app-visible meaning of "Avatar started" is that terminal changes from terminal-1 shell truth are already eligible to wake LoopBus and participate in understanding flow.
...
### 11. App action semantics are separate from native shortcut delivery semantics

`cli-shell` owns app action semantics such as transcript open, transcript close, placement changes, send, and managed toggle. The owning native terminal program owns which key chords are actually delivered to the app and which modifier bits those chords arrive with. The repaired architecture therefore must not treat one hard-coded modifier interpretation as equivalent to native truth.

Rationale:

- controller and view-model tests can prove that a app action exists without proving that the real native host ever delivered the configured shortcut
...
- Treat a passing controller shortcut test as sufficient proof that the native host delivered the same shortcut.
  - Rejected because that collapses app semantics and host-delivery truth into one false layer.

Implementation note:

- app shortcut config should name app actions first and allow host-delivery truth to map onto them
- matching logic must not assume `meta` is the only lawful native Command projection
- for the current native `cli-shell` stack, visible app actions should be owned by OpenTUI focusable/clickable primitives such as buttons, inputs, or equally explicit host primitives instead of by transparent overlay hotspots or plain text mouse-handler labels that bypass component ownership
- final native acceptance must record which modifier truth the owning host actually delivered for the tested actions
- if the owning host blocks a configured shortcut entirely, acceptance must record that host fact and still prove the same app action through click or a host-lawful override path instead of silently downgrading the app action requirement
...
- [Native terminal interaction routing is stricter than ordinary Web event handling] -> Mitigation: make shell-input versus app-action routing explicit in component and app contracts instead of relying on ad-hoc local state.
- [Native shortcut tests can pass while real host modifiers differ] -> Mitigation: model app actions separately from host-delivered shortcut profiles and require native evidence for delivered modifier truth.
...
1. App identity is clear
   - `cli-shell` reads and behaves like a real shell app
...
4. Shell-first app chrome is preserved
   - the bottom extension is exactly one rendered line in collapsed mode
   - app actions remain interactive through native pointer/click or shortcut paths
   - visible app affordances are owned by explicit OpenTUI/native primitives rather than by transparent overlay hitboxes or plain text mouse-handler chrome
   - shortcut-based app actions are verified against the actual modifier truth delivered by the owning native host, not only against mocked `meta` key events
   - if a host blocks one configured shortcut, the acceptance record names that host fact and still proves the same app action through another lawful native interaction path
   - shell scroll gestures mutate shared viewport truth rather than a native-local mirror
   - shell-terminal cursor visibility belongs to the focused shell projection, while transcript or app input cursor visibility belongs to the explicitly focused app input box
...
This end state is not the final stop by itself. It is the architecture gate that must pass before the archived `add-cli-shell-app` delivery can be re-accepted.

The required close-out sequence is:

1. finish this corrective architecture change
2. reopen the affected `add-cli-shell-app` obligations in implementation tracking
3. re-check the archived app promises against the repaired architecture
4. rerun app acceptance for the original shell app effect

The minimum reopen mapping is:

- archived `add-cli-shell-app` `3.2`: terminal grant and focus must target the actual summoned runtime actor rather than a catalog identity lookup
...
- archived `add-cli-shell-app` `4.7`: toolbar action buttons must be truly interactive rather than only rendered
- archived `4.13`: terminal-mode input routing must hold under the repaired projection architecture
- archived `4.14`: dialogue and app actions must not leak into backend shell input
- archived `4.15`: shell-terminal resize handling must respect explicit geometry authority
- archived `4.17`: app chrome must still render as one continuous terminal-cell surface rather than split fidelity tiers
...
1. Opening architecture evidence
   - file/path evidence showing backend truth, component projection, and app composition are separate layers
...
2. Native app evidence
   ...
   - one explicit note for the owning native terminal program recording which modifier truth actually reached the app for the tested shortcut paths, for example `meta`, `super`, `option`, or host-blocked
   - if one configured shortcut is host-blocked, one explicit note showing which native click path or host-lawful override path still activated the same app action
...
- one step where bottom chrome and transcript-open app state visible from native host are also visible from the Web attachment because they both come from terminal-2 truth rather than from a native-only host overlay
...
4. Continuous renderer evidence
   ...
   - one cursor-handoff walkthrough proving click or pointer focus transfer between the shell box and transcript/app input box changes the sole visible cursor owner according to the OpenTUI focused-renderable tree rather than according to local requested-focus state alone

5. Startup truth evidence
   - one record showing that "Avatar started" is withheld before LoopBus observation is live
   - one live runtime trace showing that the app-ready signal appears only after terminal changes are eligible to wake observation flow
...
6. Reference inheritance evidence
   - one note linking the closing app effect back to the archived `add-cli-shell-app` reference set
...
- `.chat/separate-cli-shell-app-from-terminal-view-components/opening-architecture.md`
- `.chat/separate-cli-shell-app-from-terminal-view-components/closing-app-effect.md`
- `.chat/separate-cli-shell-app-from-terminal-view-components/matrix-a-native-cli-shell.md`
- `.chat/separate-cli-shell-app-from-terminal-view-components/matrix-b-shared-attachments.md`
- `.chat/separate-cli-shell-app-from-terminal-view-components/matrix-c-renderer-fidelity.md`
- `.chat/separate-cli-shell-app-from-terminal-view-components/matrix-d-loopbus-readiness.md`
- `.chat/separate-cli-shell-app-from-terminal-view-components/reference-inheritance.md`
...
- without those laws in place, implementation could accidentally re-solve the same problem in app-local code and reintroduce a second truth

## Migration Plan

1. Rewrite terminology and contracts so the old ambiguous host term disappears from the change.
2. Repair app-extension runtime binding so session actor truth and session-owned focus planes align for app-owned terminals and rooms.
...
7. Reopen the archived `add-cli-shell-app` delivery obligations that depended on the repaired architecture.
8. Re-run app acceptance for `add-cli-shell-app` against its original app references and promises.
...
- implementation starts from the transport/runtime truth gap before more cli-shell-local TUI repair, because local app fixes cannot manufacture shared viewport truth after the fact
