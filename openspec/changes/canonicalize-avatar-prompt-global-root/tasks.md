...
- [x] 1.2 Update `packages/app-runtime/SPEC.md` so app prompt seed/read identity is global Avatar canonical root only.
...
## 3. App API And SDK Surface

- [x] 3.1 Remove `workspacePath` from `appAvatarPromptSeedInputSchema`.
- [x] 3.2 Update app-extension runtime client/store types so prompt seed cannot be called with `workspacePath`.
...
- [x] 4.2 Add app-extension coverage proving prompt seed writes global root even when launched from a project workspace.
...
- [x] 6.1 Run targeted app-server tests for session config, app-extension runtime, app-kernel prompt editing, and prompt-store.
