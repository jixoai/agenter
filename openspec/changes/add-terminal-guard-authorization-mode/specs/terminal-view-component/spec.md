...
- **THEN** the host cannot resolve the request by mutating component-local or product-local state alone
...
- **THEN** it exposes a product customization callback equivalent to `onRequestPermissions`
...
#### Scenario: Non-cli-shell product can embed the same approval affordance
- **WHEN** a product without cli-shell managed/takeover semantics embeds `web-terminal-view` or `shell-terminal-view`
...
- **THEN** the component does not require cli-shell managed labels, hosting attention, or product delegation state
