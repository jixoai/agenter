...
- [x] 3.3 Expose runtime terminal lifecycle through descriptor-backed CLI commands with explicit `bootstrap` and `stop` verbs plus lifecycle-aware list projections.
- [x] 3.4 Sync built-in terminal skill and lifecycle references so shell guidance teaches status inspection, bootstrap, and stop using the new lifecycle law.
...
- [x] 5.5 Add targeted runtime CLI / built-in skill regression coverage for explicit `terminal bootstrap`, `terminal stop`, lifecycle-aware `terminal list`, and the split instance-name vs PTY-title display law.
