...
- Round: 5
- Status: ListDetail and detail-pagination alignment before backend/page implementation.
...
- `canvases/record-components.js` defines the enhanced Web Components used by the canvases: `x-basic-record-card`, `x-model-run-card`, `x-model-run-chip-gallery`, `x-compact-body`, and `x-config-body`.
...
- `canvases/list-detail-anchor.html` owns list/detail selection, latest/fixed/new-record anchor state, and the current detail grammar targets. Its detail surface is a Declarative Shadow DOM component, because detail reconstruction is a separate surface from the stable list index.
...
- Upgrades Model Run detail from a horizontal row graph into a vertical step inspection surface:
  - left rail: sticky chips for input, thinking, tool, text/error/pending;
...
- Upgrades Config detail into tabs:
  ...
  - YAML diff is the primary visual so multiline system prompt changes remain readable.
