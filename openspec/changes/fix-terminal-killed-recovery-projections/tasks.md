...
- [x] 1.4 Add client-sdk/app-runtime tests proving a killed terminal binding is not treated as a reusable live app shell and a clean live terminal is created when normal app startup needs one.
...
- [x] 4.5 Update app binding recovery so killed terminal bootstrap is only reachable through an explicit recovery/forensic path, not normal app startup.
...
- [x] 6.3 After the client-sdk/Studio/cli-shell pass, perform self-review iteration 2: map each user-facing surface to the intended projection contract and verify there is no app-local lifecycle reconstruction.
...
- [x] 6.5 Treat self-review as non-app workflow only: do not add runtime UI, daemon state, database rows, or public APIs solely to represent the review loop.
...
- [x] 6.7 Repair the self-review method: distinguish platform projections from app-specific selectable projections and require negative dirty-data samples before declaring user-facing alignment.
