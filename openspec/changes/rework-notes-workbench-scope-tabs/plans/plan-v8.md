...
- Round: 8
- Status: User accepted the page structure as close enough and requested final detail refinements: loading count badges must not show `(0)`, Search/Query empty detail states should use shadcn `Empty`, Query SQL input should be CodeMirror with SQL highlighting, and Query results should be structured like Search rows instead of raw JSON.
- Previous plan backup: `plans/plan-v7.md` was created before this Round 8 detail/query polish.
