...
当前 runtime shell 只有 `Heartbeat`、`Attention`、`Settings` 三个稳定 peer tabs。后续 retry-policy 拆分已经把 provider transport retry、runtime retry policy、runtime compact policy 变成正交法则：Settings 负责 durable policy，scheduler/publication 负责发布 policy-resolved recovery state。

因此本 change 不再讨论 provider `maxRetries` 或 durable policy schema。它只补 live recovery surface：操作者遇到 runtime `error` / `backoff` / `blocked` 时，可以在 runtime shell 内看到恢复状态，并通过一次性 `Retry now` 控制请求唤醒 runtime，而不是依赖 stop/start 或修改 Settings。
...
- 保持 durable retry/compact policy 只在 Runtime Settings 中编辑。

**Non-Goals:**
- 不重新定义 provider transport retry、runtime retry policy 或 compact policy。
- 不把 `Retry now` 做成 durable settings mutation。
...
只在既有 runtime shell 上增加 recovery surface，不新增 `Recovery` peer tab。这样可以在不改变主导航法则的前提下，补齐操作者最需要的 recovery 能力。

surface 的职责是：
- 显示 latest runtime error
- 显示 policy-resolved `retryCount`、`blockedReason`、`nextWakeAt`
- 提供 `Retry now`
...
### 3. Runtime Settings 继续拥有 durable recovery policy

durable retry/backoff 与 compact trigger 仍由 Runtime Settings 编辑；recovery surface 只读取 policy-resolved 状态和发出一次性 control intent。

Alternative considered:
- 在 recovery surface 里直接编辑 retry policy
  - Rejected，因为这会把 live control 和 durable law 混回一个 surface。

### 4. Heartbeat quick config 继续保持 execution-scoped
...
### 5. 不把 recovery incident 写进 Heartbeat transcript
...
- [Risk] recovery diagnostics 仍然部分依赖现有 scheduler/publication shape，表达力有限。 → Mitigation: 只展示 policy-resolved durable facts，缺失项明确显示 unavailable，不在前端推断。
...
1. 更新 delta specs，明确 recovery diagnostics、policy ownership 与 manual retry 的 surface/control contract。
2. 为 runtime control plane 增加 manual retry action，并让 runtime publication 暴露 policy-resolved recovery 所需事实。
3. 在现有 runtime shell 中增加 recovery disclosure surface，并保持 Heartbeat quick config execution-scoped、Runtime Settings policy-owned。
