...
| `packages/app-server/src/session-identity.ts`                                                          | Initial audit found `resolveWorkspaceAvatarSessionId(_workspacePath, avatar)` ignored workspace path and resolved runtime id from Avatar only.                                                                             | The code path already encoded avatar-first runtime identity despite the legacy function name.                                                          |
| `packages/app-server/src/session-catalog.ts`                                                           | Initial audit found `findByWorkspaceAvatar(workspacePath, avatar)` delegated to `findByAvatar(avatar)`, while `workspacePath` remained metadata / bootstrap context.                                                       | The old `workspace + avatar` API shape was compatibility residue; runtime lookup no longer used project workspace as identity.                         |
...
| `packages/app-runtime/src/assistant-init.ts`                                                           | `appAssistantEnsureInputSchema` still accepted `workspacePath` for Avatar ensure even though `AppRuntimeClient.ensureAssistant` only used Avatar nickname/display/classify.                                                | API shape kept project authority visible after the implementation logic had already stopped using it.                                                  |
| `packages/app-runtime/src/runtime-session.ts`                                                          | `appRuntimeSessionClearInputSchema` still modeled clear as `workspacePath + avatarNickname`.                                                                                                                               | Runtime reset authority conflicted with Avatar-scoped runtime identity.                                                                                |
| `packages/client-sdk/src/app-runtime.ts`                                                               | `clearRuntimeSession` filtered by `session.workspacePath` and `session.avatar`.                                                                                                                                            | Client behavior could preserve stale same-principal sessions if they came from another project path.                                                   |
...
- Initial Memory pack seeding used WorkspaceSystem private assets selected by `workspacePath + avatarNickname`.
...
- Follow-up audit found remaining API/naming residue where assistant ensure, runtime clear, and session id helpers still exposed project-shaped parameters even when implementation already treated runtime identity as Avatar-scoped.
...
- [x] 3. Write specs from the selected intent.
- [x] 4. Write BDD tasks from specs.
- [x] 5. Implement tasks.
- [x] 6. Self-review against intent and decide whether to loop.
- [x] 7. Reopen around project-hard-code residues and remove assistant/session API shapes that still made project workspace look like identity authority.
