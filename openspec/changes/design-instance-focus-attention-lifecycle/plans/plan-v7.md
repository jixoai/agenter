...
- Round: 1
- Status: research-plan updated after separating kernel lifecycle capability from shell-next-owned terminal-room binding; no implementation started
...
| -------- | ----------------------------- | ------------------------------------- |
| Should the shell-next product binding be persisted as a durable first-class graph, or is reconstructing it from owner metadata acceptable for now? | Automatic lifecycle reactions need a reliable terminal -> room relation even after restart and possibly when shell-next UI is not running. | Recommend first-class durable binding graph or typed binding index, but the concrete terminal-room relation remains shell-next-owned. |
| Should terminal killed -> room archived run even when the shell-next process is already closed? | If yes, the execution host cannot live only inside the local shell-next TUI process. | Recommend daemon-side product lifecycle host that executes shell-next-declared policy without making it a kernel-owned terminal-room law. |
...
| Should shell-next direct in-memory instance access mean direct imports or a daemon-side product host? | A local product process cannot safely import app-server's singleton instances without collapsing process boundaries. | Recommend daemon-side host gets real instances and runs shell-next-owned policy; shell-next process consumes typed lifecycle API/projections. |
...
- User confirmations still required:
  ...
  - Whether automatic shell-next room archiving uses a daemon-side host for durability while keeping policy ownership in shell-next.
...
5. Product extension runtime owns the generic binding/lifecycle host surface.
6. shell-next owns the concrete product binding and policy: for a shell-next binding, terminal killed archives the bound room.
...
| Lifecycle reaction host location | If the reaction must run after shell-next exits, the host belongs in daemon/product-extension runtime, not local TUI. | Use a durable daemon-side host, but treat the terminal-room binding and archive reaction as shell-next-owned policy. |
...
| Direct instance access | Direct shell-next imports would collapse extension/process boundaries. | Put real instance access in a daemon-side host that executes shell-next-owned policy; shell-next receives typed projections/API. |
...
- Persist or index product-owned bindings so shell-next's terminal -> room relation is inspectable after restart without becoming a universal kernel relation.
- Add a generic daemon-side product lifecycle reaction host that can observe real terminal/room control-plane lifecycle facts and invoke shell-next-declared product policy.
...
Why this is the durable path: it keeps all core systems orthogonal, makes shell-next an ordinary product policy owner, and lets future products bind terminal/room/task/browser resources without copying shell-next-specific glue.
...
Why this is weaker: it only works while the shell-next process is alive, reconstructs causal binding from metadata each time, and risks becoming another product-specific watcher pattern. It can be acceptable as a narrow stepping stone only if the durable binding host law is still written first.
...
Rejected because shell-next is an extension/product process. Direct imports would make process-local implementation shape the platform boundary, and it would not naturally survive daemon restart or remote product hosting. The correct place for true memory instances is inside the daemon/app-server product lifecycle host; the ownership of the terminal-room policy still belongs to shell-next.
...
- Custom exit condition from intent: before implementation, the change must state whether it is creating a new instance URI/ref law or extending existing product metadata binding, and must state both where the shell-next lifecycle reaction runs and that shell-next owns the terminal-room binding policy.
