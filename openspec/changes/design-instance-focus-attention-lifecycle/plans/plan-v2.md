...
| `packages/app-server/src/attention-src.ts` and `loopbus-plugin-runtime.ts` | Runtime source refs use protocol-native namespaces such as `msg:<chatId>/<messageId>`, `tty:<terminalId>[/eventId]`, and `task:<subjectId>`. | There is a typed `src` law for source reading, but it is not a global instance URI law for product binding. |
...
- `sourceRef.src`: source-read address such as `tty:<terminalId>` or `msg:<chatId>/<messageId>`; useful for attention facts, but not enough by itself to encode a product terminal-room binding.
...
- Gives product binding a natural instance ref for room lifecycle: `room:<chatId>`.
...
| Is `room:<chatId>` the canonical instance ref, or should a new URI such as `agenter://room/<chatId>` wrap it? | Source refs and product instance refs may be separate layers. | Prefer protocol-native short refs for LoopBus/source identity and structured `InstanceRef` for product binding; avoid overloading one string with every role. |
...
| Path | Why rejected |
| ---- | ------------ |
| TerminalSystem directly archives MessageSystem rooms when a terminal dies. | Violates orthogonal atoms; TerminalSystem would own product topology. |
...
| Treat `msg:` / `tty:` source refs as the complete product binding format. | They address source reads, not multi-resource product ownership or lifecycle reactions. |
