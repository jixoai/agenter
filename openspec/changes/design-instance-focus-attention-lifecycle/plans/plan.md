...
- Round: 1
- Status: research-plan updated after removing the over-platformized product lifecycle host idea; no implementation started
...
> 1. `msg:<chatId>/<messageId> 只表达 transcript row`这行什么意思，你的意思是表达“一条消息”？但是messageSystem本身并不表达消息啊，messageSystem目前承载的是“向谁发消息”的能力，它是一种联系人的能力。我更倾向于 `msg:from/to` 这样的设计。但我也不确定这样的设计有没有用。还需要讨论
> 2. 我没看明白，用大白话或者用我的话语体系来和我沟通

> `msg:<from>/<to>` => `msg:<superadmin|domain|source>/<contact>` 这样会合理一些，可以用来定位某个实例的某个用户
>
> 我们可以在底层去使用它，至于superadmin|domain|source，这三个其实都一样，都是superadmin的address

> 没必要用entry，直接用`room:<roomId>#entryId`。因为消息不能脱离房间，房间是它的上下文，我们必须先到房间，才能通过房间来查询entryId。没必要专门为某一条记录去做一个协议头。通过`room:...#`这样的标识也能很容易认出这是指room-entry

> 首先，生命周期的能力确实是由内核提供的，但是终端与房间的绑定关系，这是 Shell next 这个应用特有的。其他的建议我没有意见 你重新再梳理一下。

> - Product runtime: 提供可持久化 binding / lifecycle reaction host
>
> 从这一句就可以看出你的架构预想存在问题，这个根本就不用做到底层去。在 shell next 这边 通过编程的能力来实现这个功能就行了。就跟 vite 和 vite 生态的插件，根本不用耦合到 Vite 的内部去
...
| 4 | User | Refine `msg:<from>/<to>` into `msg:<superadmin|domain|source>/<contact>`, where the first segment is effectively a superadmin address and the ref can locate a user/contact for an instance. | `msg:` is now a candidate bottom-layer contact locator, not a transcript-row source. |
| 5 | User | Do not create a separate entry protocol head; use `room:<roomId>#<entryId>` because a message/entry cannot be queried outside the room context. | Row-level identity is a room-fragment address, not a standalone namespace. |
| 6 | User | Lifecycle capability is kernel-provided, but terminal-room binding is Shell Next application-specific. | Distinguish execution substrate from policy ownership: kernel hosts lifecycle mechanics; shell-next owns this concrete binding and reaction. |
| 7 | User | Reject making a bottom-level Product runtime lifecycle reaction host; shell-next should implement the terminal-room binding reaction through its own programming ability, like Vite plugins do not couple into Vite internals. | Remove product lifecycle host as a platform law; keep only public lifecycle APIs/events that shell-next can program against. |
...
| `extensions/shell-next/src/product/runtime.ts` | Terminal source `terminateTerminal` only calls `stopGlobalTerminal({ terminalId })`. | There is no visible shell-next product-code reaction here that archives the bound room after terminal death. |
...
| `openspec/specs/attention-src-registry/spec.md` | `msg` currently formats both room-scope `msg:<chatId>` and row-like `msg:<chatId>/<messageId>` refs, and groups row sources by room bucket. | Splitting `room:` from `msg:` would redefine the source registry boundary, not just rename strings; the old names may be conceptually wrong. |
| `packages/message-system/src/message-control-plane.ts` | In-memory `MessageControlPlane` has channel listeners, focus listeners, create/list/get/archive/unarchive/delete channel APIs, and follow-up attention writes using `msg:<chatId>/<messageId>`. | Current package code mixes room-management implementation with message delivery surfaces; this should not make `msg:` the durable room transcript namespace by default. |
...
| `realign-cli-shell-with-core-system-boundaries` | Product binding should expose terminal id, room id, AvatarRuntime identity, and attention context ids. | Extend; this is close to the projection shell-next needs in order to program its own lifecycle reaction. |
...
| `底层房间与终端的关键，需要从AGENTER.mdx这个提示词出发` | The AI's semantic choice of which room/terminal to use belongs to prompt-guided reasoning and explicit user instruction. | The platform provides refs/APIs; shell-next provides its product binding; Avatar decides operational association in context. |
...
| -------- | ----------------------------- | ------------------------------------- |
| Where should the shell-next terminal-room binding reaction live? | It is shell-next application-specific, not a platform lifecycle law. | Implement it in shell-next product code using public terminal lifecycle and room archive APIs. |
| Does this need a platform Product runtime reaction host? | User explicitly rejected pushing this to the bottom layer. | No. Platform should expose lifecycle APIs/events; shell-next programs the reaction, similar to plugin code using host APIs. |
...
| Should the source namespace split be `room:` for room lifecycle/scope and `msg:` for delivery/contact routes? | This changes the meaning of attention source identity after room-management separation. | `room:` naturally names room lifecycle/state. `msg:<superadmin-address>/<contact>` is the current candidate for delivery/contact capability. |
| If transcript rows need their own source ref, should they be a child of `room:` rather than `msg:`? | A row in room history belongs to room-management, not necessarily messageSystem's delivery/contact capability. | Use `room:<roomId>#<entryId>`; no separate `entry:` / `room-entry:` protocol head is needed. |
| Should shell-next direct in-memory instance access mean direct imports? | A local product process should not collapse core boundaries. | No direct core imports; shell-next uses public APIs/projections/events, but the reaction code itself belongs in shell-next. |
...
3. product-specific bindings express app-owned cross-resource policy without making TerminalSystem import MessageSystem or AttentionSystem;
...
This must not mean "the Avatar can only discuss that terminal in that room". If the user says "go discuss it in shell-181", the Avatar should be able to use the exposed room/terminal refs and choose that route through prompt-guided reasoning. The platform keeps lifecycle facts coherent; shell-next keeps its product binding facts coherent.
...
- Current platform laws:
  ...
  - Product extension runtime currently binds resources through `productId + resourceKey + resourceKind + ownerSystem` metadata, but this should remain an API surface, not a bottom-layer lifecycle reaction engine.
...
- Does this fit as a regular atom:
  ...
  - shell-next terminal -> room archive is shell-next's product policy; it fits only if the terminal-room relation is explicit and durable enough.
...
- Breaking update stance:
  - Do not push shell-next's terminal-room reaction into a generic platform host. Prefer public lifecycle APIs/events plus shell-next product code.
- User confirmations still required:
  - Whether to create a new first-class instance URI/ref law now.
  - Whether shell-next's programmed reaction only needs to run while shell-next is active, or whether shell-next should own its own background worker/plugin process.
  - Whether `room:` should replace room-scope `msg:<chatId>`, and whether row-level transcript refs should be room-owned instead of `msg:`-owned.
...
- The product cannot express semantic routing:
  ...
  - "shell-next binding X contains terminal A and room B" is a shell-next-owned product fact persisted through platform binding infrastructure.
...
- Product binding metadata: current reconstruction surface; shell-next may use it through public APIs, but the platform should not infer shell-next-specific terminal-room semantics from it.

Plain-language namespace model:

- `room:<roomId>` names the room world: room instance lifecycle, metadata, membership, read state, archive/delete, room revision, and transcript revision facts owned by room-management.
- `msg:<superadmin-address>/<contact>` is the current candidate for the messageSystem world: a contact locator / delivery route, meaning "under this superadmin address, locate this contact".
- `superadmin`, `domain`, and `source` are naming variants for the same first segment in this discussion: the superadmin address.
- A transcript row is a trace left inside the room world. It should not automatically be called `msg:` just because it contains user-visible message text.
- If row-level addressing is needed, use a room fragment ref: `room:<roomId>#<entryId>`. The room is the required context for resolving the entry id.
- `msg:<chatId>` should disappear in the new law if `room:<roomId>` exists, because a room-scope address is not a message delivery route.

Meaning:

- `room:` is the place / field / room instance.
- `msg:` is the line / route / contact locator for sending.
- A transcript row is the footprint left in the room after communication happens, so its address is a room fragment rather than a new protocol family.
- Notification grouping can bucket row-level room entries under `room:<roomId>` instead of using `msg:<chatId>` as both row bucket and room source.
...
- Leaves `msg:` free to model messageSystem's real authority: under which superadmin address, locate which contact, and send through which route.
...
- Message follow-up tasks currently commit `msg:<chatId>/<messageId>`; specs must decide whether that legacy shape migrates to a room-owned row ref or stays as compatibility during transition.
...
5. Product extension/runtime APIs expose generic resource binding, lifecycle events, and owner APIs.
6. shell-next code owns the concrete product binding and policy: for a shell-next binding, terminal killed archives the bound room.
...
| Lifecycle reaction location | This is shell-next application behavior, not a bottom-layer Product runtime host. | Implement in shell-next product code by subscribing to terminal lifecycle and calling room archive APIs. |
...
| `msg:` split | This affects source registry, notification grouping, and message follow-up refs. | Prefer `room:<roomId>` for room lifecycle/scope; keep `msg:<superadmin-address>/<contact>` as the delivery/contact candidate; do not assign transcript rows to `msg:` without a spec decision. |
| Direct instance access | Direct shell-next imports would collapse extension/process boundaries. | shell-next uses public typed projections/API/events; it does not import core control-plane singletons. |

## Options

### Option A: Public Lifecycle APIs + Shell-Next Product Code, Recommended

Keep the platform law small and let shell-next program the product-specific reaction:

- Define typed instance refs, either as structured `{ system, id }` objects or canonical URI strings.
- Optionally align source refs with the instance law: `tty:<terminalId>`, `room:<roomId>`, `room:<roomId>#<entryId>`, and `msg:<superadmin-address>/<contact>`.
- Ensure TerminalSystem exposes a public lifecycle event/API that shell-next can subscribe to or await.
- Ensure room-management exposes a public archive API.
- Add room archive -> room context muted in the runtime/message adapter boundary.
- Implement shell-next's terminal-room binding and terminal killed -> archive bound room reaction in shell-next code.
- Keep semantic terminal-room work routing out of product code and in AGENTER.mdx/Avatar reasoning.

Why this is the durable path: it keeps all core systems orthogonal, avoids a bottom-layer Product runtime reaction host, and treats shell-next like a Vite plugin/ecosystem consumer: it uses public hooks and APIs rather than coupling into platform internals.

### Option B: Bottom-Layer Product Lifecycle Host, Rejected

Create a generic daemon-side product lifecycle reaction host that observes terminal/room lifecycle and invokes product policies.

Rejected because this pushes shell-next's application-specific terminal-room relation into the bottom layer. The platform should expose hooks; shell-next should program against them.
...
Rejected because shell-next should use public APIs/events rather than importing core control-plane singletons. This preserves the same boundary as plugin ecosystems: product code is programmable, but not coupled to internals.
...
| Is `room:<roomId>` the canonical instance ref, or should a new URI such as `agenter://room/<roomId>` wrap it? | Source refs and product/app instance refs may be separate layers. | Prefer protocol-native short refs for LoopBus/source identity and use typed refs in shell-next code where needed; avoid overloading one string with every role. |
| Does `msg:<superadmin-address>/<contact>` deserve to be an attention source, or only a messageSystem route/ref? | A delivery route is not the same thing as room history changing. | Treat it as a bottom-layer route/contact ref until we find a concrete source-read or attention use case. |
...
| Give shell-next direct in-process ownership of app-server control-plane instances. | Collapses extension boundary; shell-next should program against public APIs/events instead of importing core internals. |
...
- Custom exit condition from intent: before implementation, the change must state whether it is creating a new instance URI/ref law or extending existing product metadata binding, and must state that shell-next implements terminal killed -> archive bound room through product code using public lifecycle APIs/events.
- Additional exit condition: before implementation, specs must state whether room-scope source identity is `room:<roomId>`, whether `msg:<superadmin-address>/<contact>` becomes the bottom-layer message/contact locator after messageSystem/room-management separation, and must state that code binds lifecycle state only while semantic room-terminal routing remains Avatar/prompt-owned.
