...
- Round: 1
- Status: research-plan updated after removing the over-platformized app lifecycle host idea; no implementation started
...
> - App runtime: 提供可持久化 binding / lifecycle reaction host
>
...
| 7 | User | Reject making a bottom-level App runtime lifecycle reaction host; shell-next should implement the terminal-room binding reaction through its own programming ability, like Vite plugins do not couple into Vite internals. | Remove app lifecycle host as a platform law; keep only public lifecycle APIs/events that shell-next can program against. |
...
| `apps/shell-next/src/app/runtime.ts` | Terminal source `terminateTerminal` only calls `stopGlobalTerminal({ terminalId })`. | There is no visible shell-next app-code reaction here that archives the bound room after terminal death. |
...
| `realign-cli-shell-with-core-system-boundaries` | App binding should expose terminal id, room id, AvatarRuntime identity, and attention context ids. | Extend; this is close to the projection shell-next needs in order to program its own lifecycle reaction. |
...
| `底层房间与终端的关键，需要从AGENTER.mdx这个提示词出发` | The AI's semantic choice of which room/terminal to use belongs to prompt-guided reasoning and explicit user instruction. | The platform provides refs/APIs; shell-next provides its app binding; Avatar decides operational association in context. |
...
| -------- | ----------------------------- | ------------------------------------- |
| Where should the shell-next terminal-room binding reaction live? | It is shell-next application-specific, not a platform lifecycle law. | Implement it in shell-next app code using public terminal lifecycle and room archive APIs. |
| Does this need a platform App runtime reaction host? | User explicitly rejected pushing this to the bottom layer. | No. Platform should expose lifecycle APIs/events; shell-next programs the reaction, similar to plugin code using host APIs. |
...
| Should shell-next direct in-memory instance access mean direct imports? | A local app process should not collapse core boundaries. | No direct core imports; shell-next uses public APIs/projections/events, but the reaction code itself belongs in shell-next. |
...
3. app-specific bindings express app-owned cross-resource policy without making TerminalSystem import MessageSystem or AttentionSystem;
...
- Current platform laws:
  ...
  - App extension runtime currently binds resources through `appId + resourceKey + resourceKind + ownerSystem` metadata, but this should remain an API surface, not a bottom-layer lifecycle reaction engine.
...
- Does this fit as a regular atom:
  ...
  - shell-next terminal -> room archive is shell-next's app policy; it fits only if the terminal-room relation is explicit and durable enough.
...
- Breaking update stance:
  - Do not push shell-next's terminal-room reaction into a generic platform host. Prefer public lifecycle APIs/events plus shell-next app code.
...
- The app cannot express semantic routing:
  ...
  - "shell-next binding X contains terminal A and room B" is a shell-next-owned app fact persisted through platform binding infrastructure.
...
- App binding metadata: current reconstruction surface; shell-next may use it through public APIs, but the platform should not infer shell-next-specific terminal-room semantics from it.
...
5. App extension/runtime APIs expose generic resource binding, lifecycle events, and owner APIs.
6. shell-next code owns the concrete app binding and policy: for a shell-next binding, terminal killed archives the bound room.
...
| Lifecycle reaction location | This is shell-next application behavior, not a bottom-layer App runtime host. | Implement in shell-next app code by subscribing to terminal lifecycle and calling room archive APIs. |
...
### Option A: Public Lifecycle APIs + Shell-Next App Code, Recommended

Keep the platform law small and let shell-next program the app-specific reaction:
...
Why this is the durable path: it keeps all core systems orthogonal, avoids a bottom-layer App runtime reaction host, and treats shell-next like a Vite plugin/ecosystem consumer: it uses public hooks and APIs rather than coupling into platform internals.

### Option B: Bottom-Layer App Lifecycle Host, Rejected

Create a generic daemon-side app lifecycle reaction host that observes terminal/room lifecycle and invokes app policies.
...
Rejected because shell-next should use public APIs/events rather than importing core control-plane singletons. This preserves the same boundary as plugin ecosystems: app code is programmable, but not coupled to internals.
...
| Is `room:<roomId>` the canonical instance ref, or should a new URI such as `agenter://room/<roomId>` wrap it? | Source refs and app/app instance refs may be separate layers. | Prefer protocol-native short refs for LoopBus/source identity and use typed refs in shell-next code where needed; avoid overloading one string with every role. |
...
- Custom exit condition from intent: before implementation, the change must state whether it is creating a new instance URI/ref law or extending existing app metadata binding, and must state that shell-next implements terminal killed -> archive bound room through app code using public lifecycle APIs/events.
