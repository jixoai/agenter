...
- Round: 1
- Status: research-plan updated after user corrected the `msg:` / `room:` boundary; no implementation started
...
- If row-level addressing is needed, prefer a room-owned child ref such as `room:<roomId>#entry/<entryId>` or a named `room-entry:<roomId>/<entryId>` family, pending spec design.
...
- A transcript row is the footprint left in the room after communication happens.
