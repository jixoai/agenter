...
TerminalSystem SHALL remain a terminal system product with its own terminal instances and contracts. cli-shell's tmux shell pane, room pane, and tmux session topology SHALL remain extension-local state and SHALL NOT be represented as TerminalSystem `terminal-1`, `terminal-2`, or composed product-terminal roles.
...
- **AND** TerminalSystem does not receive composed cli-shell product chrome metadata
...
- **THEN** it does not publish `ProductTerminalComposedSurfaceState`
