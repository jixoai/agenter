...
- Round: 2
- Status: User confirmed that `.scm/.wasm` boundary changes stay investigation-first and locked the ghostty phase-1 host matrix; the plan now encodes both constraints.
...
| -------- | ----------------------------- | ------------------------------------- |
| Now that the provenance audit shows OpenTUI tree-sitter assets leave `bundle/agenter` only if the bundled TUI edge changes, should phase 1 widen into that boundary change? | The investigation is done; the remaining question is scope, not uncertainty. | Current evidence says yes: without a package/release boundary change or upstream OpenTUI export split, the assets stay in `bundle/agenter`. |
...
- User confirmations still required:
  - Whether phase 1 should widen into a package/release boundary change for the bundled TUI edge, because current evidence says that is the only clean way to evict OpenTUI syntax assets from `bundle/agenter` without an upstream `@opentui/core` export split.
...
- The current OpenTUI asset conclusion is structural: as long as `bundle/agenter` bundles `@agenter/tui`, and `@agenter/tui` imports the monolithic `@opentui/core` entry, the syntax assets stay. Removing them cleanly therefore requires either a package/release boundary change for the TUI edge or an upstream/export-level split in OpenTUI.
...
| ---- | ---------------------------- | -------------------------- |
| Package/release boundary for OpenTUI syntax assets | The provenance audit now shows the assets leave only when the bundled `@agenter/tui` edge leaves `bundle/agenter`, or when OpenTUI itself offers a lean export surface. | Do not widen scope automatically; return with the conclusion and wait for confirmation. |
...
| -------- | -------------- | ------------------------------------- |
| Should phase 1 widen into a package/release boundary change for the bundled TUI edge now that the provenance audit is complete? | This determines whether `.scm/.wasm` stays a provenance-only finding or escalates into a new packaging/product boundary change. | No boundary change by default; the evidence is now ready for explicit scope approval. |
...
- Custom exit condition from intent:
  ...
  - No un-discussed package-boundary or platform-matrix decision should be hidden inside the artifacts.
