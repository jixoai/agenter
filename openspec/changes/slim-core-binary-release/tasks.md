...
- [x] 1.4 Lock the phase-1 `@jixo/ghostty-native-*` platform matrix to `darwin`, `windows`, `linux-arm64`, and `linux-amd64`, then translate that approved host set into explicit package names and CI coverage.
...
- [ ] 2.12 Confirm each task checkbox will be updated only by the agent that completed and verified that task in the current working context.
