...
- Round: 2
- Status: Investigation extended with `../openspecui` reactive filesystem survey, default Avatar locked-prompt law, and package namespace decision; plan revised before specs/tasks.
...
> 这个包使用 @jixo/reactive-fs, 未来方便openspecui这个项目也可以复用。其它我没意见，开始完善change
...
| 8 | User | The extracted package must be named `@jixo/reactive-fs` so `../openspecui` can reuse it in the future. | Reactive FS is a cross-project Jixo platform atom, not an Agenter-only package. |
...
The file-reactivity layer should be extracted from `../openspecui` into an independent Jixo package named `@jixo/reactive-fs`. Prompt authority then becomes one consumer of this package: Avatar startup/model-boundary prompt rendering reads through the reactive filesystem, and prompt dependency changes produce a declared downstream refresh signal instead of an invisible in-memory mutation.
...
- Reactive filesystem is a regular platform atom:
  ...
  - It can therefore be moved as an independent package, `@jixo/reactive-fs`, instead of being embedded into PromptStore.
...
- A new `@jixo/reactive-fs` package should expose the reactive primitives imported from `../openspecui`: `ReactiveContext`, `ReactiveState`, reactive file/stat/dir operations, watcher pool initialization/status, and project watcher status.
...
| Package name and extraction target for reactive FS | Creates a new reusable atom and public/internal API surface. | Use `@jixo/reactive-fs`; keep prompt semantics outside it so OpenSpecUI can reuse it. |
