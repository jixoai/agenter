...
The intended app law is already clear:

1. `Run in Background` should exit the shell-next UI without destroying the attached PTY or its app binding.
...
4. `apps/cli-shell` stays read-only.
...
- Ensure app command launch uses a managed daemon authority that is not stopped when the foreground app process exits.
...
- Keep shell-next app/view code as app-global routing, raw event forwarding, and visual projection only.
...
- Affects `packages/cli` app daemon bootstrap and `apps/shell-next` attach-client close-confirm flow.
