...
本轮复核后确认，导致“下一次 selector 消失”的强根因在 CLI product launcher：当没有可复用 daemon 时，它启动了 in-process daemon，并在 product 退出后 stop。`AppKernel.stop()` 会 dispose TerminalSystem control plane，进而停止 live terminal，所以 selector 只能看到 terminal 消失。

本轮修正：

- product command launcher 不再暴露/持有 `startDaemon()` 返回的 local daemon handle；它改为 `ensureManagedDaemonAuthority()`，启动或复用 managed background daemon authority。
- foreground product 退出后不再 stop daemon；daemon shutdown 只属于显式 `agenter daemon stop/restart`。
...
- `Terminate terminal` 仍然先调用 terminal source `terminate()`；product-bound live terminal 会调用 `stopGlobalTerminal`。
- product attach runtime 一律在 app 结束后调用 `store.disconnect()`，因为它只是本地 SDK transport/subscription cleanup，不是 terminal/resource lifecycle cleanup。
...
- 之前把 `Run in Background` 的验证停在“terminal source 没有 terminate”，覆盖太浅，没有覆盖 product launcher 的 daemon ownership。
