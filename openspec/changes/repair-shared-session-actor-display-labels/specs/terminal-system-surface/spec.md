...
#### Scenario: Session-backed terminal actor prefers the human avatar label
- **WHEN** a terminal seat or `call as` option resolves to a running session that exposes both a human `avatar` label and an opaque runtime `name`
