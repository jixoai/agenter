...
### Requirement: Raw PTY transport SHALL remain boundary-local for cli-shell product composition

Raw PTY transport SHALL remain lawful at terminal boundaries that understand terminal control bytes, but cli-shell product composition SHALL NOT treat raw bytes as the final product truth when terminal-2 composed screen truth is required.
...
- **WHEN** terminal-2 final product screen must be rendered by a raw-capable host
- **THEN** terminal-2 MAY encode the final product screen into raw output
...
- **WHEN** `cli-shell --web` renders the product surface
- **THEN** it SHALL consume terminal-2 final product output
- **AND** it SHALL NOT directly stream terminal-1 raw PTY bytes as the official Web product surface

### Requirement: Cli-shell transport SHALL support native stdout and Web transport as equivalent terminal-2 adapters

Cli-shell SHALL support native stdout and Web browser transport as equivalent adapters over terminal-2 final product screen.
...
- **AND** it SHALL NOT require a separate terminal-2 child PTY just to pipe final product bytes back into the same host

#### Scenario: Web adapter streams terminal-2 product output
- **WHEN** cli-shell runs in `--web` mode
- **THEN** it SHALL stream terminal-2 product output to the browser terminal renderer
- **AND** the browser SHALL observe the same final product surface as native mode

#### Scenario: Transport adapter does not own product state
- **WHEN** either native stdout or Web transport emits terminal-2 output
- **THEN** the adapter SHALL NOT own shell selection, dialogue selection, scrollbar state, focus state, or product chrome truth
...
#### Scenario: Row-cache is not product-layer duplicate suppression
...
- **AND** product, viewport, and frontend rendering code SHALL NOT skip work by comparing business-level visible frame objects before serialization
