...
- `--web` can accidentally render a partial shell view instead of the same product surface as native cli-shell.
...
- terminal-2 can be treated as a pass-through projection rather than the final product screen truth.
...
terminal-2
  composed final product screen truth
  shell offscreen frame + terminal-chat frame + bottom/status/product chrome
...
The important physical boundary is that terminal-2 owns the visible product result. Native and Web are only host adapters for terminal-2.

## Goals / Non-Goals

**Goals:**

- Make `--web` visually and behaviorally equivalent to native cli-shell for the final terminal-2 product surface.
...
- Keep terminal-2 as a composed product terminal truth that can be encoded to raw output for native and Web hosts.
- Remove host-local accepted product chrome as a durable source of truth.
...
- Do not redesign cli-shell product semantics such as Avatar selection, managed mode, room identity, or product session naming except where they intersect terminal projection truth.
...
- Treat raw transport as the only real contract and rebuild all product chrome by writing ANSI into another PTY.
...
Terminal-2 must have terminal identity, geometry, frames, cursor/focus metadata, viewport state, and publication. It does not need a real child PTY process because it does not run a shell command. It is the composed product output.
...
- Render dialogue as plain product text inside terminal-2 with local scroll state.
...
### 5. Terminal-2 Compositor Owns Final Product Screen Only

Terminal-2 compositor combines already-rendered offscreen frames and product chrome into one final product screen:

```txt
terminal-2 = layout(shell-offscreen-frame, terminal-chat-frame, bottom/status/product chrome)
```
...
host event
  ...
  -> native and Web render the same product result
```
...
`--web` is not a reduced view and not a shell-only debug surface. It exists so automated browser E2E can verify the same product screen that users see natively, because Ghostty accessibility is not reliable enough for the assistant to validate all UI behavior directly.

Rejected alternative:

- Let `--web` render only terminal-1 or a product-local Web approximation.
...
Frame payload optimization belongs in the transport codec layer, not in product, viewport, or frontend rendering logic. A client still pulls frames on its own cadence and redraws the decoded cells through the same paint path. The optimization only changes how the current viewport rows are encoded between backend and client.
...
- Add a product-layer duplicate-frame skip before serialization.
...
- [Raw output from a composed frame may be less efficient than direct PTY raw forwarding] -> Keep direct raw forwarding for terminal-1 where the target is shell-only; for cli-shell product hosts, correctness of terminal-2 final product equivalence wins. Add diff/frame optimization after the single truth is stable.
...
6. Refactor terminal-2 to publish the final product screen used by both native and Web.
7. Make native cli-shell write terminal-2 output to current stdout.
8. Make `--web` consume terminal-2 final product screen and match native layout/effects.
