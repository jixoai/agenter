...
The fixed `Avatars / Catalog` surface SHALL remain list-first and density-aware across desktop and compact viewports. The selected runtime lens SHALL keep runtime identity, status, and jump actions readable without promoting long runtime ids or repeated card framing above the primary avatar identity hierarchy.

#### Scenario: Compact avatar catalog keeps identity and runtime facts readable

- **WHEN** the operator opens `Avatars / Catalog` on a compact viewport
- **THEN** the catalog still shows the selected avatar identity and runtime lens without top-edge fact collisions or horizontal overflow
- **THEN** long runtime ids and paths are demoted into dense metadata blocks instead of crowding the route title area

#### Scenario: Desktop avatar catalog stays list-first while lens actions remain adjacent

- **WHEN** the operator opens `Avatars / Catalog` on a desktop viewport
- **THEN** the left catalog list remains the primary scanning surface
- **THEN** the selected runtime lens keeps status, actions, and canonical facts visually grouped without a large empty secondary card field

#### Scenario: Avatar catalog keeps the shared workbench language while tightening compact hierarchy

- **WHEN** the operator revisits `Avatars / Catalog` after the shell already establishes tab and toolbar context
- **THEN** the route does not introduce an extra route-local title dialect or repeated title band just to restate location
- **THEN** the compact runtime lens stacks title, passive facts, and actions without first-line crowding
