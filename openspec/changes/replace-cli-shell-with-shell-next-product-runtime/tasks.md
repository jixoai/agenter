...
- [x] 1.1 Create OpenSpec proposal, design, delta specs, and implementation tasks for shell-next app-runtime parity.
- [x] 1.2 Update the `shell2` app descriptor to remain local-only but advertise daemon-backed runtime planes once attach is implemented.
- [x] 1.3 Add launcher tests proving `shell` remains routed to `agenter-app-shell` and `shell2` remains routed to `agenter-app-shell-next`.

## 2. App Bootstrap And Argv
...
- [x] 2.2 Add shell-next app runtime dependencies for daemon client/store creation, settings read/write, keybindings read, bootstrap, navigation selection, and app startup.
- [x] 2.3 Implement app attach selection: require explicit session/avatar in non-TTY, otherwise reuse navigation when needed and persist startup settings.
...
- [x] 3.1 Add a app-attached shell source that creates `CliShellLiveTerminalProtocolSource` from attached terminal transport data.
- [x] 3.2 Make default `shell2` app attach start from live TerminalSystem transport; keep Local BunPTY available only for explicit local/dev command mode.
...
- [x] 6.1 Run `openspec validate replace-cli-shell-with-shell-next-app-runtime --strict`.
- [x] 6.2 Run focused shell-next and launcher tests.
- [x] 6.3 Run `bun run --filter 'agenter-app-shell-next' typecheck`.
