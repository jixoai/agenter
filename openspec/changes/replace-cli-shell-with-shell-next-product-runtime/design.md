...
`shell-next` currently has a product-free renderable mux, layout tree, protocol pane boundary, FrameBuffer terminal projection, and visible Help/Chat/statusbar actions. The stable `agenter shell` command still routes to tmux-backed `agenter-ext-shell`; `agenter shell2` routes to local-only `agenter-ext-shell-next`.

The replacement gap is product runtime parity. `shell-next` starts local BunPTY panes today, while `cli-shell` performs daemon/client-sdk bootstrap, Avatar/session selection, TerminalSystem binding, MessageSystem Room binding, settings, managed attention state, approval top layer, and heartbeat status.
...
- Make shell-next's default product path attach to real daemon/client-sdk resources.
...
- Do not rename `extensions/cli-shell` or switch `agenter shell` in this change.
- Do not emulate tmux actions or status-bar dispatch in shell-next.
- Do not treat Local BunPTY as fallback for failed product attach.
- Do not publish shell-next or introduce a native addon.

## Decisions

### 1. Product attach is a separate shell-next runtime mode

`runShellNext` will parse cli-shell-compatible product argv and start a product attach mode by default. It will create a daemon client/store, auto-login through existing store behavior, resolve session/avatar selection, bootstrap terminal and room bindings, and then start `ShellNextApp` with product-bound sources and surfaces.
...
Additional split terminal panes should create their own product terminal bindings or explicitly use the chosen local source policy; they must not silently duplicate the same transport source.

### 3. Room and approvals are OpenTUI product surfaces, not terminal panes
...
### 5. Shared atoms must leave the legacy product boundary before replacement

During incubation, shell-next may import safe cli-shell atoms to move quickly. Before claiming replacement readiness, shared terminal projection/live mirror/settings/keybinding atoms must be extracted or relocated so stable shell-next does not depend on legacy `agenter-ext-shell` as a runtime package.

## Risks / Trade-offs

- [Risk] Reusing cli-shell atoms can keep a hidden dependency on legacy tmux product code. Mitigation: only import classified safe atoms and add tests that shell-next mux/core does not import tmux host modules.
...
1. Add shell-next product attach argv and bootstrap path behind `shell2`.
