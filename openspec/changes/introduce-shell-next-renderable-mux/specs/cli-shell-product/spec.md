...
The existing cli-shell product SHALL remain the implementation behind the stable `shell` command while shell-next is incubated. Shell-next SHALL use `shell2` as a separate local command until a later acceptance and rename change explicitly promotes it.

#### Scenario: Shell-next does not replace shell during incubation
- **WHEN** shell-next code exists under `extensions/shell-next`
- **THEN** `bun agenter shell` still starts the existing cli-shell product
...
- **THEN** the existing `extensions/cli-shell` directory remains in place
