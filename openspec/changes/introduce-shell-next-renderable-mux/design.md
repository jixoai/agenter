...
The current `agenter-ext-shell` product uses tmux as the native host/compositor. That made sense while the earlier FrameBuffer/offscreen terminal path was unstable, but it now creates the wrong incubation boundary: the product depends on a globally installed tmux/psmux runtime, and the codebase cannot mature a reusable embedded mux API while tmux owns split layout, focus, status, and pane hosting.

The new work starts in `extensions/shell-next` instead of rewriting `extensions/cli-shell` in place. `shell-next` is an incubation product, not a published package. It must be architected so the generic pieces can later move into an independent package while the product-specific pieces remain in the Agenter extension.

The user-facing product story is intentionally simple for the first iteration: run `bun agenter shell2`, see a native TTY shell surface, split panes like tmux, route input to the focused pane, and reuse existing cli-shell room/help/top-layer capabilities only where they do not drag tmux coupling into the new runtime.
...
- Create `extensions/shell-next` as a clean incubation area.
...
- Produce reusable renderable primitives such as `PaneRenderable`, `RootLayout`, and `ChildLayoutNode` rather than a product-only screen.
- Keep shell-next architecture separable into future packages:
  ...
  - Agenter shell product wiring.
- Reuse proven cli-shell atoms when they are orthogonal to tmux:
  - product bootstrap/client-sdk binding,
...
- Do not rename `extensions/cli-shell` or publish shell-next in this change.
...
- Do not let product concepts such as Avatar, MessageRoom, TerminalSystem, AttentionSystem, or launcher descriptors leak into the generic renderable mux model.
...
### 3. The core model is framework-shaped but product-free

The reusable core inside shell-next should start as product-free TypeScript modules:
...
- Rejected because it would reproduce the current product-shell coupling and make later package extraction expensive.
...
Product code supplies a pane source. Layout code supplies the rectangle. `PaneRenderable` only renders the source's current frame and forwards user input to the source.

The existing cli-shell terminal frame and live mirror code can seed this implementation, but the new primitive must have a cleaner constructor boundary and a smaller product surface.
...
- **OpenTUI renderable family**: OpenTUI renderable sources are for in-process product UI surfaces, such as internal Room/Help/statusbar work where using OpenTUI directly is more efficient for development and avoids fake terminal panes.
...
### 7. Product wiring stays outside renderable primitives

The Agenter-specific shell-next product layer owns:
...
The launcher will register a `shell2` product command that resolves locally to `extensions/shell-next`. It should not be treated as the future package name or published command. It is a side-by-side acceptance entry that allows comparing old `shell` and new `shell2`.
...
The statusbar is product chrome built with OpenTUI renderables. It is not part of `PaneRenderable`, and it does not become an AttentionItem renderer.
...
- product launcher tests proving `shell2` routes locally and `shell` remains unchanged,
...
### 11. Product completion requires the existing FrameBuffer projection stack

Sections 1-7 establish the mux/layout foundation only. Product completion requires shell-next terminal panes to reuse the already proven cli-shell terminal projection stack rather than treating terminal frames as plain text:
...
The renderable mux core must not import cli-shell. It should expose a terminal pane factory extension point, and shell-next product wiring can choose the frame-backed adapter. Local BunPTY sources may still be independent process-backed sources, but their snapshots must carry the same backend-owned facts needed by the frame-backed projection: rich lines, cursor, viewport start, and scrollback rows.

Help and Chat are product surfaces/actions, not mux laws. They should remain visible as OpenTUI-native actions in shell2, while the eventual Room integration can select either a direct OpenTUI renderable source or an isolated BunPTY/protocol source based on the hosted surface nature.

## Risks / Trade-offs

- [Risk] Reusing cli-shell code may import tmux assumptions into shell-next.
  - Mitigation: classify reused code as either product wiring, terminal projection, OpenTUI surface, or tmux-host residue before copying. Do not reuse tmux host/status/action modules in the generic mux layer.
- [Risk] OpenTUI layout/render invalidation may not be enough for high-frequency terminal repaint.
  - Mitigation: keep pane frame replay and render pacing explicit; test with the existing FrameBuffer/live mirror patterns before adding product chrome.
...
- [Risk] Shell-next may duplicate current cli-shell product code.
  - Mitigation: duplicate only during incubation when needed, but record extraction candidates in tasks and keep reusable modules product-free.
...
1. Introduce `extensions/shell-next` and `shell2` as an isolated local command.
...
5. Reuse product bootstrap and room/help/top-layer atoms only after the pane/layout runtime is testable.
...
7. After acceptance, perform a separate rename change:
   - current `extensions/cli-shell` -> shell legacy location/name,
   - `extensions/shell-next` -> stable shell,
