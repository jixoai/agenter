...
1. Long-term and legacy cli-shell specs still describe `terminal-1` / `terminal-2`, where TerminalSystem carries cli-shell product composition. That pollutes TerminalSystem with product chrome and product topology.
2. The tmux migration specs and `extensions/cli-shell/SPEC.md` then say the visible terminal surface is extension-local tmux. That removes Shell truth from TerminalSystem and makes Avatar/tooling unable to see the actual TerminalSystem target.
...
- Restore the system ontology: TerminalSystem is Shell truth; cli-shell is a TUI projection/input product.
- Keep `extensions/cli-shell` as the correct product ownership location.
- Define generic SDK laws product TUIs can reuse without cli-shell-specific core branches.
...
- Do not hard-code `if productId === "cli-shell"` inside TerminalSystem, MessageSystem, AvatarRuntime, or core prompt assembly.
- Do not restore `terminal-2` as product chrome truth.
...
### Decision 1: Define product TUI as projection plus input over core systems

cli-shell must model its first screen as a product view over a bound TerminalSystem terminal and a bound MessageSystem room. Product state such as status bar layout, open Chat pane, and top-layer position can be local UI state. Terminal output, terminal input effects, terminal write approvals, room messages, Avatar prompt, and attention remain core-system facts.
...
Rejected alternative: TerminalSystem owns cli-shell product chrome through `terminal-2`.

Reason: That makes TerminalSystem carry product topology and UI composition for one extension product.

### Decision 2: Product binding must create or select one TerminalSystem terminal resource

For `bun agenter shell --session=7 --avatar=bangeel`, cli-shell must have a product binding that can answer:

- current product resource key: `shell-7`
...
### Decision 3: Generic SDK comes before product implementation
...
One useful test for the boundary is to imagine there is another product named `cli-shell-web` that is completely separate from native `cli-shell` but wants to present the same core facts through a browser host. Under the corrected boundary, that thought experiment should feel boring:

- the Web product still consumes TerminalSystem for shell truth
...
- it still consumes the same product binding identities and terminal/room/action APIs
- only the host framework changes from tmux/OpenTUI to browser runtime + DOM renderer

If a future Web host would require new core branches just because it is Web, then the SDK boundary is still too product-shaped. The right answer is to widen the generic SDK surface, not to encode `cli-shell` or `cli-shell-web` behavior inside TerminalSystem, MessageSystem, AvatarRuntime, or AttentionSystem.

This also gives a concrete minimum-face test for the SDK:

- product binding must expose the current terminal id, room id, AvatarRuntime identity, and attention context ids
...
- runtime facts must expose current product/session binding without becoming a second prompt source
...
### Decision 4: Product runtime facts are not prompt ownership

cli-shell may provide current product facts to the selected Avatar: "this room is bound to terminal X and room Y". But `AGENTER.mdx` path resolution, Slot composition, prompt persistence, and prompt change detection remain Avatar/Core concerns.
...
### Decision 5: TopLayer and managed mode are product UI over core facts
...
Managed/on/off commits and settles product-scoped attention. It may be projected in the status bar and may affect Avatar scheduling. It does not grant terminal write authority and does not become a TerminalSystem mode.

### Decision 6: Existing active changes must be rewritten before apply

`move-cli-shell-to-extension-tmux-host` contains one valid architectural move: cli-shell belongs under `extensions/cli-shell`, and core must not import product implementation. But its "TerminalSystem is not a participant" statement is too strong and is superseded by this change.

`refine-cli-shell-tmux-product-shell` contains useful interaction stories for status, Chat, Help, and TopLayer. But its tmux-native product-shell wording is superseded and must be downgraded to "tmux/OpenTUI can host presentation for SDK-bound core resources."
...
- [Risk] Restoring TerminalSystem as Shell truth could be misread as restoring `terminal-2`.
  -> Mitigation: specs must distinguish "TerminalSystem owns Shell" from "TerminalSystem owns product chrome"; only the former is valid.

- [Risk] The generic SDK surface may become too broad or too product-shaped.
  -> Mitigation: every SDK method must be framed around core-system nouns and reusable product needs, not cli-shell UI nouns.
...
- [Risk] Developers may keep smuggling product context back into a second prompt source.
...
- [Risk] Prompt context for arbitrary selected Avatars can accidentally overwrite user files.
  -> Mitigation: product context must be injected as session/context guidance or other runtime facts only; no forced rewrite.
...
4. Update `extensions/cli-shell/SPEC.md` and durable specs before implementation resumes.
5. Implement SDK gaps with BDD tests at the product/core boundary.
