...
### Requirement: Extension root is the active first-party product source
...
FROM: `Agenter CLI SHALL launch first-party product command packages through descriptors`
TO: `Agenter CLI SHALL launch app command packages through descriptors`

FROM: `Product package resolution SHALL be local-first`
