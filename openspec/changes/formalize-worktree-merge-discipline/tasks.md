...
- [x] 2.3 Add a repo-local landing script that snapshots dirty target state, requires a fast-forward feature ref, lands it with `git merge --ff-only`, restores only non-overlapping dirty paths, and reports skipped overlaps.
...
- [x] 3.4 Update the durable operator guidance for the landing step and record targeted verification for the landing script. Verified `bash -n ./.gemini/scripts/wt-land-ff.sh`, `./.gemini/scripts/wt-land-ff.sh --help`, `./.gemini/scripts/wt-land-ff.sh HEAD --dry-run --report-file /tmp/wt-land-dryrun.txt`, and the non-fast-forward safety refusal via `./.gemini/scripts/wt-land-ff.sh a82220d --dry-run`.
