...
The current `@agenter/cli-shell` product has no product-facing way to select an official non-default terminal backend, so users cannot explicitly request `ghostty-native` even though the platform law already reserves that backend ownership slot for Termless. At the same time, cli-shell still treats `bottom` as a multi-row dialogue placement, which violates the terminal-first law now required for this product: the shell-terminal bottom surface must remain exactly one rendered line.

## What Changes

- Add explicit `--backend=<name>` grammar to `@agenter/cli-shell`, keeping the current default backend as xterm while allowing opt-in `ghostty-native`.
...
- Adopt official `@termless/ghostty-native` as the opt-in backend implementation for cli-shell while keeping xterm as the default backend until a later verified promotion change.
...
- `termless-backend-adoption`: adopt official `@termless/ghostty-native` as an opt-in backend without promoting it to the default slot.
