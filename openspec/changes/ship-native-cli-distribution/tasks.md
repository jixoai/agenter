...
- [x] 3.3 Introduce a single source of truth for the Agenter native CLI platform matrix, archive filenames, archive checksums, and npm/Homebrew projection mapping.
...
- [x] 3.6 Compile the Agenter CLI to standalone executables with Bun for every supported target and stage them into the canonical GitHub release archive layout.
- [x] 3.7 Replace the current JS-bundle-first release bundle path for `agenter` with release-archive-first packaging, while preserving the existing package/release manifest truth model.
- [x] 3.8 Update release tooling so npm projection packages are built from the canonical release archives instead of independently-built local binaries.
- [x] 3.9 Update GitHub Actions release automation to use Marketplace actions for Bun setup, GitHub release archive publishing, and downstream formula projection where appropriate.
...
- [x] 3.11 Generate and project the Homebrew formula from main-repo truth so `brew tap jixoai/agenter` and `brew install agenter` resolve to canonical release archives and checksums.
- [x] 3.12 Create the root `README.md` and document npm/Homebrew install surfaces, supported targets, and the release/archive truth model.
- [x] 3.13 Add concise intent comments at the critical effect points for platform resolution, archive truth projection, and Homebrew formula generation.
...
- [x] 4.2 Run targeted release-tooling tests proving GitHub release archives are emitted as canonical binary truth and downstream npm projection consumes them.
...
- [x] 4.6 Run `bun run openspec:vision -- commit-check ship-native-cli-distribution --phase self-review` before writing final review evidence.
- [x] 4.7 If GitHub auth is available, create or update the tap repository projection and verify the generated formula content matches main-repo truth; otherwise record the exact blocker and keep the rest of the verification evidence honest.
