...
> Its architecture assumptions about tmux-native product truth are superseded by `realign-cli-shell-with-core-system-boundaries`.

## Why

The first tmux migration fixed the core/product boundary, but it reduced cli-shell's product UI to a shallow tmux split: one shell pane plus one always-visible room pane. That removed the expected Chat entry and bottom status bar.

cli-shell should not go back to TerminalSystem `terminal-2`. The missing layer is a tmux-native product shell: tmux should host the interaction chrome through status line, key bindings, popup entry, and an explicit pane fallback policy.

## What Changes

- Add a tmux-native cli-shell product shell plan owned by `extensions/cli-shell`.
...
- Isolate cli-shell inside its own tmux socket namespace so product bindings do not mutate the user's default tmux server.
...
- `extensions/cli-shell/test/tmux-host.test.ts`
- `extensions/cli-shell/SPEC.md`
- `extensions/cli-shell/README.md`
