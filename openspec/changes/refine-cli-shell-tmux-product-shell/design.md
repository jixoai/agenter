...
> Its architecture conclusion that tmux session-local state is product truth is superseded by `realign-cli-shell-with-core-system-boundaries`.

## Context

The product boundary is now correct: cli-shell is an extension, and TerminalSystem is not its compositor. The product experience is still incomplete because the current tmux host only creates a split layout. It uses tmux as a process launcher, not as a product shell.

## Architecture Decision

### Chosen: tmux-native product shell

cli-shell will treat tmux as the local UI runtime:

- tmux session/window/pane identities are product-local.
- cli-shell uses its own tmux socket namespace, so product bindings never mutate the user's normal tmux server.
...
- the bottom tmux status line carries product chrome, with identity on the left and actions on the right.
- the default tmux window list is hidden for cli-shell sessions because it competes with the product action rail.
...
This keeps cli-shell outside TerminalSystem while restoring the product shell behavior users expect.

### Rejected: permanent split as the product UI

A permanent right-side room pane is not a product shell. It consumes screen width, hides the expected Chat entry, and drops the bottom status bar. It is acceptable as a fallback, not as the default.
...
Inside Chat, the titlebar offers close plus `◨` / `◧` / `⿴`. These are not Chat self-layout buttons. They are product-shell controls. For pane-to-pane left/right changes, cli-shell moves the existing pane with tmux instead of killing and restarting the OpenTUI Room process. For pane-to-cover changes, tmux cannot move a live pane into a popup, so cli-shell models that as an explicit surface conversion: close the pane and open a cover popup. This distinction must stay visible in the code and tests so future work does not accidentally turn every layout change into a Room restart.
...
Terminal write approvals do not belong inside the Room. Room is the MessageRoom surface; approvals are product-global shell chrome. cli-shell therefore owns a separate OpenTUI top surface (`shell top`) that can be launched as a tmux popup and subscribe to pending terminal approval requests. The top surface is allowed to use the same client-sdk/store APIs as Room, but its state, rendering, and mouse handling are independent. Room may nudge the product shell to open `shell top` when it observes pending approvals, but it must not render the approval card itself.

The top surface is a general product top layer, not a MessageRoom child. Today it renders terminal write approvals; later it can host product notifications without changing Room. Approval actions must be reachable by keyboard and mouse. The mouse hit test belongs to the top-layer OpenTUI renderables, not to Room absolute-coordinate regions.
...
- `managed:on/off` toggles cli-shell hosting attention for this tmux-hosted product session.
- `Help` opens the shortcut guide.
- `Chat` opens the Chat popup.

`Dock`, `Mouse`, `Shell`, and `Refresh` remain product-local expert key bindings. They should not occupy the right-side status action rail.

`managed:on/off` is a real switch, not a label. A status click dispatches the `managed` product action, which commits or settles cli-shell hosting attention and then writes the projected tmux option. This action stays in cli-shell extension code; it must never become a TerminalSystem field or a hard-coded core mode.
...
- Status line shall be installed before attach so the user sees product chrome immediately.
...
- Mouse shall be available through an explicit product key binding.
- Chat popup binding shall be present in the session.
- Chat shall be a singleton product surface: status click, key binding, dock fallback, and layout controls shall focus or transition the existing Chat surface instead of duplicating it.
...
- Status entries shall highlight `@agenter_cli_shell_active_action` so open Help/Chat/Shell state is visible in the product chrome.
- Product bindings shall pass `#{session_name}` and cli-shell session options through tmux format expansion into `tmux-action`; the real Help/Chat/managed/Dock/Mouse/Shell actions shall be executed by cli-shell extension code, not by long ad-hoc tmux command strings.
- Nested tmux formats used inside `run-shell` shall be deferred as `##{...}` when the format belongs to an inner tmux command.
- Product status styling shall explicitly set a high-contrast cli-shell status palette and must not rely on `#[default]`, because that would inherit the user's global tmux theme and can make the product chrome unreadable.
...
- cli-shell-specific UI law shall remain inside `extensions/cli-shell`.
...
The exact keys are product-local defaults and can later become config, but this change keeps them deterministic for tests and usability.
