...
- Do not move cli-shell product behavior into core terminal-system.
...
cli-shell will model enhancements such as `semanticWordSelection`, `semanticRowSelection`, `wordNavigation`, `followCursorOnInput`, and `homeEndFallback`. These flags are product-level policy consumed by the offscreen renderer/input bridge.
...
A focused test will encode the current recommended defaults for supported backends. Product runtime reads the recommendation map and enables only missing capabilities. This keeps startup simple and reproducible.
