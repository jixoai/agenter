...
The MCP system SHALL persist global configs, project enablement, live instance recovery state, snapshots, and action facts in avatar-private SQLite storage under root-workspace runtime authority. Query surfaces SHALL read from SQLite-backed facts rather than from a separate ad-hoc JSON registry.
