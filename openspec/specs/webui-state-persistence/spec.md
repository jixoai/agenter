# webui-state-persistence Specification

## Purpose

Define the durable storage selection law for WebUI state so persistence decisions follow platform rules instead of feature-local heuristics.
...
#### Scenario: Room viewer preference syncs through auth-scoped KV

- **WHEN** the operator changes the `View as` actor for a message room
- **THEN** the WebUI stores that per-room viewer preference through the auth-scoped KV plane instead of feature-local memory or settings files
- **AND** a later refresh can restore the same room viewer while stale invalid room actors fall back to current room truth
