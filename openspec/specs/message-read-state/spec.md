...
- **WHEN** a privileged Contact creates a room whose creator is user `A`
- **THEN** the room persists `A` as a durable admin seat before the room accepts messages
- **THEN** the first durable message tracks `A` inside `readContactIds` or `unreadContactIds` according to send-time state
...
Message-system SHALL persist `readContactIds` and `unreadContactIds` on each durable room message. Those arrays SHALL capture the relevant collaborators at send time and SHALL NOT be rewritten when new users join later.
...
### Requirement: Mark-read SHALL move Contact ids between frozen message arrays

Message-system SHALL advance room read-state by moving Contact ids from `unreadContactIds` to `readContactIds` on the addressed visible message and earlier visible messages, instead of recomputing history from mutable seat cursors. The same durable mark-read mutation SHALL also update Contact-global unread totals and Contact-room unread counters from the affected message range.

#### Scenario: Read progress advances without changing frozen membership
- **WHEN** a room user reads up to message `m3`
- **THEN** that user's Contact id moves from unread to read on `m3` and earlier visible messages
- **THEN** later room membership changes do not alter the already-frozen arrays on those messages

#### Scenario: Mark-read shrinks Contact unread aggregates from the same mutation
- **WHEN** Contact `A` marks room `R` read through visible message `m3`
...
### Requirement: Joined-later Contacts SHALL remain excluded from retroactive unread obligations while still being allowed to appear as readers

Message-system SHALL keep joined-later Contacts excluded from retroactive unread obligations on older messages, but an explicit later mark-read MAY append those Contacts to `readContactIds` on previously visible history without rewriting frozen `unreadContactIds`.

#### Scenario: Joined-later Contact does not inherit old unread debt
- **WHEN** Contact `C` joins after message `m1` was sent
- **THEN** `m1` does not contribute to `C`'s unread room counter
- **THEN** `m1` remains outside `C`'s retroactive unread obligation set

#### Scenario: Joined-later Contact can later become a historical reader
- **WHEN** Contact `C` later opens old history containing `m1` and marks that history read
- **THEN** `C` MAY be appended to `m1.readContactIds`
- **THEN** `m1.unreadContactIds` remains frozen to the original send-time membership

### Requirement: Client mark-read acknowledgement SHALL stay monotonic per room Contact
...
#### Scenario: Credential refresh does not duplicate Contact acknowledgement
- **WHEN** the Room route rehydrates and resolves a different credential source for the same viewer Contact
- **AND** the current visible message already includes that Contact inside `readContactIds`
- **THEN** the client keeps one Contact-scoped acknowledgement floor
- **THEN** it does not emit another `globalMarkRead` for that same Contact and message
...
- **AND** `m3.readContactIds` already includes the current viewer Contact
...
Room-facing projections SHALL derive aggregate progress from the latest visible message's frozen read arrays and SHALL expose a detailed Contact breakdown that distinguishes `unread` from `joined later`.

#### Scenario: Read ring shows partial progress
- **WHEN** some tracked Contacts have read the latest visible message and others have not
- **THEN** the projection exposes aggregate counts or progress that can render as a compact read ring
- **THEN** the detailed view still exposes the Contact breakdown behind that compact summary
