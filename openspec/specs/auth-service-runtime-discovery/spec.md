# auth-service-runtime-discovery Specification

## Purpose
Define how a local `auth-service` authority publishes objective runtime discovery facts so same-authority consumers can reuse the existing single writer without guessing ports.
