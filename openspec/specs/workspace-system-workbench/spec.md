...
### Requirement: Workspace CLI mode SHALL execute catalog commands through one dedicated shell dialog

`CLI` mode SHALL keep the grouped command catalog as the discovery surface and SHALL provide one dedicated shell dialog inside the same workspace tab for execution. The shell dialog SHALL show which shell surface will own the run, SHALL seed its first command from the selected row's `suggestedCommand`, and SHALL auto-run once on open while still allowing later typed commands on that same backend shell surface.
