...
- **低噪声 operator surface**：Studio 页面默认服务长期使用的专业操作者，而不是首次上手的婴儿式教程。主体内容只保留会改变判断、状态或下一步动作的信息；解释性文案、约束说明和恢复路径默认收纳进 `HelpHint`、tooltip 或聚焦的 empty/error state。
- **边界预算守恒**：每增加一层 card、border、shadow 或说明区，都必须承担新的结构归属。没有新增归属的视觉边界就是信息噪声，应删除或合并到已有 shell / split-detail / divider 结构里。
