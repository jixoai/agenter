# Studio Layout Prompt and Rubric

This document is the repository-level contract for dense Studio layout work. Use it before generating layout code, reviewing screenshots, or scoring a shell/page change.

## Generation Prompt

```text
You are refining Agenter Studio layout density.
...
```text
Review this Agenter Studio change against the layout contract.
...
screenshots:
  - path: packages/studio/test/storybook/__screenshots__/chat-panel-compact.png
metrics:
  asyncSurface:
    state: ready-loading
    ownsScroll: false
