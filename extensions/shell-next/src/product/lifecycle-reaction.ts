...
import type { ShellNextBindingProjection } from "./bootstrap";
import type { ShellNextRuntimeStore } from "./runtime-dependencies";

type ShellNextLifecycleReactionStore = Pick<
  ShellNextRuntimeStore,
  "archiveGlobalRoom" | "getState" | "listGlobalTerminalHistory" | "subscribe"
>;

export interface ShellNextTerminalRoomLifecycleReaction {
...
export const createShellNextTerminalRoomLifecycleReaction = (input: {
  store: ShellNextLifecycleReactionStore;
  binding: ShellNextBindingProjection;
  room: GlobalRoomEntry;
}): ShellNextTerminalRoomLifecycleReaction => {
  let disposed = false;
  let archived = false;
