...
export interface OpenComposeFrameInteractionTraceEvent {
  kind:
...
export interface OpenComposeFramePaintStats {
  durationMs: number;
  rows: number;
  spans: number;
  glyphs: number;
...
export interface OpenComposeFrameProjectionUpdate {
  lines?: readonly TerminalRenderRichLine[];
  selectionRegion?: OpenComposeFrameRenderableOptions["selectionRegion"];
  selectionRegions?: readonly OpenComposeSelectionRegion[];
  selectionSources?: readonly OpenComposeSelectionSource[];
  selectionOverlays?: readonly TerminalTransportSelectionOverlay[];
  selectionBg?: RGBA;
...
export class OpenComposeFrameRenderable extends FrameBufferRenderable {
  #lines: readonly TerminalRenderRichLine[];
  #selectionRegion: NonNullable<OpenComposeFrameRenderableOptions["selectionRegion"]> | null;
  #selectionRegions: readonly OpenComposeSelectionRegion[];
  #selectionSources: readonly OpenComposeSelectionSource[];
  #selectionOverlays: readonly TerminalTransportSelectionOverlay[];
  #selectionBg: RGBA;
  ...
  set selectionRegion(selectionRegion: OpenComposeFrameRenderableOptions["selectionRegion"]) {
  ...
  set selectionSources(selectionSources: readonly OpenComposeSelectionSource[] | undefined) {
  ...
  get lastPaintStats(): OpenComposeFramePaintStats {
    return this.#lastPaintStats;
  }
  ...
  protected paintBackendFrame(): OpenComposeFramePaintStats {
  ...
  protected syncNativeCursor(): void {
    ...
    // Terminal projections keep cursor row/col in 0-based viewport-local cells.
  ...
  protected resolveSelectionRegionForPoint(
    localX: number,
    localY: number,
  ): { x: number; y: number; width: number; height: number; owner: OpenComposeSelectionRegion["owner"] | null } {
  ...
  protected resolveFallbackSelectionRegion(): {
    ...
    width: number;
    height: number;
    owner: OpenComposeSelectionRegion["owner"] | null;
  } {
  ...
  private traceInteraction(
    ...
    extra: Partial<OpenComposeFrameInteractionTraceEvent["detail"]> = {},
  ): void {
