...
import {
  ...
  type ShellNextNavigationAvatarItem,
  type ShellNextNavigationShellItem,
} from "./navigation-model";
import { padShellNextRoomText } from "../product-room/room-model";

export interface ShellNextNavigationAppInput {
  shellItems: readonly ShellNextNavigationShellItem[];
  defaultShellIndex: number;
  needsShell: boolean;
  avatarItems: readonly ShellNextNavigationAvatarItem[];
  ...
  onComplete(selection: ShellNextNavigationSelection): void;
  onCancel?(): void;
}

export interface ShellNextNavigationSelection {
  shellName: string;
...
const shellLabel = (item: ShellNextNavigationShellItem): string => {
...
const avatarLabel = (item: ShellNextNavigationAvatarItem): string => {
...
export class ShellNextNavigationApp {
  readonly #input: ShellNextNavigationAppInput;
  ...
  constructor(input: ShellNextNavigationAppInput & { renderer: CliRenderer; ownsRenderer?: boolean }) {
    this.#input = input;
    ...
    this.#title = new TextRenderable(this.#renderer, {
      id: "shell-next-navigation-title",
      position: "absolute",
      top: 1,
      left: 2,
      width: 1,
    ...
    this.#subtitle = new TextRenderable(this.#renderer, {
      id: "shell-next-navigation-subtitle",
      position: "absolute",
      top: 2,
      left: 2,
      width: 1,
    ...
    this.#footer = new TextRenderable(this.#renderer, {
      id: "shell-next-navigation-footer",
      position: "absolute",
      top: 1,
      left: 2,
      width: 1,
    ...
    this.#dialogBox = new BoxRenderable(this.#renderer, {
      id: "shell-next-navigation-dialog",
      position: "absolute",
      top: 4,
      left: 4,
      width: 1,
    ...
    this.#dialogTitle = new TextRenderable(this.#renderer, {
      id: "shell-next-navigation-dialog-title",
      position: "absolute",
      top: 1,
      left: 2,
      width: 1,
    ...
    this.#dialogInput = new InputRenderable(this.#renderer, {
      id: "shell-next-navigation-dialog-input",
      position: "absolute",
      top: 3,
      left: 2,
      width: 1,
    ...
    this.#dialogStatus = new TextRenderable(this.#renderer, {
      id: "shell-next-navigation-dialog-status",
      position: "absolute",
      top: 5,
      left: 2,
      width: 1,
  ...
  #renderRows(width: number, height: number): void {
    ...
    const subtitle =
      this.#step === "shell"
        ? "Choose a live shell-next Shell or create the next shell-N."
        : "Choose an Avatar or create one by nickname.";
    const contentWidth = Math.max(1, width - 5);
    this.#title.content = padShellNextRoomText(title, contentWidth);
    this.#subtitle.content = padShellNextRoomText(this.#notice ?? subtitle, contentWidth);
    this.#footer.content = padShellNextRoomText("↑/↓ select | Enter confirm | Esc cancel | Mouse click", contentWidth);
    ...
    while (this.#rows.length < availableRows) {
      const row = new TextRenderable(this.#renderer, {
        id: `shell-next-navigation-row-${this.#rows.length}`,
        position: "absolute",
    ...
    for (const [rowIndex, row] of this.#rows.entries()) {
      ...
      if (!item) {
        row.content = padShellNextRoomText("", contentWidth);
        row.bg = "#0f172a";
        continue;
      }
      const selected = itemIndex === selectedIndex;
      const label = this.#step === "shell" ? shellLabel(item as ShellNextNavigationShellItem) : avatarLabel(item as ShellNextNavigationAvatarItem);
      row.content = padShellNextRoomText(`${selected ? ">" : " "} ${label}`, contentWidth);
  ...
  #renderDialog(): void {
    ...
    this.#dialogTitle.content = padShellNextRoomText("Create Avatar", width);
    this.#dialogStatus.content = padShellNextRoomText("Enter create + select | Esc back", width);
