...
import { buildShellNextStatusbarLeft, type ShellNextStatusbarState } from "../renderable-mux/statusbar";
import type { OpenTuiRenderableSurface } from "../renderable-mux/pane-source";

export interface ShellNextStatusSurfaceInput {
  readonly renderer: CliRenderer;
  readonly node: ChildLayoutNode;
  readonly getState: () => ShellNextStatusbarState;
}
...
export class ShellNextStatusSurface implements OpenTuiRenderableSurface {
  readonly #renderer: CliRenderer;
  readonly #getState: () => ShellNextStatusbarState;
  readonly #root: TextRenderable;
  #node: ChildLayoutNode;

  constructor(input: ShellNextStatusSurfaceInput) {
  ...
  refresh(): void {
    this.#root.content = truncateLine(buildShellNextStatusbarLeft(this.#getState()), Number(this.#root.width));
