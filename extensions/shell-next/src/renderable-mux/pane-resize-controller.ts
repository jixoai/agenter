...
export class ShellNextPaneResizeController {
  readonly #renderer: CliRenderer;
  readonly #layout: RootLayout;
  readonly #onLayoutChanged: () => void;
  ...
  constructor(input: ShellNextPaneResizeControllerInput) {
  ...
  #createHandle(region: ResizeRegion): ResizeHandleRenderables {
    const hitTarget = new BoxRenderable(this.#renderer, {
      id: `shell-next-resizer-hit-${region.id}`,
      position: "absolute",
      width: region.width,
    ...
    const visual = new TextRenderable(this.#renderer, {
      id: `shell-next-resizer-visual-${region.id}`,
      position: "absolute",
