...
import { ShellNextResizeSendScheduler } from "../src/terminal-projection/resize-send-scheduler";

const wait = (ms: number): Promise<void> => new Promise((resolve) => setTimeout(resolve, ms));

describe("Feature: shell-next pane-level resize send scheduling", () => {
  test("Scenario: Given multiple geometry changes inside the 200ms debounce window When the window stays open Then no backend resize is sent early", async () => {
    ...
    const scheduler = new ShellNextResizeSendScheduler({
      delayMs: 200,
      send: (size) => {
        delivered.push(size);
      },
