...
describe("Feature: Studio static root", () => {
  ...
  test("Scenario: Given Studio owns static serving When starting the host Then health and runtime env stay product-owned", async () => {
    const layout = createPackageLayout({ build: true });
    ...
    const healthJson = (await health.json()) as { ok?: boolean; product?: string };
    expect(health.status).toBe(200);
    expect(healthJson).toEqual({ ok: true, product: "studio" });
