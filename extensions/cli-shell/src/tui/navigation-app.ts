...
export interface CliShellNavigationAppInput {
  shellItems: readonly CliShellNavigationShellItem[];
  defaultShellIndex: number;
  needsShell: boolean;
  avatarItems: readonly CliShellNavigationAvatarItem[];
  ...
  onComplete(selection: CliShellNavigationSelection): void;
  onCancel?(): void;
}

export interface CliShellNavigationSelection {
  shellName: string;
...
const shellLabel = (item: CliShellNavigationShellItem): string => {
...
const avatarLabel = (item: CliShellNavigationAvatarItem): string => {
...
export class CliShellNavigationApp {
  readonly #input: CliShellNavigationAppInput;
  ...
  constructor(input: CliShellNavigationAppInput & { renderer: CliRenderer; ownsRenderer?: boolean }) {
    this.#input = input;
    ...
    this.#subtitle = new TextRenderable(this.#renderer, {
      id: "cli-shell-navigation-subtitle",
      position: "absolute",
      top: 2,
      left: 2,
      width: 1,
    ...
    this.#footer = new TextRenderable(this.#renderer, {
      id: "cli-shell-navigation-footer",
      position: "absolute",
      top: 1,
      left: 2,
      width: 1,
    ...
    this.#dialogBox = new BoxRenderable(this.#renderer, {
      id: "cli-shell-navigation-dialog",
      position: "absolute",
      top: 4,
      left: 4,
      width: 1,
    ...
    this.#dialogTitle = new TextRenderable(this.#renderer, {
      id: "cli-shell-navigation-dialog-title",
      position: "absolute",
      top: 1,
      left: 2,
      width: 1,
    ...
    this.#dialogInput = new InputRenderable(this.#renderer, {
      id: "cli-shell-navigation-dialog-input",
      position: "absolute",
      top: 3,
      left: 2,
      width: 1,
    ...
    this.#dialogStatus = new TextRenderable(this.#renderer, {
      id: "cli-shell-navigation-dialog-status",
      position: "absolute",
      top: 5,
      left: 2,
      width: 1,
  ...
  #renderRows(width: number, height: number): void {
    ...
    const subtitle =
      this.#step === "shell"
        ? "Choose a live cli-shell Shell or create the next shell-N."
        : "Choose an Avatar or create one by nickname.";
    const contentWidth = Math.max(1, width - 5);
    this.#title.content = padCliShellRoomText(title, contentWidth);
    this.#subtitle.content = padCliShellRoomText(this.#notice ?? subtitle, contentWidth);
    this.#footer.content = padCliShellRoomText("↑/↓ select | Enter confirm | Esc cancel | Mouse click", contentWidth);
    ...
    while (this.#rows.length < availableRows) {
      const row = new TextRenderable(this.#renderer, {
        id: `cli-shell-navigation-row-${this.#rows.length}`,
        position: "absolute",
    ...
    for (const [rowIndex, row] of this.#rows.entries()) {
      ...
      if (!item) {
        row.content = padCliShellRoomText("", contentWidth);
        row.bg = "#0f172a";
        continue;
      }
      const selected = itemIndex === selectedIndex;
      const label = this.#step === "shell" ? shellLabel(item as CliShellNavigationShellItem) : avatarLabel(item as CliShellNavigationAvatarItem);
      row.content = padCliShellRoomText(`${selected ? ">" : " "} ${label}`, contentWidth);
  ...
  #renderDialog(): void {
    ...
    this.#dialogTitle.content = padCliShellRoomText("Create Avatar", width);
    this.#dialogStatus.content = padCliShellRoomText("Enter create + select | Esc back", width);
...
export const startCliShellNavigationApp = async (
  input: CliShellNavigationAppInput,
): Promise<{ app: CliShellNavigationApp; renderer: CliRenderer }> => {
  const renderer = input.renderer ?? (await createCliRenderer({ exitOnCtrlC: false, useMouse: true }));
  const app = new CliShellNavigationApp({
    ...input,
    renderer,
