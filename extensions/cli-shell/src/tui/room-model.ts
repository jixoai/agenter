...
export interface CliShellRoomRenderRow {
  key: string;
  plainText: string;
...
export const padCliShellRoomText = (text: string, width: number): string => {
  const clipped = clipText(text, width);
  return `${clipped}${" ".repeat(Math.max(0, Math.trunc(width) - measure(clipped)))}`;
};

export const wrapCliShellRoomText = (input: { text: string; width: number; prefix?: string }): string[] => {
...
export const buildCliShellRoomRows = (input: {
  messages: readonly GlobalRoomMessage[];
  avatarActorId: string;
  width: number;
}): CliShellRoomRenderRow[] => {
  const rows: CliShellRoomRenderRow[] = [];
  ...
  for (const message of input.messages) {
    ...
    const wrapped = wrapCliShellRoomText({
