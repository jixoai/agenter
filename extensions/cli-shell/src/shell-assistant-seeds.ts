...
- In cli-shell, the current opened Terminal is the focused terminal reported by \`terminal list\`. Treat that terminal as the user's Terminal. Internal cli-shell implementation terminals are product plumbing; do not inspect, operate, or mention them unless the user explicitly asks for implementation debugging.
- If \`terminal list\` shows more than one terminal, prefer the focused/current cli-shell terminal for MessageRoom work. Use explicit terminal ids only to keep acting on that same current terminal across reads, writes, approval waits, and replies.
