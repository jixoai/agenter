...
import { ShellButtonPressController } from "../src/renderable-mux/button-press-controller";
...
describe("Feature: shell shared Button click commitment", () => {
  test("Scenario: Given one button is pressed When mouseup completes on that same button Then the action fires once, but mousedown alone does not fire it", () => {
    ...
    const controller = new ShellButtonPressController({
      resolveAction: (event) => {
