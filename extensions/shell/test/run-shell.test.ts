...
import { bootstrapShellRoom } from "../src/product/bootstrap";

import { createShellRuntimeApprovalStore } from "../src/product/approval-store";
import { parseShellArgs } from "../src/product/argv";
import type { ShellProductRunDependencies } from "../src/product/runtime";
...
const createTestDependencies = (input: {
  ...
  tty?: boolean;
  bootstrap?: ShellProductRunDependencies["bootstrapRoom"];
  startApp?: ShellProductRunDependencies["startApp"];
  navigation?: ShellProductRunDependencies["startNavigationTui"];
  savedSettings?: ShellSettings[];
  output?: string[];
  liveSourceCalls?: Array<{ id: string; terminalId: string; transportUrl: string }>;
  readHeartbeatStatus?: ShellProductRunDependencies["readHeartbeatStatus"];
}): ShellProductRunDependencies => ({
  createClient: () => ({ close() {} }) as ReturnType<ShellProductRunDependencies["createClient"]>,
  createStore: () => input.store as unknown as ReturnType<ShellProductRunDependencies["createStore"]>,
...
describe("Feature: shell product runtime argv", () => {
  ...
  test("Scenario: Given explicit view selection When parsing argv Then shell treats view as a product attach selector", () => {
...
describe("Feature: shell product runtime bootstrap", () => {
  ...
  test("Scenario: Given non-TTY shell attach has explicit selectors When running Then it bootstraps product resources and prints a summary", async () => {
    const store = new FakeShellStore();
  ...
  test("Scenario: Given background-run exits product attach When reconnecting Then shell reuses the still-live terminal binding", async () => {
  ...
  test("Scenario: Given shell bootstraps a product terminal When TerminalSystem creates the binding Then git-backed history is requested", async () => {
  ...
  test("Scenario: Given product attach exits by terminate When the bound terminal is killed Then shell archives the bound room through public APIs", async () => {
    const store = new FakeShellStore();
    seedAvatar(store, "bangeel");
    const dependencies = createTestDependencies({
      store,
  ...
  test("Scenario: Given attached terminal has no transport URL When creating the product-bound source Then shell fails the product source policy", async () => {
    const store = new FakeShellStore();
    seedAvatar(store, "bangeel");
    const dependencies = createTestDependencies({
      store,
  ...
  test("Scenario: Given cleanup command When running shell Then cleanup ignores tmux sessions and reports product resources", async () => {
    const store = new FakeShellStore();
    seedAvatar(store, "bangeel");
    await bootstrapShellRoom({
      store,
