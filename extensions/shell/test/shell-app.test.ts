...
describe("Feature: shell app runtime", () => {
  ...
  test("Scenario: Given a product-bound terminal policy without split capability When Ctrl+B then N is pressed Then shell does not create a local terminal substitute", async () => {
    setup = await createTestRenderer({ width: 64, height: 18, useMouse: true, kittyKeyboard: true });
    const recordings: RecordingSource[] = [];
  ...
  test("Scenario: Given a product-bound terminal pane When Run in background is clicked Then shell only closes the UI attachment", async () => {
    setup = await createTestRenderer({ width: 64, height: 18, useMouse: true, kittyKeyboard: true });
    const recordings: RecordingSource[] = [];
    const createProductBoundSource = (id: string): PaneSource => {
      const recording = createRecordingProtocolSource(id);
  ...
  test("Scenario: Given a product-bound terminal pane When Terminate terminal is clicked Then shell terminates the attached terminal before exiting", async () => {
    setup = await createTestRenderer({ width: 64, height: 18, useMouse: true, kittyKeyboard: true });
    const recordings: RecordingSource[] = [];
    const createProductBoundSource = (id: string): PaneSource => {
      const recording = createRecordingProtocolSource(id);
