...
describe("Feature: shell terminal input ownership boundary", () => {
  test("Scenario: Given ShellApp handles product keys When auditing source Then terminal input semantics stay below the app layer", () => {
    const appSource = readSource("app/shell-app.ts");
  ...
  test("Scenario: Given terminal sources own backend input When auditing source Then host input controllers live in those sources", () => {
    for (const relativePath of [
      ...
      "sources/shell-live-terminal-source.ts",
    ]) {
