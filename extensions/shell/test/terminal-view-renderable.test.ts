...
describe("Feature: shell terminal semantic selection ownership", () => {
  test("Scenario: Given backend word selection becomes visible during a semantic double click When the click sequence completes Then shell does not clear it on release", async () => {
  ...
  test("Scenario: Given backend line selection becomes visible during a semantic triple click When the click sequence completes Then shell does not clear it on release", async () => {
    activeRenderer = await createTestRenderer({ width: 40, height: 4, useMouse: true });
    const semanticPoints: Array<{ ownerId: string; row: number; col: number }> = [];
    const hostInput = createTerminalHostInputController();
    const lines = ["$ echo alpha beta gamma", "shell prompt"];
  ...
  test("Scenario: Given a scrolled terminal frame When a word is double clicked Then Shell sends the absolute backend row to the selection controller", async () => {
    activeRenderer = await createTestRenderer({ width: 40, height: 4, useMouse: true });
    const hostInput = createTerminalHostInputController();
  ...
  test("Scenario: Given a scrolled terminal frame When the operator drags across visible rows Then Shell sends absolute backend rows", async () => {
