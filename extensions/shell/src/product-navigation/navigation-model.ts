...
import { normalizeShellName } from "../product/argv";
import { SHELL_DEFAULT_AVATAR, SHELL_PRODUCT_ID } from "../product/product";
import type { ShellSettings } from "../product-room/settings";
...
const isShellTerminal = (terminal: GlobalTerminalEntry): boolean =>
  ...
  terminal.metadata?.productId === SHELL_PRODUCT_ID &&
...
const readKnownShellResourceKey = (terminal: GlobalTerminalEntry): string | null => {
  if (terminal.metadata?.productId !== SHELL_PRODUCT_ID || terminal.metadata.ownerSystem !== "terminal-system") {
    return null;
  }
  const resourceKey = readStringMetadata(terminal.metadata, "resourceKey");
