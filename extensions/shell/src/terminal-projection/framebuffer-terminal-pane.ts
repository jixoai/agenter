...
import { SHELL_CLIPBOARD_TARGETS } from "../renderable-mux/host-copy";
...
import { ShellResizeSendScheduler } from "./resize-send-scheduler";
...
export interface ShellFrameBufferTerminalPaneInput {
...
export class ShellFrameBufferTerminalPane {
  readonly #renderer: CliRenderer;
  readonly #source: TerminalProtocolPaneSource;
  readonly #root: BoxRenderable;
  readonly #frame: OpenComposeTerminalFrameRenderable;
  readonly #chrome: ShellPaneChromeController;
  readonly #input: Omit<ShellFrameBufferTerminalPaneInput, "renderer" | "node" | "source">;
  ...
  readonly #resizeScheduler: ShellResizeSendScheduler;
  readonly #buttonPress: ShellButtonPressController<string>;

  constructor(input: ShellFrameBufferTerminalPaneInput) {
    ...
    this.#resizeScheduler = new ShellResizeSendScheduler({
    ...
    if (typeof copied === "string") {
      return (
        ...
        this.#renderer.copyToClipboardOSC52(
          copied,
          target === "primary" ? SHELL_CLIPBOARD_TARGETS.primary : SHELL_CLIPBOARD_TARGETS.clipboard,
        )
      );
    }
    return copied;
...
export const createShellFrameBufferTerminalPane: TerminalPaneFactory = (input: TerminalPaneFactoryInput) =>
  new ShellFrameBufferTerminalPane(input);
