...
import { SHELL_DEFAULT_AVATAR, SHELL_PRODUCT_ID, createShellProductRuntimeClient } from "./product";
import {
...
export interface ShellStore extends ProductExtensionRuntimeStore {
...
export type ShellProductHostStore = ShellInteractiveHostStore;
...
export interface ShellBindingProjection {
  productId: typeof SHELL_PRODUCT_ID;
  resourceKey: string;
  terminalId: string;
  roomId: string;
...
const resolveSelectedAvatar = async (
  ...
  const runtimeClient = createShellProductRuntimeClient(store);
...
const resolveShellAvatarRuntime = async (input: {
  ...
  runtimeClient: ReturnType<typeof createShellProductRuntimeClient>;
  avatar: GlobalAvatarCatalogEntry;
  avatarCreated: boolean;
  session: SessionEntry;
  clearedRuntimeSessionIds: string[];
  ...
  const runtimeClient = createShellProductRuntimeClient(input.store);
