...
export interface ShellHeartbeatStatusStore
  extends Pick<ShellStore, "autoLogin" | "setAuthToken"> {
...
const STATUS_ICONS: Record<ShellToolbarStatusKind, string> = {
  idle: "◉",
  "message-tool": "✉",
  thinking: "◌",
  "text-progressing": "✎",
...
export const resolveShellToolbarStatus = (
  groups: readonly HeartbeatGroupItem[],
): ShellToolbarStatusKind => {
...
export const summarizeShellHeartbeat = (input: {
  groups: readonly HeartbeatGroupItem[];
  shellName: string;
  connected: boolean;
  ...
  if (!input.connected) {
    return SHELL_HEARTBEAT_COPY.disconnected;
  }
  if (input.observationReady === false) {
    return SHELL_HEARTBEAT_COPY.observationPending;
  }

  const latest = resolveLatestHeartbeatPart(input.groups);
  if (!latest.part || !latest.item) {
    return input.observationReady
      ? SHELL_HEARTBEAT_COPY.observationReady
      : SHELL_HEARTBEAT_COPY.observationPending;
  }
...
export const resolveShellToolbarStatusIcon = (status: ShellToolbarStatusKind): string => STATUS_ICONS[status];
...
export const formatShellHeartbeatStatus = (input: {
  groups: readonly HeartbeatGroupItem[];
  shellName: string;
  connected: boolean;
  observationReady?: boolean;
}): string => {
  const status = resolveShellToolbarStatus(input.groups);
  const icon = resolveShellToolbarStatusIcon(status);
  const summary = clipHeartbeatPreview(summarizeShellHeartbeat(input));
  return normalizeStatusText(`${icon} ${summary}`);
};

export const readShellHeartbeatStatus = async (input: {
  store: ShellHeartbeatStatusStore;
  ...
  try {
    ...
    if (!autoLogin.ok) {
      return formatShellHeartbeatStatus({
        groups: [],
