...
export interface ShellRoomRenderRow {
  key: string;
  plainText: string;
...
export const padShellRoomText = (text: string, width: number): string => {
  const clipped = clipText(text, width);
  return `${clipped}${" ".repeat(Math.max(0, Math.trunc(width) - measure(clipped)))}`;
};

export const wrapShellRoomText = (input: { text: string; width: number; prefix?: string }): string[] => {
...
export const buildShellRoomRows = (input: {
  messages: readonly GlobalRoomMessage[];
  avatarActorId: string;
  width: number;
}): ShellRoomRenderRow[] => {
  const rows: ShellRoomRenderRow[] = [];
  ...
  for (const message of input.messages) {
    ...
    const wrapped = wrapShellRoomText({
