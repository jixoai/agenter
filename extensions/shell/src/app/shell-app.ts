...
import { createShellProductSurface, toggleShellProductSurface } from "./product-surface-actions";
...
export class ShellApp implements ShellAppController {
  ...
  #toggleProductSurface(kind: "help" | "chat"): boolean {
    ...
    return toggleShellProductSurface({
      kind,
      renderer: this.#renderer,
      layout: this.#layout,
      mux: this.#mux,
  ...
  #createRootSurface(node: ChildLayoutNode): OpenTuiRenderableSurface {
    ...
    if (sourceId === "view-help") {
      return createShellProductSurface({
        kind: "help",
        renderer: this.#renderer,
        node,
      });
