...
- App-owned assistant prompt 是 avatar-first 的：`AGENTER.mdx` 必须按 global Avatar principal root 寻址；raw recording 默认通过 NoteSystem 写入 active `AVATAR_HOME` 下的 note facts。NoteSystem 的 identity/query authority 是 SQLite-backed notebook -> section -> page model，文件 artifact 只作为人类可读存储。legacy memory files 只是用户资产或未来 derivation 输入，不再作为 app-shell 默认记录 API；project workspace 只能作为 cwd/mount/grant/workbench/exec/tool surface 或显式 workspace-private overlay，不能成为默认人格/关系记录 owner。
