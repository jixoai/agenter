...
- MCP system 是 Avatar-owned durable system atom，不是 SessionRuntime-owned atom，也不是 provider-side hidden tool injection：MCP 全局配置、项目启用、实例生命周期、project-local discovery snapshot、SQL 查询与 tool 调用都由按 Avatar principal 分区的 mcpSystem 统一拥有；AvatarRuntime 只能投影 root-workspace `mcp` CLI/API，不得创建或持有 MCP truth。
...
- `MCP` 是 mcpSystem 的 inspection/control workbench：它通过 client-sdk/runtime-store typed MCP facade 读取 Avatar-owned global registry 与 exact-project projection，并以 `Configs / Avatars` page-toolbar tabs 分离 config truth 与 Avatar-owned ownership projection；`Configs` 固定是跨 Avatar 的 config-first list-detail，owner Avatar 是 config 自身事实并在 row/detail/new form 中显式呈现；`Avatars` 固定是 Avatar-first overview，并用标准 avatar affordance 做 ownership projection。Studio 不得 import `app-server` MCP internals、不得把 global config 创建合并成 project enablement、也不得用持续教程文案或嵌套卡片替代 avatar/project/lifecycle/snapshot/action facts。轻量 `Inspect` 必须镜像 runtime `mcp probe`：使用 isolated probe session 做 connect、ping、tool/resource/prompt/app/template 级调试，并以 HelpHint/Raw envelope 暴露 CLI 相关性，不得形成 GUI-only MCP 协议分支。
...
- Agenter CLI 的公开发布法则是 `agenter` wrapper shell + 显式平台包矩阵 `@agenter/cli-{darwin-(arm64|x64)|linux-(arm64|x64)-(gnu|musl)|win32-(arm64|x64)}`；launcher/runtime source authority 始终保留在 private `@agenter/cli`，公开平台包只是二进制分发原子。
- Agenter native CLI 的 target / package / archive naming truth 由 `scripts/binaries/agenter-cli-artifacts.ts` 拥有；GitHub release binary archives 是 canonical binary truth，npm wrapper 与 Homebrew formula 都只能投影这份 truth，不能各自再维护第二套平台矩阵。
